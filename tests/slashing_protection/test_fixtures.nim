...
type
  ...
  TestStep = object
    should_succeed: bool ## Is "interchange" given a valid import
    ...
    interchange: SPDIR
    blocks: seq[CandidateBlock] ## Blocks to try as proposer after DB is imported
...
chronicles.formatIt CandidateBlock:
  it.toHexLogs
chronicles.formatIt CandidateVote:
  it.toHexLogs
...
const InterchangeTestsDir =
  FixturesDir / "tests-slashing-v5.3.0" / "tests" / "generated"
const TestDir = ""
const TestDbPrefix = "test_slashprot_"

proc statusOkOrDuplicateOrMinSlotViolation(
    status: Result[void, BadProposal], candidate: CandidateBlock
): bool =
  ...
  if status.error.kind == DoubleProposal and
    ...
    warn "Block already exists in the DB", candidateBlock = candidate
    return true
    ...
    warn "Block violates low watermark requirement. It might be an already pruned block.",
      candidateBlock = candidate, error = status.error
    return true
  return false

proc statusOkOrDuplicateOrMinEpochViolation(
    status: Result[void, BadVote], candidate: CandidateVote
): bool =
  ...
  if status.error.kind == DoubleVote and
    ...
    warn "Attestation already exists in the DB", candidateAttestation = candidate
    return true
    ...
    warn "Attestation violates low watermark requirement. It might be an already pruned attestation.",
      candidateAttestation = candidate, error = status.error
    return true
  return false
...
proc runTest(identifier: string) =
  ...
  let db =
    SlashingProtectionDB.init(Eth2Digest t.genesis_validators_root, TestDir, dbname)
  ...
  for step in t.steps:
    ...
    if not step.should_succeed:
      doAssert siFailure == status, "Unexpected error:\n" & "    " & $status & "\n"
    elif step.contains_slashable_data:
      doAssert status in {siPartial, siSuccess},
        "Unexpected error:\n" & "    " & $status & "\n"
    else:
      doAssert siSuccess == status, "Unexpected error:\n" & "    " & $status & "\n"
    ...
    for blck in step.blocks:
      ...
      let status = db.db_v2.checkSlashableBlockProposal(
        Opt.none(ValidatorIndex), pubkey, Slot blck.slot
      )
      if blck.should_succeed:
        ...
        if status.isOk(): # Skip duplicates
          ...
          doAssert status.isOk(), "Failure to register block: " & $status
      else:
    ...
    for att in step.attestations:
      ...
      let status = db.db_v2.checkSlashableAttestation(
        Opt.none(ValidatorIndex), pubkey, Epoch att.source_epoch, Epoch att.target_epoch
      )
      if att.should_succeed:
        ...
        if status.isOk(): # Skip duplicates
          ...
          doAssert status.isOk(), "Failure to register attestation: " & $status
      else:
        doAssert status.isErr(),
          "Unexpected success:\n" & "    " & $status & "\n" & "    for " &
            $toHexLogs(att)
...
suite "Slashing Interchange tests " & preset():
  for kind, path in walkDir(InterchangeTestsDir, relative = true, checkDir = true):
