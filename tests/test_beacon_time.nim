...
import chronos/timer, unittest2, ../beacon_chain/spec/beacon_time
...
suite "Beacon time":
  template doBasicsTest(timeParams: TimeParams) =
    ...
    check:
      ...
      FAR_FUTURE_SLOT == FAR_FUTURE_SLOT.epoch.start_slot()
      FAR_FUTURE_SLOT == FAR_FUTURE_SLOT.sync_committee_period.start_slot()
      FAR_FUTURE_EPOCH == FAR_FUTURE_EPOCH.start_slot().epoch()
      FAR_FUTURE_SLOT ==
        FAR_FUTURE_SLOT.start_beacon_time(timeParams).slotOrZero(timeParams)
      FAR_FUTURE_PERIOD == FAR_FUTURE_PERIOD.start_epoch().sync_committee_period()
      FAR_FUTURE_PERIOD == FAR_FUTURE_PERIOD.start_slot().sync_committee_period()

      BeaconTime(ns_since_genesis: -10000000000).slotOrZero(timeParams) == GENESIS_SLOT
      Slot(5).since_epoch_start() == 5
      (Epoch(42).start_slot() + 5).since_epoch_start() == 5

      Slot(5).start_beacon_time(timeParams) > Slot(4).start_beacon_time(timeParams)
