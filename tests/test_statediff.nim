...
from ../beacon_chain/statediff import applyDiff, diffStates, getBeaconStateDiffSummary
...
func diffStates(state0, state1: capella.BeaconState): BeaconStateDiff =
  ...
  doAssert state1.historical_summaries.len - state0.historical_summaries.len in [0, 1]
    ...
    for i in 0 ..< testStates.len:
      for j in (i + 1) ..< testStates.len:
        ...
        let
          ...
          diff =
            diffStates(testStates[i].capellaData.data, testStates[j].capellaData.data)
