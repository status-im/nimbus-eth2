...
suite "Quarantine" & preset():
  ...
  proc cmp(a: openArray[BlobSidecar], b: openArray[ref BlobSidecar]): bool =
    if len(a) != len(b):
      return false
  ...
  proc runDataSidecarTest(
    ...
    echo "0"
    echo len(sidecars.toOpenArray(offsets[0][0], offsets[0][1]))

    quarantine.putDataSidecars(broots[0], sidecars)
