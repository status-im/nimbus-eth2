...
suite "Inclusion list pool" & preset():
  ...
  test "Stores transactions for the slot" & preset():
    let
      ...
      il = makeInclusionList(
        slot, member, dependentRoot, [makeTx([byte 0x01, 0x02])])
    ...
    let txs = pool[].getInclusionListTransactions(
      slot, dependentRoot, only_timely = true)
    check:
      txs.len == 1
      txs[0] == makeTx([byte 0x01, 0x02])

  test "Lists are keyed by dependent root" & preset():
    let il = makeInclusionList(
      slot, committee[0], dependentRoot, [makeTx([byte 0x01])])

    check:
      pool[].addInclusionList(il, is_timely = true, wallTime)
      pool[].getInclusionListTransactions(
        slot, ZERO_HASH, only_timely = true).len == 0
      pool[].isInclusionListBitsInclusive(
        slot, ZERO_HASH, committee, default(InclusionListBits),
        only_timely = true)
      not pool[].isInclusionListBitsInclusive(
        slot, dependentRoot, committee, default(InclusionListBits),
        only_timely = true)
  ...
  test "Accepts two distinct lists then drops the third" & preset():
    let
      vi = committee[0]
      il1 = makeInclusionList(slot, vi, dependentRoot, [makeTx([byte 0x01])])
      il2 = makeInclusionList(slot, vi, dependentRoot, [makeTx([byte 0x02])])
      il3 = makeInclusionList(slot, vi, dependentRoot, [makeTx([byte 0x03])])
  ...
  test "Stale slots are pruned" & preset():
    ...
    check:
      pool[].addInclusionList(il, is_timely = true, wallTime)
      pool[].getInclusionListTransactions(
        slot, dependentRoot, only_timely = true).len == 1
    ...
    let
      ...
      future = makeInclusionList(
        slot + 5, committee[1], dependentRoot, [makeTx([byte 0x02])])
  ...
  test "A list for a future slot is rejected" & preset():
    let il = makeInclusionList(
      slot + 1, committee[0], dependentRoot, [makeTx([byte 0x01])])

    check:
      not pool[].addInclusionList(il, is_timely = true, wallTime)
      pool[].numSeen(slot + 1, committee[0]) == 0

  test "A list for the next slot within clock disparity is accepted" & preset():
    let
      il = makeInclusionList(
        slot, committee[0], dependentRoot, [makeTx([byte 0x01])])
    ...
    check:
      pool[].addInclusionList(il, is_timely = true, wallTime)
      pool[].addInclusionList(next, is_timely = true, wallEarly)
      pool[].getInclusionListTransactions(
        slot, dependentRoot, only_timely = true).len == 1
      pool[].getInclusionListTransactions(
        slot + 1, dependentRoot, only_timely = true).len == 1
  ...
  test "Serves stored lists, signature included" & preset():
    ...
    let il = makeInclusionList(
      slot, committee[0], dependentRoot, [makeTx([byte 0x01])], signature)
    check pool[].addInclusionList(il, is_timely = true, wallTime)

    let served = pool[].getInclusionLists(
      slot, dependentRoot, [committee[0]], maxLists = 16)
    check:
      served.len == 1
      served[0] == il

    # A list is only served for the dependent root it was collected under ...
    check pool[].getInclusionLists(
      slot, ZERO_HASH, [committee[0]], maxLists = 16).len == 0

    # ... for the slot it belongs to ...
    check pool[].getInclusionLists(
      slot + 1, dependentRoot, [committee[0]], maxLists = 16).len == 0

    # ... and for validators the requester actually asked about.
    check pool[].getInclusionLists(
      slot, dependentRoot, [committee[0] + 1000], maxLists = 16).len == 0
  ...
  test "Untimely lists are still served over req/resp" & preset():
    ...
    check:
      ...
      pool[].getInclusionLists(
        slot, dependentRoot, [committee[0]], maxLists = 16).len == 1
  ...
  test "Equivocators are not served" & preset():
    ...
    check:
      pool[].addInclusionList(il1, is_timely = true, wallTime)
      pool[].getInclusionLists(
        slot, dependentRoot, [vi], maxLists = 16).len == 1
    ...
    check:
      pool[].addInclusionList(il2, is_timely = true, wallTime)
      pool[].getInclusionLists(
        slot, dependentRoot, [vi], maxLists = 16).len == 0
  ...
  test "Response is deduplicated and capped" & preset():
    let
      ...
      il1 = makeInclusionList(
        slot, committee[1], dependentRoot, [makeTx([byte 0x02])])
    ...
    check pool[].getInclusionLists(
      slot, dependentRoot, [committee[0], committee[0], committee[1]],
      maxLists = 16).len == 2
