...
suite "Attestation pool processing" & preset():
  ...
  test "Attestation from different branch" & preset():
    ...
    template fillToEpoch(state: ref ForkedHashedBeaconState, cache: var StateCache) =
      while getStateField(state[], slot).epoch <= epoch:
        ...
        check dag.addHeadBlock(verifier, blck.phase0Data, OnPhase0BlockAdded(nil)).isOk
  ...
  test "Can add and retrieve simple attestations" & preset():
    let
      ...
      attestation = makeAttestation(state[], state[].latest_block_root, bc0[0], cache)
    ...
    check:
      ...
      toSeq(pool[].attestations(Opt.none(Slot), Opt.none(CommitteeIndex))) ==
        @[attestation]
      toSeq(
        pool[].attestations(Opt.some(attestation.data.slot), Opt.none(CommitteeIndex))
      ) == @[attestation]
      toSeq(
        pool[].attestations(
          Opt.some(attestation.data.slot),
          Opt.some(attestation.data.index.CommitteeIndex),
        )
      ) == @[attestation]
      toSeq(
        pool[].attestations(
          Opt.none(Slot), Opt.some(attestation.data.index.CommitteeIndex)
        )
      ) == @[attestation]
      toSeq(
        pool[].attestations(
          Opt.some(attestation.data.slot + 1), Opt.none(CommitteeIndex)
        )
      ) == []
      toSeq(
        pool[].attestations(
    ...
    let
      root1 = addTestBlock(
        state[], cache, attestations = attestations, nextSlot = false
      ).phase0Data.root
    ...
    let att2 = makeAttestation(state[], root1, bc1[1], cache)
    pool[].addAttestation(
      att2, @[bc1[1]], att2.loadSig, att2.data.slot.start_beacon_time
    )

    let combined = pool[].getAttestationsForBlock(state[], cache)
    ...
    check:
      ...
      pool[]
      .getAggregatedAttestation(2.Slot, 0.CommitteeIndex)
      .get().aggregation_bits
      .countOnes() == 2

      pool[]
      .getAggregatedAttestation(2.Slot, hash_tree_root(att2.data))
      .get().aggregation_bits
      .countOnes() == 2
  ...
  test "Working with aggregates" & preset():
    ...
    var
      att0 = makeAttestation(state[], state[].latest_block_root, bc0[0], cache)
      att0x = att0
      att1 = makeAttestation(state[], state[].latest_block_root, bc0[1], cache)
      att2 = makeAttestation(state[], state[].latest_block_root, bc0[2], cache)
      att3 = makeAttestation(state[], state[].latest_block_root, bc0[3], cache)
  ...
  test "Everyone voting for something different" & preset():
    ...
    check:
      ...
      pool[].getAttestationsForBlock(state[], cache).lenu64() == MAX_ATTESTATIONS

      pool[]
      .getAggregatedAttestation(getStateField(state[], slot) - 1, 0.CommitteeIndex)
      .isSome()
  ...
  test "Attestations may arrive in any order" & preset():
    ...
    let
      ...
      attestation0 = makeAttestation(state[], state[].latest_block_root, bc0[0], cache)
    ...
    let
      ...
      attestation1 = makeAttestation(state[], state[].latest_block_root, bc1[0], cache)
  ...
  test "Attestations should be combined" & preset():
    ...
    let
      ...
      attestation0 = makeAttestation(state[], state[].latest_block_root, bc0[0], cache)
      attestation1 = makeAttestation(state[], state[].latest_block_root, bc0[1], cache)
  ...
  test "Fork choice returns latest block with no attestations":
    ...
    let head = pool[].selectOptimisticHead(b1Add[].slot.start_beacon_time).get().blck
    check:
      head == b1Add[]
    ...
    let head2 = pool[].selectOptimisticHead(b2Add[].slot.start_beacon_time).get().blck
  ...
  test "Fork choice returns block with attestation":
    ...
    let head = pool[].selectOptimisticHead(b10Add[].slot.start_beacon_time).get().blck
    ...
    let head2 = pool[].selectOptimisticHead(b10Add[].slot.start_beacon_time).get().blck
    ...
    let head3 = pool[].selectOptimisticHead(b10Add[].slot.start_beacon_time).get().blck
    ...
    let head4 = pool[].selectOptimisticHead(b11Add[].slot.start_beacon_time).get().blck
  ...
  test "Trying to add a duplicate block from an old pruned epoch is tagged as an error":
    ...
    for epoch in 0 ..< 5:
      ...
      for slot in start_slot ..< start_slot + SLOTS_PER_EPOCH:
        let new_block =
          addTestBlock(state[], cache, attestations = attestations).phase0Data
        ...
        let head =
          pool[].selectOptimisticHead(blockRef[].slot.start_beacon_time).get().blck
        doAssert:
          head == blockRef[]
