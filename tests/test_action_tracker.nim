...
suite "subnet tracker":
  test "should register stability subnets on attester duties":
    var tracker = ActionTracker.init(
      default(UInt256), false, EPOCHS_PER_SUBNET_SUBSCRIPTION, SUBNETS_PER_NODE)
  ...
  test "should use runtime stability subnet parameters":
    let
      ...
      tracker256 = ActionTracker.init(
        ...
        subnetsPerNode = 1)
      trackerZero = ActionTracker.init(
        default(UInt256), false,
        epochsPerSubnetSubscription = 64,
        subnetsPerNode = 0)
    ...
    let
      subnets64 =
        tracker64.stabilitySubnets(Epoch(64).start_slot())
      subnets256 =
        tracker256.stabilitySubnets(Epoch(64).start_slot())
      subnetsZero =
        trackerZero.stabilitySubnets(Epoch(64).start_slot())

    check:
      subnets64.countOnes() == 1
      subnets256.countOnes() == 1
      subnetsZero.countOnes() == 0
      subnets64[16]
      not subnets64[49]
      subnets256[49]
      not subnets256[16]
  ...
  test "should subscribe to all subnets when flag is enabled":
    let tracker = ActionTracker.init(
      default(UInt256),
      subscribeAllAttnets = true,
      epochsPerSubnetSubscription = EPOCHS_PER_SUBNET_SUBSCRIPTION,
      subnetsPerNode = SUBNETS_PER_NODE)
