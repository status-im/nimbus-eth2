...
import unittest2, ./testutil, ../beacon_chain/spec/[network, validator]
...
suite "Honest validator":
  ...
  test "General pubsub topics":
    check:
      ...
      getProposerSlashingsTopic(forkDigest) ==
        "/eth2/00000000/proposer_slashing/ssz_snappy"
      getAttesterSlashingsTopic(forkDigest) ==
        "/eth2/00000000/attester_slashing/ssz_snappy"
      getAggregateAndProofsTopic(forkDigest) ==
        "/eth2/00000000/beacon_aggregate_and_proof/ssz_snappy"
      getBlsToExecutionChangeTopic(forkDigest) ==
        "/eth2/00000000/bls_to_execution_change/ssz_snappy"
      getSyncCommitteeContributionAndProofTopic(forkDigest) ==
        "/eth2/00000000/sync_committee_contribution_and_proof/ssz_snappy"
  ...
  test "isNearSyncCommitteePeriod":
    ...
    for i in 1'u64 .. 20'u64:
      for j in 0'u64 .. SYNC_COMMITTEE_SUBNET_COUNT:
        check:
          nearSyncCommitteePeriod((EPOCHS_PER_SYNC_COMMITTEE_PERIOD * i - j).Epoch).get ==
            j

    # Smaller values EPOCHS_PER_SYNC_COMMITTEE_PERIOD would mean the wrap-around
    # causes false test failures
    static:
      doAssert EPOCHS_PER_SYNC_COMMITTEE_PERIOD >= 8
    for i in 1'u64 .. 20'u64:
      for j in (SYNC_COMMITTEE_SUBNET_COUNT + 1'u64) .. 7'u64:
        check:
          nearSyncCommitteePeriod((EPOCHS_PER_SYNC_COMMITTEE_PERIOD * i - j).Epoch).isNone
  ...
  test "Liveness failsafe conditions":
    ...
    for i in 0 .. MAX_MISSING_CONTIGUOUS + 1:
      check:
        not livenessFailsafeInEffect(x, i.Slot)
    ...
    for i in MAX_MISSING_CONTIGUOUS + 2 .. FAULT_INSPECTION_WINDOW + 10:
      check:
        livenessFailsafeInEffect(x, i.Slot)
    ...
    for i in FAULT_INSPECTION_WINDOW * 3 ..
        FAULT_INSPECTION_WINDOW * 3 + MAX_MISSING_CONTIGUOUS:
      check:
        not livenessFailsafeInEffect(x, i.Slot)
    for i in FAULT_INSPECTION_WINDOW * 3 + MAX_MISSING_CONTIGUOUS + 1 ..
    ...
    for i in FAULT_INSPECTION_WINDOW * 5 ..<
    ...
    for i in FAULT_INSPECTION_WINDOW * 5 ..
      ...
      check:
        not livenessFailsafeInEffect(x, i.Slot)
    for i in FAULT_INSPECTION_WINDOW * 5 + MAX_MISSING_CONTIGUOUS * 2 + 1 ..
    ...
    for i in countup(
      ...
      FAULT_INSPECTION_WINDOW * 9 + MAX_MISSING_CONTIGUOUS * 2,
      2,
    ):
      x[i].data[0] = i.uint8
      x[i + 1].data[0] = i.uint8 # missing block

    for i in FAULT_INSPECTION_WINDOW * 7 ..
      ...
      check:
        not livenessFailsafeInEffect(x, i.Slot)
    for i in FAULT_INSPECTION_WINDOW * 9 + MAX_MISSING_WINDOW * 2 ..
    ...
    for i in SLOTS_PER_HISTORICAL_ROOT ..
      ...
      for index in 0'u64 ..< index_count:
        check:
          compute_shuffled_index(
            compute_inverted_shuffled_index(index, index_count, seed), index_count, seed
          ) == index
          compute_inverted_shuffled_index(
            compute_shuffled_index(index, index_count, seed), index_count, seed
          ) == index
