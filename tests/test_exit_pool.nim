...
import
  ../beacon_chain/spec/[datatypes/base, forks, presets, signatures, state_transition],
  ../beacon_chain/consensus_object_pools/[block_quarantine, blockchain_dag, exit_pool],
...
suite "Validator change pool testing suite":
  setup:
    let
      ...
      dag =
        init(ChainDAGRef, cfg, makeTestDB(SLOTS_PER_EPOCH * 3), validatorMonitor, {})
  ...
  test "addValidatorChangeMessage/getAttesterSlashingMessage":
    for i in 0'u64 .. MAX_ATTESTER_SLASHINGS + 5:
      ...
      withState(dag.headState):
        check:
          pool[].getBeaconBlockValidatorChanges(cfg, forkyState.data).attester_slashings.lenu64 ==
