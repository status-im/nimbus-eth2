...
proc makeDeposit*(
    i: int, flags: UpdateFlags = {}, cfg = defaultRuntimeConfig
): DepositData =
  let
...
proc makeInitialDeposits*(
    n = SLOTS_PER_EPOCH, flags: UpdateFlags = {}, cfg = defaultRuntimeConfig
): seq[DepositData] =
  for i in 0 ..< n.int:
...
proc build_empty_merge_execution_payload(
  ...
  payload.block_hash = rlpHash blockToBlockHeader(
    bellatrix.BeaconBlock(body: bellatrix.BeaconBlockBody(execution_payload: payload))
  )

  bellatrix.ExecutionPayloadForSigning(executionPayload: payload, blockValue: Wei.zero)

from stew/saturating_arith import saturate

proc build_empty_execution_payload(
    state: bellatrix.BeaconState, feeRecipient: Eth1Address
): bellatrix.ExecutionPayloadForSigning =
  ## Assuming a pre-state of the same slot, build a valid ExecutionPayload
  ...
  let
    ...
    timestamp = compute_timestamp_at_slot(state, state.slot)
    randao_mix = get_randao_mix(state, get_current_epoch(state))
    base_fee = calcEip1599BaseFee(
      GasInt.saturate latest.gas_limit,
  ...
  payload.executionPayload.block_hash = bellatrix.BeaconBlock(
    body: bellatrix.BeaconBlockBody(execution_payload: payload.executionPayload)
...
proc addTestBlock*(
  ...
  let
    proposer_index = get_beacon_proposer_index(state, cache, getStateField(state, slot))
    ...
    randao_reveal =
      if skipBlsValidation notin flags:
        get_epoch_signature(
          ...
          getStateField(state, slot).epoch,
          privKey,
        )
        .toValidatorSig()
      else:
    ...
    let execution_payload =
      when consensusFork > ConsensusFork.Bellatrix:
        ...
        if cfg.CAPELLA_FORK_EPOCH != FAR_FUTURE_EPOCH:
          ...
          if forkyState.data.slot > cfg.BELLATRIX_FORK_EPOCH * SLOTS_PER_EPOCH + 10:
...
func makeAttestation(
  ...
  Attestation(data: data, aggregation_bits: aggregation_bits, signature: sig)
