    ...
    const blockBodies = [
      (electra.BeaconBlockBody)(
        ...
        execution_requests: electra.ExecutionRequests(
          deposits: List[DepositRequest, Limit MAX_DEPOSIT_REQUESTS_PER_PAYLOAD].init(@[
            DepositRequest(
          ...
          withdrawals: List[WithdrawalRequest, Limit MAX_WITHDRAWAL_REQUESTS_PER_PAYLOAD].init(@[
          ]),
          consolidations: List[ConsolidationRequest, Limit MAX_CONSOLIDATION_REQUESTS_PER_PAYLOAD].init(@[
          ])),
      ),
      (electra.BeaconBlockBody)(
        execution_payload: electra.ExecutionPayload(
