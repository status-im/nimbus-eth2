...
suite "ColumnMap test suite":
  test "fill test":
    ...
    for columnSize in 4 .. 128:
      ...
      for i in 0 ..< columnsCount:
        ...
        check:
          $ColumnMap.init(columns) == "[" & $numbers.mapIt($it).join(", ") & "]"
  ...
  test "and() operation test":
    const TestVectors = [
      ...
      ([1, 5, 10, 15, 20, 25, 64, 65], [1, 5, 6, 7, 8, 9, 64, 65], "[1, 5, 64, 65]"),
      (
    ...
    for vector in TestVectors:
      ...
      check:
        ...
        "[" & map3.items().toSeq().mapIt($int(it)).join(", ") & "]" == vector[2]
  ...
  test "supernode test":
    for max in ((NUMBER_OF_COLUMNS div 2) + 1) ..< NUMBER_OF_COLUMNS:
      ...
      check:
        map.items().toSeq().mapIt($int(it)).join(", ") == columns.mapIt($it).join(", ")
