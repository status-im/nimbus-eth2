...
const SSZDir = SszTestsDir / const_preset / "deneb" / "ssz_static"
...
proc checkSSZ(
    T: type deneb.SignedBeaconBlock, dir: string, expectedHash: SSZHashTreeRoot
) =
      ...
      for pathKind, sszTestKind in walkDir(path, relative = true, checkDir = true):
        ...
        for pathKind, sszTestCase in walkDir(path, relative = true, checkDir = true):
          ...
          of "AggregateAndProof":
            checkSSZ(AggregateAndProof, path, hash)
          of "Attestation":
            checkSSZ(Attestation, path, hash)
          of "AttestationData":
            checkSSZ(AttestationData, path, hash)
          of "AttesterSlashing":
            checkSSZ(AttesterSlashing, path, hash)
          of "BeaconBlock":
            checkSSZ(deneb.BeaconBlock, path, hash)
          of "BeaconBlockBody":
            checkSSZ(deneb.BeaconBlockBody, path, hash)
          of "BeaconBlockHeader":
            checkSSZ(BeaconBlockHeader, path, hash)
          of "BeaconState":
            checkSSZ(deneb.BeaconState, path, hash)
          of "BlobIdentifier":
          ...
          of "ExecutionPayload":
            checkSSZ(ExecutionPayload, path, hash)
          ...
          of "IndexedAttestation":
            checkSSZ(IndexedAttestation, path, hash)
