...
suite "EF - Phase 0 - Sanity - Slots " & preset():
  ...
  for kind, path in walkDir(phase0SanitySlotsDir, relative = true, checkDir = true):
...
suite "EF - Altair - Sanity - Slots " & preset():
  ...
  for kind, path in walkDir(altairSanitySlotsDir, relative = true, checkDir = true):
...
suite "EF - Bellatrix - Sanity - Slots " & preset():
  ...
  for kind, path in walkDir(bellatrixSanitySlotsDir, relative = true, checkDir = true):
    runTest(
      bellatrix.BeaconState, bellatrixSanitySlotsDir, "Bellatrix", suiteName, path
    )
...
suite "EF - Capella - Sanity - Slots " & preset():
  ...
  for kind, path in walkDir(capellaSanitySlotsDir, relative = true, checkDir = true):
...
suite "EF - Deneb - Sanity - Slots " & preset():
  ...
  for kind, path in walkDir(denebSanitySlotsDir, relative = true, checkDir = true):
