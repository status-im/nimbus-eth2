...
from ./fixtures_utils import
  SSZ, SszTestsDir, hash_tree_root, loadBlock, parseTest, readSszBytes, toSszType
...
proc runTest(
  ...
  test prefix & testName & " - " & unitTestName & preset():
    let preState =
      newClone(parseTest(testPath / "pre.ssz_snappy", SSZ, consensusFork.BeaconState))
    var
    ...
    for i in 0 ..< numBlocks:
      ...
      if hasPostState:
        ...
        discard state_transition(
          ...
          .expect("should apply block")
      else:
        let res = state_transition(
          defaultRuntimeConfig, fhPreState[], blck, cache, info, flags = {}, noRollback
        )
    ...
    if hasPostState:
      let postState = newClone(
        parseTest(testPath / "post.ssz_snappy", SSZ, consensusFork.BeaconState)
      )
      when false:
