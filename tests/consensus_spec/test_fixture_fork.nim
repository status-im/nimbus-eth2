...
proc runTest(
    BeaconStateAnte, BeaconStatePost: type,
    forkName, forkDir: static[string],
  ...
  test "EF - " & forkName & " - Fork - " & unitTestName & preset():
    let
      preState = newClone(parseTest(testDir / "pre.ssz_snappy", SSZ, BeaconStateAnte))
      postState = newClone(parseTest(testDir / "post.ssz_snappy", SSZ, BeaconStatePost))
    ...
    check:
      upgradedState[].hash_tree_root() == postState[].hash_tree_root()
...
suite "EF - Altair - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(
      phase0.BeaconState, altair.BeaconState, "Altair", OpForkDir, suiteName, path
    )
...
suite "EF - Bellatrix - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(
      altair.BeaconState, bellatrix.BeaconState, "Bellatrix", OpForkDir, suiteName, path
    )
...
suite "EF - Capella - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(
      bellatrix.BeaconState, capella.BeaconState, "Capella", OpForkDir, suiteName, path
    )
...
suite "EF - Deneb - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(capella.BeaconState, deneb.BeaconState, "Deneb", OpForkDir, suiteName, path)
...
suite "EF - Electra - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(
      deneb.BeaconState, electra.BeaconState, "Electra", OpForkDir, suiteName, path
    )
...
suite "EF - Fulu - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(electra.BeaconState, fulu.BeaconState, "Fulu", OpForkDir, suiteName, path)
...
suite "EF - Gloas - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(fulu.BeaconState, gloas.BeaconState, "Gloas", OpForkDir, suiteName, path)
