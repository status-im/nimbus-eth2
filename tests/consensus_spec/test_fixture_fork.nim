...
suite "EF - Altair - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(
      phase0.BeaconState, altair.BeaconState, "Altair", OpForkDir, upgrade_to_altair,
      suiteName, path,
    )
...
suite "EF - Bellatrix - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(
      altair.BeaconState, bellatrix.BeaconState, "Bellatrix", OpForkDir,
...
suite "EF - Capella - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(
      bellatrix.BeaconState, capella.BeaconState, "Capella", OpForkDir,
...
suite "EF - Deneb - Fork " & preset():
  ...
  for kind, path in walkDir(OpForkDir, relative = true, checkDir = true):
    runTest(
      capella.BeaconState, deneb.BeaconState, "Deneb", OpForkDir, upgrade_to_deneb,
      suiteName, path,
    )
