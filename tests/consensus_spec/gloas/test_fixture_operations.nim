...
const testDirs = toHashSet(
  [
    ...
    OpConsolidationRequestDir, OpDepositRequestDir, OpDepositsDir,
    OpWithdrawalRequestDir, OpExecutionPayloadDir, OpExecutionPayloadBidDir,
    OpPayloadAttestationDir, OpProposerSlashingDir, OpSyncAggregateDir,
    OpVoluntaryExitDir, OpWithdrawalsDir,
  ]
)

doAssert toHashSet(mapIt(toSeq(walkDir(OpDir, relative = false)), it.path)) == testDirs
...
proc runTest[T, U](
  ...
  test prefix & baseDescription & opName & " - " & identifier:
    let preState =
      newClone(parseTest(testDir / "pre.ssz_snappy", SSZ, gloas.BeaconState))
    let done =
    ...
    if fileExists(testDir / "post.ssz_snappy"):
      let
        postState =
          newClone(parseTest(testDir / "post.ssz_snappy", SSZ, gloas.BeaconState))
...
suite baseDescription & "Attester Slashing " & preset():
  proc applyAttesterSlashing(
      preState: var gloas.BeaconState, attesterSlashing: electra.AttesterSlashing
  ): Result[void, cstring] =
    var cache: StateCache
    doAssert (
...
suite baseDescription & "BLS to execution change " & preset():
  proc applyBlsToExecutionChange(
      preState: var gloas.BeaconState, signed_address_change: SignedBLSToExecutionChange
  ): Result[void, cstring] =
...
suite baseDescription & "Consolidation Request " & preset():
  proc applyConsolidationRequest(
      preState: var gloas.BeaconState, consolidation_request: ConsolidationRequest
  ): Result[void, cstring] =
    var cache: StateCache
    process_consolidation_request(
      defaultRuntimeConfig, preState, sortValidatorBuckets(preState.validators.asSeq)[],
...
suite baseDescription & "Deposit Request " & preset():
  func applyDepositRequest(
    ...
    process_deposit_request(defaultRuntimeConfig, preState, depositRequest, {})
...
suite baseDescription & "Execution Payload " & preset():
  proc makeApplyExecutionPayloadCb(path: string): auto =
    return proc(
        preState: var gloas.BeaconState, signed_envelope: SignedExecutionPayloadEnvelope
    ): Result[void, cstring] =
      ...
      var cache: StateCache
      let hashedState =
        (ref gloas.HashedBeaconState)(data: preState, root: hash_tree_root(preState))

      func executePayload(_: deneb.ExecutionPayload): bool =
        payloadValid
      let res = process_execution_payload(
        defaultRuntimeConfig, hashedState[], signed_envelope, executePayload, cache
      )
...
suite baseDescription & "Execution Payload Bid " & preset():
  proc applyExecutionPayloadBid(
    ...
    process_execution_payload_bid(defaultRuntimeConfig, preState, blck)

  for path in walkTests(OpExecutionPayloadBidDir):
    runTest[gloas.BeaconBlock, typeof applyExecutionPayloadBid](
      OpExecutionPayloadBidDir, suiteName, "Execution Payload Bid", "block",
      applyExecutionPayloadBid, path,
    )

suite baseDescription & "Payload Attestation " & preset():
  proc applyPayloadAttestation(
      preState: var gloas.BeaconState, payloadAttestation: PayloadAttestation
  ): Result[void, cstring] =
    var cache: StateCache
  ...
  for path in walkTests(OpPayloadAttestationDir):
    runTest[PayloadAttestation, typeof applyPayloadAttestation](
      OpPayloadAttestationDir, suiteName, "Payload Attestation", "payload_attestation",
      applyPayloadAttestation, path,
    )
...
suite baseDescription & "Withdrawal Request " & preset():
  ...
  for path in walkTests(OpWithdrawalRequestDir):
    runTest[WithdrawalRequest, typeof applyWithdrawalRequest](
      OpWithdrawalRequestDir, suiteName, "Withdrawal Request", "withdrawal_request",
      applyWithdrawalRequest, path,
    )
...
suite baseDescription & "Withdrawals " & preset():
  func applyWithdrawals(
      preState: var gloas.BeaconState, executionPayload: deneb.ExecutionPayload
  ): Result[void, cstring] =
