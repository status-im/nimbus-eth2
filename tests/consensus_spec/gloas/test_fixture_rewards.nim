...
import
  ...
  ../../../beacon_chain/spec/datatypes/[altair, gloas],
  # Test utilities
  ../../testutil,
  ../fixtures_utils,
  ../os_ops
...
const
  RewardsDirBase = SszTestsDir / const_preset / "gloas" / "rewards"
...
proc runTest(rewardsDir, identifier: string) =
  ...
  let
    state = newClone(parseTest(testDir / "pre.ssz_snappy", SSZ, gloas.BeaconState))
    flagDeltas = [
  ...
  for validator_index, reward0, reward1, reward2, penalty0, penalty1, penalty2 in get_flag_and_inactivity_deltas(
    ...
    flagDeltas2[TimelyFlag.TIMELY_TARGET_FLAG_INDEX].penalties[validator_index] =
      penalty1
    flagDeltas2[TimelyFlag.TIMELY_HEAD_FLAG_INDEX].penalties[validator_index] = 0.Gwei
...
suite "EF - Gloas - Rewards " & preset():
  for rewardsDir in [RewardsDirBasic, RewardsDirLeak, RewardsDirRandom]:
    for kind, path in walkDir(rewardsDir, relative = true, checkDir = true):
      test "EF - Gloas - Rewards - " & path & preset():
