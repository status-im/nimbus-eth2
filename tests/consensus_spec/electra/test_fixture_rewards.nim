...
const
  RewardsDirBase = SszTestsDir / const_preset / "electra" / "rewards"
...
proc runTest(rewardsDir, identifier: string) =
  ...
  let
    state = newClone(parseTest(testDir / "pre.ssz_snappy", SSZ, electra.BeaconState))
    flagDeltas = [
...
suite "EF - Electra - Rewards " & preset():
  for rewardsDir in [RewardsDirBasic, RewardsDirLeak, RewardsDirRandom]:
    for kind, path in walkDir(rewardsDir, relative = true, checkDir = true):
      test "EF - Electra - Rewards - " & path & preset():
