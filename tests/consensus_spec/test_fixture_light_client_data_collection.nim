...
proc loadForked[T: not Opt](
    t: typedesc[T], s: JsonNode, path: string, fork_digests: ForkDigests
): T {.raises: [ValueError].} =
  if s == nil:
  ...
  let
    fork_digest = ForkDigest distinctBase(ForkDigest).fromHex(s["fork_digest"].getStr())
    consensusFork = fork_digests.consensusForkForDigest(fork_digest).expect(
        "Unknown fork " & $fork_digest
      )
    filename = s["data"].getStr()
  when T is SomeForkedLightClientObject:
    withLcDataFork(lcDataForkAtConsensusFork(consensusFork)):
      when lcDataFork > LightClientDataFork.None:
        T.init(parseTest(path / filename & ".ssz_snappy", SSZ, T.Forky(lcDataFork)))
      else:
        raiseAssert $consensusFork & " does not support LC data"
  else:
    withConsensusFork(consensusFork):
      T.init(parseTest(path / filename & ".ssz_snappy", SSZ, T.Forky(consensusFork)))
...
proc loadSteps(
  ...
  for step in steps[0]:
    if step.hasKey"new_block":
      ...
      result.add TestStep(
        kind: TestStepKind.NewBlock, blck: ForkedSignedBeaconBlock.loadForked(s)
      )
      ...
      result.add TestStep(
        ...
        checks: NewHeadChecks(
          latestFinalizedCheckpoint: Checkpoint(
            epoch: Epoch(checks["latest_finalized_checkpoint"]["epoch"].getInt()),
            root:
              Eth2Digest.fromHex(checks["latest_finalized_checkpoint"]["root"].getStr()),
          ),
          bootstraps: checks["bootstraps"].foldl(
            (
              block:
                check:
                  not a.hasKeyOrPut(
            ...
            newTable[Eth2Digest, ForkedLightClientBootstrap](),
          )[],
          bestUpdates: checks["best_updates"].foldl(
            (
              block:
                check:
                  not a.hasKeyOrPut(
            ...
            newTable[SyncCommitteePeriod, ForkedLightClientUpdate](),
          )[],
          latestFinalityUpdate:
            ForkedLightClientFinalityUpdate.loadForked(checks{"latest_finality_update"}),
          latestOptimisticUpdate: ForkedLightClientOptimisticUpdate.loadForked(
            checks{"latest_optimistic_update"}
          ),
        ),
      )
    else:
...
proc runTest(suiteName, path: string, consensusFork: static ConsensusFork) =
  ...
  test "Light client - Data collection - " & relativePathComponent:
    let
      ...
      initial_state = loadForkedState(path / "initial_state.ssz_snappy", consensusFork)
    ...
    for i, step in steps:
      ...
      of TestStepKind.NewHead:
        ...
        check:
          step.checks.latestFinalizedCheckpoint.epoch == dag.finalizedHead.slot.epoch
          ...
          step.checks.latestFinalityUpdate == dag.getLightClientFinalityUpdate()
          step.checks.latestOptimisticUpdate == dag.getLightClientOptimisticUpdate()
