...
import yaml, ../../beacon_chain/spec/[state_transition, forks], ./os_ops
  ...
  test testName & " - " & unitTestName & preset():
    ...
    for i in 0 ..< numBlocks:
      if i <= fork_block:
        let
          blck =
            parseTest(testPath / "blocks_" & $i & ".ssz_snappy", SSZ, AnteBeaconBlock)
        ...
        let
          blck =
            parseTest(testPath / "blocks_" & $i & ".ssz_snappy", SSZ, PostBeaconBlock)
          res = state_transition(
            cfg,
            fhPreState[],
            blck,
