...
import chronicles, yaml, ../../beacon_chain/spec/[state_transition, forks], ./os_ops
...
proc getTransitionInfo(
    testPath: string
): TransitionInfo {.raises: [IOError, OSError, YamlConstructionError, YamlParserError].} =
...
proc runTest(
    ...
    cfg: RuntimeConfig,
    testName, testDir: static[string],
  ...
  test testName & " - " & unitTestName & preset():
    ...
    for i in 0 ..< numBlocks:
      if i <= fork_block:
        let
          blck =
            loadBlock(testPath / "blocks_" & $i & ".ssz_snappy", AnteBeaconBlock.kind)
        ...
        let
          blck =
            loadBlock(testPath / "blocks_" & $i & ".ssz_snappy", PostBeaconBlock.kind)
          res = state_transition(
            cfg,
            fhPreState[],
            blck,
    ...
    let postState =
      newClone(parseTest(testPath / "post.ssz_snappy", SSZ, PostBeaconState))
    when false:
      reportDiff(fhPreState.data, postState)
    doAssert getStateRoot(fhPreState[]) == postState[].hash_tree_root()

from ../../beacon_chain/spec/datatypes/phase0 import BeaconState, SignedBeaconBlock
from ../../beacon_chain/spec/datatypes/altair import BeaconState, SignedBeaconBlock
...
from ../../beacon_chain/spec/datatypes/bellatrix import BeaconState, SignedBeaconBlock
...
suite "EF - Bellatrix - Transition " & preset():
  ...
  for kind, path in walkDir(TransitionDir, relative = true, checkDir = true):
    ...
    runTest(
      ...
      bellatrix.SignedBeaconBlock, cfg, "EF - Bellatrix - Transition", TransitionDir,
      suiteName, path, transitionInfo.fork_block,
    )

from ../../beacon_chain/spec/datatypes/capella import BeaconState, SignedBeaconBlock
...
suite "EF - Capella - Transition " & preset():
  ...
  for kind, path in walkDir(TransitionDir, relative = true, checkDir = true):
    ...
    runTest(
      ...
      capella.SignedBeaconBlock, cfg, "EF - Capella - Transition", TransitionDir,
      suiteName, path, transitionInfo.fork_block,
    )

from ../../beacon_chain/spec/datatypes/deneb import BeaconState, SignedBeaconBlock
...
suite "EF - Deneb - Transition " & preset():
  ...
  for kind, path in walkDir(TransitionDir, relative = true, checkDir = true):
    ...
    runTest(
      ...
      deneb.SignedBeaconBlock, cfg, "EF - Deneb - Transition", TransitionDir, suiteName,
      path, transitionInfo.fork_block,
    )

from ../../beacon_chain/spec/datatypes/electra import BeaconState, SignedBeaconBlock
...
suite "EF - Electra - Transition " & preset():
  ...
  for kind, path in walkDir(TransitionDir, relative = true, checkDir = true):
    ...
    runTest(
      ...
      electra.SignedBeaconBlock, cfg, "EF - Electra - Transition", TransitionDir,
      suiteName, path, transitionInfo.fork_block,
    )

from ../../beacon_chain/spec/datatypes/fulu import BeaconState, SignedBeaconBlock
...
suite "EF - Fulu - Transition " & preset():
  ...
  for kind, path in walkDir(TransitionDir, relative = true, checkDir = true):
    ...
    runTest(
      ...
      fulu.SignedBeaconBlock, cfg, "EF - Fulu - Transition", TransitionDir, suiteName,
      path, transitionInfo.fork_block,
    )

from ../../beacon_chain/spec/datatypes/gloas import BeaconState, SignedBeaconBlock
...
suite "EF - Gloas - Transition " & preset():
  ...
  for kind, path in walkDir(TransitionDir, relative = true, checkDir = true):
    ...
    runTest(
      ...
      gloas.SignedBeaconBlock, cfg, "EF - Gloas - Transition", TransitionDir, suiteName,
      path, transitionInfo.fork_block,
    )
