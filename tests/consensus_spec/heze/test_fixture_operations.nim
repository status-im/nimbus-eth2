...
import
  ...
  ../../../beacon_chain/spec/datatypes/heze,
  # Test utilities
  ../../testutil,
...
const
  OpDir                       = SszTestsDir/const_preset/"heze"/"operations"
    ...
    if fileExists(testDir/"post.ssz_snappy"):
      let
        postState = newClone(parseTest(
          testDir/"post.ssz_snappy", SSZ, heze.BeaconState))
...
suite baseDescription & "Block Header " & preset():
  proc applyBlockHeader(
      preState: var heze.BeaconState, blck: heze.BeaconBlock):
      Result[void, cstring] =
    var cache: StateCache
    process_block_header(preState, blck, {}, cache)

  for path in walkTests(OpBlockHeaderDir):
    runTest[heze.BeaconBlock, typeof applyBlockHeader](
...
suite baseDescription & "Withdrawal Request " & preset():
  func applyWithdrawalRequest(
      preState: var heze.BeaconState, withdrawalRequest: WithdrawalRequest):
      Result[void, cstring] =
    var cache: StateCache
...
suite baseDescription & "Proposer Slashing " & preset():
  proc applyProposerSlashing(
      preState: var heze.BeaconState, proposerSlashing: ProposerSlashing):
      Result[void, cstring] =
    var cache: StateCache
...
suite baseDescription & "Sync Aggregate " & preset():
  proc applySyncAggregate(
      preState: var heze.BeaconState, syncAggregate: SyncAggregate):
      Result[void, cstring] =
    var cache: StateCache
...
suite baseDescription & "Voluntary Exit " & preset():
  proc applyVoluntaryExit(
      preState: var heze.BeaconState, voluntaryExit: SignedVoluntaryExit):
      Result[void, cstring] =
    var cache: StateCache
