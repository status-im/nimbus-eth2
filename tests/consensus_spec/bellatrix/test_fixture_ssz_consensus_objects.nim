...
const SSZDir = SszTestsDir / const_preset / "bellatrix" / "ssz_static"
...
proc checkSSZ(
    T: type bellatrix.SignedBeaconBlock, dir: string, expectedHash: SSZHashTreeRoot
) =
    ...
    test &"  Testing    {sszType}":
      ...
      for pathKind, sszTestKind in walkDir(path, relative = true, checkDir = true):
        ...
        for pathKind, sszTestCase in walkDir(path, relative = true, checkDir = true):
          ...
          of "BeaconBlock":
            checkSSZ(bellatrix.BeaconBlock, path, hash)
          of "BeaconBlockBody":
            checkSSZ(bellatrix.BeaconBlockBody, path, hash)
          of "BeaconBlockHeader":
            checkSSZ(BeaconBlockHeader, path, hash)
          of "BeaconState":
            checkSSZ(bellatrix.BeaconState, path, hash)
          of "Checkpoint":
