  ...
  for validator_index in participants:
    let
      ...
      signature = get_sync_committee_message_signature(
        state.fork, state.genesis_validators_root, signature_slot, block_root, privkey
      )
    if not initialized:
      initialized = true
...
proc get_sync_aggregate(
    ...
    signature_slot = FAR_FUTURE_SLOT,
): (SyncAggregate, Slot) =
  template state(): auto =
    forked.altairData.data
  ...
  template signature_state(): auto =
    signature_forked.altairData.data

  process_slots(cfg, signature_forked[], sig_slot, cache, info, flags = {}).expect(
    "no failure"
  )
  ...
  let
    ...
    committee_indices = signature_state.current_sync_committee.pubkeys.mapIt(
      all_pubkeys.find(it).ValidatorIndex
    )
...
proc block_for_next_slot(
  ...
  let attestations =
    if withAttestations:
      let block_root = withState(forked):
        forkyState.latest_block_root
      makeFullAttestations(forked, block_root, state.slot, cache)
    else:
      @[]

  addTestBlock(forked, cache, attestations = attestations, cfg = cfg)
...
func initialize_light_client_store(
    state: auto, storeDataFork: static LightClientDataFork
): auto =
...
proc runTest(storeDataFork: static LightClientDataFork) =
  suite "EF - " & $storeDataFork & " - Unittests - Light client - Sync protocol" &
    preset():
    let
      cfg = block:
    ...
    test "test_process_light_client_update_not_timeout":
      ...
      let
        ...
        next_sync_committee_branch = default(storeDataFork.NextSyncCommitteeBranch)
    ...
    test "process_light_client_update_timeout":
      ...
      var
        cache: StateCache
        info: ForkedEpochInfo
      process_slots(cfg, forked[], Slot(UPDATE_TIMEOUT), cache, info, flags = {}).expect(
        "no failure"
      )
      let
        store_period = sync_committee_period(store.optimistic_header.beacon.slot)
      ...
      template next_sync_committee(): auto =
    ...
    test "process_light_client_update_finality_updated":
      ...
      process_slots(cfg, forked[], Slot(SLOTS_PER_EPOCH * 2), cache, info, flags = {})
        .expect("no failure")
      for slot in 0 ..< 3 * SLOTS_PER_EPOCH:
        blocks.add block_for_next_slot(cfg, forked[], cache, withAttestations = true)
      ...
      check:
        finalized_header.beacon.slot == start_slot(state.finalized_checkpoint.epoch)
        finalized_header.beacon.hash_tree_root() == state.finalized_checkpoint.root
      let
