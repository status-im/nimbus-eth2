...
const SSZDir = SszTestsDir / const_preset / "phase0" / "ssz_static"
...
proc checkSSZ(
    T: type phase0.SignedBeaconBlock, dir: string, expectedHash: SSZHashTreeRoot
) =
    ...
    test &"  Testing    {sszType}":
      ...
      for pathKind, sszTestKind in walkDir(path, relative = true, checkDir = true):
        ...
        for pathKind, sszTestCase in walkDir(path, relative = true, checkDir = true):
          ...
          of "BeaconBlock":
            checkSSZ(phase0.BeaconBlock, path, hash)
          of "BeaconBlockBody":
            checkSSZ(phase0.BeaconBlockBody, path, hash)
          of "BeaconBlockHeader":
            checkSSZ(BeaconBlockHeader, path, hash)
          of "BeaconState":
            checkSSZ(phase0.BeaconState, path, hash)
          of "Checkpoint":
          ...
          of "SignedBeaconBlock":
            checkSSZ(phase0.SignedBeaconBlock, path, hash)
