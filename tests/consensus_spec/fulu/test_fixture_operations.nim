...
proc runTest[T, U](
  ...
  test prefix & baseDescription & opName & " - " & identifier:
    let preState =
      newClone(parseTest(testDir / "pre.ssz_snappy", SSZ, fulu.BeaconState))
    let done =
      applyProc(preState[], parseTest(testDir / (applyFile & ".ssz_snappy"), SSZ, T))

    if fileExists(testDir / "post.ssz_snappy"):
      let postState =
        newClone(parseTest(testDir / "post.ssz_snappy", SSZ, fulu.BeaconState))
...
suite baseDescription & "Attester Slashing " & preset():
  proc applyAttesterSlashing(
      preState: var fulu.BeaconState, attesterSlashing: electra.AttesterSlashing
  ): Result[void, cstring] =
    var cache: StateCache
    doAssert (
...
suite baseDescription & "BLS to execution change " & preset():
  proc applyBlsToExecutionChange(
      preState: var fulu.BeaconState, signed_address_change: SignedBLSToExecutionChange
  ): Result[void, cstring] =
...
suite baseDescription & "Consolidation Request " & preset():
  proc applyConsolidationRequest(
      preState: var fulu.BeaconState, consolidation_request: ConsolidationRequest
  ): Result[void, cstring] =
    var cache: StateCache
  ...
  func makeApplyExecutionPayloadCb(path: string): auto =
    return proc(
      ...
      func executePayload(_: deneb.ExecutionPayload): bool =
        payloadValid
      process_execution_payload(defaultRuntimeConfig, preState, body, executePayload)
...
suite baseDescription & "Withdrawals " & preset():
  func applyWithdrawals(
      preState: var fulu.BeaconState, executionPayload: deneb.ExecutionPayload
  ): Result[void, cstring] =
