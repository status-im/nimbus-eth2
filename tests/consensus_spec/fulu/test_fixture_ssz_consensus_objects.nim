...
import
  ...
  ../../../beacon_chain/spec/datatypes/[altair, electra, fulu],
  # Status libraries
  snappy,
  # Test utilities
  ../../testutil,
...
const SSZDir = SszTestsDir / const_preset / "fulu" / "ssz_static"
    ...
    test &"  Testing    {sszType}":
      ...
      for pathKind, sszTestKind in walkDir(path, relative = true, checkDir = true):
        ...
        for pathKind, sszTestCase in walkDir(path, relative = true, checkDir = true):
          ...
          of "BeaconState":
            checkSSZ(fulu.BeaconState, path, hash)
          of "BlobIdentifier":
          ...
          of "DataColumnSidecar":
            checkSSZ(fulu.DataColumnSidecar, path, hash)
