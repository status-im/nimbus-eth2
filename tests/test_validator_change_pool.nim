...
func makePhase0IndexedAttestation(
  ...
  phase0.IndexedAttestation(
    ...
    signature: get_attestation_signature(
      fork, genesis_validators_root, tmp, MockPrivKeys[validator_index]
    ).toValidatorSig,
  )
...
func makeElectraIndexedAttestation(
  ...
  electra.IndexedAttestation(
    ...
    attesting_indices: List[
      uint64, Limit MAX_VALIDATORS_PER_COMMITTEE * MAX_COMMITTEES_PER_SLOT
    ](@[validator_index]),
    signature: get_attestation_signature(
      fork, genesis_validators_root, tmp, MockPrivKeys[validator_index]
    ).toValidatorSig,
...
suite "Validator change pool testing suite":
  ...
  test "addValidatorChangeMessage/getProposerSlashingMessage":
    for i in 0'u64 .. MAX_PROPOSER_SLASHINGS + 5:
      for j in 0'u64 .. i:
        let msg = ProposerSlashing(
          signed_header_1: makeSignedBeaconBlockHeader(
            fork, genesis_validators_root, Slot(1), j, makeFakeHash(0)
          ),
          signed_header_2: makeSignedBeaconBlockHeader(
      ...
      withState(dag.headState):
        check:
          pool[].getBeaconBlockValidatorChanges(cfg, forkyState.data).proposer_slashings.lenu64 ==
  ...
  test "addValidatorChangeMessage/getAttesterSlashingMessage (Phase 0)":
    for i in 0'u64 .. MAX_ATTESTER_SLASHINGS + 5:
      for j in 0'u64 .. i:
        let msg = phase0.AttesterSlashing(
      ...
      withState(dag.headState):
        check:
          pool[].getBeaconBlockValidatorChanges(cfg, forkyState.data).phase0_attester_slashings.lenu64 ==
  ...
  test "addValidatorChangeMessage/getAttesterSlashingMessage (Electra)":
    ...
    for i in 0'u64 .. MAX_ATTESTER_SLASHINGS_ELECTRA + 5:
      for j in 0'u64 .. i:
        let msg = electra.AttesterSlashing(
      ...
      withState(dag.headState):
        check:
          pool[].getBeaconBlockValidatorChanges(cfg, forkyState.data).electra_attester_slashings.lenu64 ==
  ...
  test "addValidatorChangeMessage/getVoluntaryExitMessage":
    ...
    process_slots(
      dag.cfg,
      dag.headState,
      Epoch(dag.cfg.SHARD_COMMITTEE_PERIOD).start_slot + 1,
      cache,
      info,
      {},
    )
    ...
    for i in 0'u64 .. MAX_VOLUNTARY_EXITS + 5:
      ...
      withState(dag.headState):
        check:
          pool[].getBeaconBlockValidatorChanges(cfg, forkyState.data).voluntary_exits.lenu64 ==
    ...
    for i in 0'u64 .. MAX_BLS_TO_EXECUTION_CHANGES + 5:
      for j in 0'u64 .. i:
        var msg = SignedBLSToExecutionChange(
          message:
            BLSToExecutionChange(validator_index: j, from_bls_pubkey: MockPubKeys[j])
        )
        msg.signature = toValidatorSig(
          get_bls_to_execution_change_signature(
            dag.cfg.GENESIS_FORK_VERSION,
      ...
      withState(dag.headState):
        ...
        check pool[].getBeaconBlockValidatorChanges(cfg, forkyState.data).bls_to_execution_changes.len ==
          0
  ...
  test "pre-pre-fork voluntary exit":
    ...
    process_slots(
      dag.cfg,
      dag.headState,
      (Epoch(dag.cfg.SHARD_COMMITTEE_PERIOD) + 1).start_slot + 1,
      cache,
      info,
      {},
    )
