...
proc nfuzz_attester_slashing(
  ...
  decodeAndProcess(AttesterSlashingInput):
    process_attester_slashing(
      getRuntimeConfig(some "mainnet"), data.state, data.attesterSlashing, flags, cache
    ).isOk
...
proc nfuzz_proposer_slashing(
  ...
  decodeAndProcess(ProposerSlashingInput):
    process_proposer_slashing(
      getRuntimeConfig(some "mainnet"), data.state, data.proposerSlashing, flags, cache
    ).isOk
...
proc nfuzz_voluntary_exit(
  ...
  decodeAndProcess(VoluntaryExitInput):
    process_voluntary_exit(
      getRuntimeConfig(some "mainnet"), data.state, data.exit, flags, cache
    ).isOk
