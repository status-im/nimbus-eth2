...
QtObject:
  ...
  proc setup(self: VoluntaryExitList) =
  ...
  proc newVoluntaryExitList*(data: openArray[SignedVoluntaryExit]): VoluntaryExitList =
    new(result, delete)
    result.data = ObjectTableModelImpl[VoluntaryExitInfo](
      items: data.mapIt(it.toVoluntaryExitInfo())
    )
    result.setup
  ...
  method headerData*(
      self: VoluntaryExitList, section: int, orientation: QtOrientation, role: int
  ): QVariant =
