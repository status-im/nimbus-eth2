...
import stew/byteutils, ../beacon_chain/spec/datatypes/base

func toDisplayHex*(v: openArray[byte]): string =
  "<pre>0x" & toHex(v) & "</pre>"

func toDisplayHex*(v: Eth2Digest): string =
  toDisplayHex(v.data)
func toDisplayHex*(v: ValidatorSig | TrustedSig): string =
  toDisplayHex(toRaw(v))
