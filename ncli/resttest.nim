...
import
  confutils, chronicles, httputils, chronos, chronos/streams/[asyncstream, tlsstream]
...
const
  ...
  RestTesterVersion* =
    $RestTesterMajor & "." & $RestTesterMinor & "." & $RestTesterPatch
  RestTesterIdent* = "RestTester/$1 ($2/$3)" % [RestTesterVersion, hostCPU, hostOS]
  RestTesterCopyright* =
    "Copyright(C) 2021-2022" & " Status Research & Development GmbH"
  RestTesterHeader* =
    RestTesterName & ", Version " & RestTesterVersion & " [" & hostOS & ": " & hostCPU &
    "]\r\n" & RestTesterCopyright & "\r\n"
...
type
  ...
  RestTesterConf* = object
    delayTime* {.
      defaultValue: 0,
      desc:
        "Time (in seconds) to wait before initial connection could be " & "established",
      abbr: "d",
      name: "delay"
    .}: int
    ...
    attemptTimeout* {.
      defaultValue: 60,
      desc:
        "Time (in seconds) during which to continue trying to establish " &
    ...
    topicsFilter* {.
      desc: "Topics which should be included in testing", name: "topic", abbr: "t"
    .}: seq[string]

    skipTopicsFilter* {.
      desc: "Topics which should be skipped in testing", name: "skip-topic", abbr: "s"
    .}: seq[string]
    ...
    url* {.argument, desc: "Address of remote REST server to test".}: string
...
proc getTestRules(conf: RestTesterConf): Result[seq[JsonNode], cstring] =
  let data = block:
    ...
    if res.isErr():
      fatal "Could not read rules file",
        error_msg = ioErrorMsg(res.error()),
        error_os_code = $res.error(),
  ...
  if len(elems) == 0:
    fatal "There empty array of rules found in file", filename = conf.rulesFilename
  ...
  notice "Rules file loaded", total_rules_count = len(elems), rules_count = len(res)
  ok(res)

proc openConnection*(
    address: TransportAddress, uri: Uri, flags: set[TLSFlags]
): Future[HttpConnectionRef] {.async.} =
  let transp =
    try:
  ...
  if uri.scheme == "http":
    ...
    let tlsstream =
      newTLSClientAsyncStream(treader, twriter, uri.hostname, flags = flags)
    return HttpConnectionRef(
    ...
    let conn =
      try:
        ...
        notice "Unable to establish connection with remote host",
          host = hostname, sleep_until_next_attempt = $(((sleepTime * 3) div 2).seconds)
        nil
...
proc prepareRequest(
    uri: Uri, rule: JsonNode
): Result[tuple[url: string, request: string], cstring] =
  let request = rule.getOrDefault("request")
  if isNil(request):
  ...
  let (requestBodyType, requestBodyData) = block:
  ...
  let (hostPresent, datePresent) = block:
    var flag1 = false
    var flag2 = false
    for item in requestHeaders:
...
proc getResponseStatusExpect(rule: JsonNode): Result[StatusExpect, cstring] =
  ...
  let kind = block:
    ...
    if isNil(joperator):
      if len(value) > 1: StatusOperatorKind.OneOf else: StatusOperatorKind.Equals
    else:
            ...
            if len(jstart.elems) != 0:
              for elem in jstart.elems:
                ...
                of JString:
                  ...
                  return
                    err("`response.body` element has incorrect `start`" & " option")
          else:
  ...
  if len(expect.items) == 0:
    ...
    for item in expect.items:
      ...
      of BodyOperatorKind.JsonStructCmpS, BodyOperatorKind.JsonStructCmpNS,
        ...
        let strict =
          if item.kind in
              {BodyOperatorKind.JsonStructCmpS, BodyOperatorKind.JsonStructCmpSAV}:
            true
          else:
            false
        let checkvalue =
...
proc runTest(
    conn: HttpConnectionRef, uri: Uri, rule: JsonNode, workerIndex: int, testIndex: int
): Future[TestResult] {.async.} =
  var times: array[4, Duration]
  let testName = rule.getTestName()
  let testPath = uri.path & rule.getTestName()

  debug "Running test",
    name = testName, test_index = testIndex, worker_index = workerIndex

  let (_, request) = block:
    let res = prepareRequest(uri, rule)
    if res.isErr():
      return TestResult.failure(
        TestResultKind.RuleError,
        "Could not read request data: " & $res.error(),
        times = times,
      )
    res.get()

  let statusExpect = block:
    let res = getResponseStatusExpect(rule)
    if res.isErr():
      return TestResult.failure(
        TestResultKind.RuleError,
        "Could not read response status data: " & $res.error(),
        times = times,
      )
    res.get()

  let headersExpect = block:
    let res = getResponseHeadersExpect(rule)
    if res.isErr():
      return TestResult.failure(
        TestResultKind.RuleError,
        "Could not read response headers data: " & $res.error(),
        times = times,
      )
    res.get()

  let bodyExpect = block:
    let res = getResponseBodyExpect(rule)
    if res.isErr():
      return TestResult.failure(
        TestResultKind.RuleError,
        "Could not read response body data: " & $res.error(),
        times = times,
      )
    res.get()
  ...
  try:
    ...
    return TestResult.failure(
      TestResultKind.WriteRequestError,
  ...
  let rlen =
    try:
      let sm = Moment.now()
      let res =
        await conn.reader.readUntil(addr headersBuf[0], len(headersBuf), HeadersMark)
      ...
      return TestResult.failure(
        TestResultKind.ReadResponseHeadersError,
  ...
  if contentLength < 0:
    ...
    if contentLength > 0:
      ...
      try:
        ...
        return TestResult.failure(
          TestResultKind.ReadResponseBodyError,
    ...
    if res1 and res2 and res3:
      ...
      let flags = block:
        ...
        res
      return
        TestResult.failure(TestResultKind.ValidationError, times = times, flags = flags)
...
proc workerLoop(
    ...
    outputQueue: AsyncQueue[TestCaseResult],
) {.async.} =
  let hostname = uri.hostname & ":" & uri.port
  ...
  while true:
    try:
      ...
      if isNil(conn):
        ...
        debug "Opened new connection with remote host",
      ...
      warn "Unable to establish connection with remote host",
        host = hostname, worker = worker
      return
    except CatchableError as exc:
      warn "Unexpected exception while running test test run",
        host = hostname,
        error_name = exc.name,
        error_msg = exc.msg,
        index = index,
...
proc startTests(
    conf: RestTesterConf, uri: Uri, rules: seq[JsonNode]
): Future[int] {.async.} =
  ...
  let address = block:
    let res = uri.getAddress()
    if res.isErr():
      fatal "Unable to resolve remote host address, exiting", uri = $uri
      return 1
    res.get()
      ...
      for i in 0 ..< len(pending):
        if pending[i].finished():
          if i < len(workers):
            ...
            notice "Got test result",
              name = rules[tcaseRes.index].getTestName(),
  ...
  let headerLine =
    ...
    alignLeft("STATUS", 20) & alignLeft("ERROR", 7) & alignLeft("ELAPSED", 20) &
  ...
  for index, item in rules:
    ...
    let line =
      ...
      alignLeft($results[index].kind, 20) & alignLeft(errorFlag, 7) &
      alignLeft($results[index].times[3], 20) & alignLeft($results[index].message, 20)
    echo line
...
proc run(conf: RestTesterConf): int =
  let uri = block:
    let res = conf.getUri()
    if res.isErr():
      fatal "Incomplete/incorrect URL", url = conf.url, error_msg = $res.error()
      return 1
    res.get()
  ...
  notice "Exploring remote server hostname", hostname = uri.hostname & ":" & uri.port
  try:
