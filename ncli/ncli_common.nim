...
import
  stew/bitops2,
  ../beacon_chain/spec/
    [beaconstate, state_transition_epoch, state_transition_block, signatures],
...
proc getUnaggregatedFilesLastEpoch*(
    dir: string
): Epoch {.raises: [OSError, ValueError].} =
  ...
  for index, validator in info.validators:
    ...
    template get_attestation_component_reward_helper(attesting_balance: Gwei): Gwei =
      get_attestation_component_reward(
        attesting_balance, info.balances.current_epoch, base_reward, finality_delay
      )
...
func collectFromSlashedValidator(
  ...
  let
    slashingPenalty = get_slashing_penalty(state, slashed_validator.effective_balance)
    whistleblowerReward =
  ...
  withStateAndBlck(forkedState, forkedBlock):
    when consensusFork > ConsensusFork.Phase0:
      ...
      for attestation in forkyBlck.message.body.attestations:
        doAssert check_attestation(forkyState.data, attestation, {}, cache, true).isOk
        let proposerReward =
        ...
        when consensusFork >= ConsensusFork.Electra:
          for index in get_attesting_indices(
            ...
            rewardsAndPenalties[index].inclusion_delay = Opt.some(inclusionDelay.uint64)
        else:
          for index in get_attesting_indices(
  ...
  withStateAndBlck(forkedState, forkedBlock):
    for deposit in forkyBlck.message.body.deposits:
      ...
      var index = findValidatorIndex(
        ...
        sortValidatorBuckets(forkyState.data.validators.asSeq)[], pubkey,
      )
      if index.isNone:
