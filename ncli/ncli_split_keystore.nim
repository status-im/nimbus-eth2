...
type Config = object
  threshold {.desc: "Used to generate distributed keys", name: "threshold".}: uint32

  remoteSignersUrls {.desc: "URLs of the remote signers", name: "remote-signer".}:
    seq[string]

  dataDir {.desc: "A directory containing validators and secrets", name: "data-dir".}:
    InputDir
  ...
  key {.desc: "A public key of the keystore", name: "key".}: string

  outDir {.
    desc: "A directory to store the generated validator keystores", name: "out-dir"
  .}: OutDir
...
proc main() =
  ...
  template rngCtx(): untyped =
    rng[]

  let
    validatorsDir = conf.validatorsDir
    secretsDir = conf.secretsDir
    keystore = loadKeystore(validatorsDir, secretsDir, conf.key, true, nil).valueOr:
    ...
    shares = generateSecretShares(
      keystore.privateKey, rngCtx, conf.threshold, sharesCount
    ).valueOr:
