...
proc makeSimulationBlock(
...
): Result[capella.BeaconBlock, cstring] =
  ...
  var blck = partialBeaconBlock(
    cfg, state, proposer_index, randao_reveal, eth1_data, graffiti, attestations,
    deposits, exits, sync_aggregate, execution_payload,
  )
...
proc makeSimulationBlock(
    ...
    # removed if we don't use invalid signatures there
    verificationFlags: UpdateFlags = {},
): Result[deneb.BeaconBlock, cstring] =
  ## Create a block for the given state. The latest block applied to it will
  ## be used for the parent_root value, and the slot will be take from
  ## state.slot meaning process_slots must be called up to the slot for which
  ## the block is to be created.
...
cli do(
  ...
  eth1Chain.addBlock Eth1Block(
    number: Eth1BlockNumber 1, timestamp: Eth1BlockTimestamp genesisTime
  )
      ...
      for committee_index in get_committee_indices(committees_per_slot):
        ...
        for index_in_committee, validator_index in committee:
          if rand(r, 1.0) <= attesterRatio:
            let
              data = makeAttestationData(updatedState, slot, committee_index, bid.root)
              sig = get_attestation_signature(
                fork, genesis_validators_root, data, MockPrivKeys[validator_index]
              )
              attestation = Attestation
                .init(
                  [uint64 index_in_committee], committee.len, data, sig.toValidatorSig()
                )
                .expect("valid data")
    ...
    for aggregator in aggregators:
      ...
      if contributionWasProduced:
        let
          ...
          validatorPrivKey = MockPrivKeys[aggregator.validatorIdx.ValidatorIndex]
  ...
  proc getNewBlock[T](
      state: var ForkedHashedBeaconState, slot: Slot, cache: var StateCache
  ): T =
    let
      finalizedEpochRef = dag.getFinalizedEpochRef()
      proposerIdx =
        get_beacon_proposer_index(state, cache, getStateField(state, slot)).get()
  ...
  proc proposeDenebBlock(slot: Slot) =
    ...
    dag.withUpdatedState(tmpState[], dag.getBlockIdAtSlot(slot).expect("block")):
      let
        ...
        added = dag.addHeadBlock(verifier, newBlock) do(
          blckRef: BlockRef,
          signedBlock: deneb.TrustedSignedBeaconBlock,
          epochRef: EpochRef,
          unrealized: FinalityCheckpoints
        ):
          # Callback add to fork choice if valid
  ...
  for i in 0 ..< slots:
    ...
    while true:
      let nextBlockTime =
        lastEth1BlockAt + max(1.0, gauss(r, float cfg.SECONDS_PER_ETH1_BLOCK, 3.0))
    ...
    if blockRatio > 0.0:
      withTimer(timers[t]):
        ...
        of ConsensusFork.Deneb:
          proposeDenebBlock(slot)
        of ConsensusFork.Capella:
          proposeCapellaBlock(slot)
  ...
  if replay:
    withTimer(timers[tReplay]):
      ...
      doAssert dag.updateState(
        replayState[], dag.getBlockIdAtSlot(Slot(slots)).expect("block"), false, cache
      )
