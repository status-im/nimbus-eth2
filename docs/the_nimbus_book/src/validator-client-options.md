...
| payload-attestation | payload-attestation-data, payload-attestation-publish                                             |
| publish             | attestation-publish, aggregated-publish, block-publish, sync-publish, payload-attestation-publish |
| data                | attestation-data, aggregated-data, block-data, sync-data, payload-attestation-data                |
| all                 | attestation, aggregated, block, sync, payload-attestation, duties                                 |
...
The configuration can be defined in a TOML file, and loaded using the `--config-file` option. Command line arguments take precedence over the config file. The config file can be reloaded while running, as follows.
...
=== "Windows"
    ...
    Get-Process nimbus_validator_client -EA SilentlyContinue | % {
        ...
        try { $e = [System.Threading.EventWaitHandle]::OpenExisting($n) } catch { return }
...
Sentry nodes setups allow separating block production traffic from attestations, payload attestations and sync committee messages, making sure that a separate public IP address is used when proposing blocks.
