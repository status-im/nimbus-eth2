...
proc getForkedBlock*(
  ...
  if (let blck = db.getBlock(root, deneb.TrustedSignedBeaconBlock); blck.isSome()):
...
proc getNearbyState(
    dag: ChainDAGRef, state: ref ForkedHashedBeaconState, bid: BlockId, lowSlot: Slot
): Opt[void] =
...
proc advanceSlots*(
  ...
  while getStateField(state, slot) < slot:
    ...
    process_slots(
      dag.cfg, state, getStateField(state, slot) + 1, cache, info, dag.updateFlags
    )
    ...
    if save:
      ...
      withState(state):
        ...
        if preEpoch != postEpoch:
          dag.validatorMonitor[].registerEpochInfo(postEpoch, info, forkyState.data)
...
proc applyBlock(
  ...
  of ConsensusFork.Phase0:
    ...
    state_transition(
      dag.cfg, state, data, cache, info, dag.updateFlags + {slotProcessed}, noRollback
    )
...
proc init*(
  ...
  for blck in db.getAncestorSummaries(head.root):
    ...
    let newRef = BlockRef.init(
      blck.root, Opt.none Eth2Digest, executionValid = false, blck.summary.slot
    )
    if headRef == nil:
      headRef = newRef
  ...
  block:
    ...
    while headBlocks.len > 0:
      dag.applyBlock(dag.headState, headBlocks.pop().bid, cache, info).expect(
        "head blocks should apply"
      )
  ...
  dag.era =
    EraDB.new(cfg, eraPath, getStateField(dag.headState, genesis_validators_root))
...
proc computeRandaoMix(
    dag: ChainDAGRef, bdata: ForkedTrustedSignedBeaconBlock
): Opt[Eth2Digest] =
...
proc getBlockRange*(
    dag: ChainDAGRef, startSlot: Slot, skipStep: uint64, output: var openArray[BlockId]
): Natural =
...
proc updateHead*(
  ...
  let
    ...
    lastKnownValidatorsChangeStatuses =
      getValidatorChangeStatuses(dag.headState, knownValidators)
  ...
  var cache: StateCache
  if not updateState(dag, dag.headState, newHead.bid.atSlot(), false, cache):
  ...
  if dag.headState.is_merge_transition_complete() and not lastHeadMergeComplete and
  ...
  if dag.vanityLogs.onKnownBlsToExecutionChange != nil and
      checkBlsToExecutionChanges(dag.headState, lastKnownValidatorsChangeStatuses):
...
proc getProposalState*(
  ...
  if not state[].can_advance_slots(head.root, slot):
    ...
    if not dag.updateState(
      state[], head.atSlot(slot - 1).toBlockSlotId().expect("not nil"), false, cache
    ):
