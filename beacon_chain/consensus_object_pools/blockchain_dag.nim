  ...
  withConsensusFork(dag.cfg.consensusForkAtEpoch(bid.slot.epoch)):
    when consensusFork >= ConsensusFork.Gloas:
      let data = dag.db.getExecutionPayloadEnvelope(bid.root).valueOr:
        ...
        # This is valid in ePBS. If this is wrong (DB corruption), the next
        # block's process_execution_payload_bid will catch the mismatch
        # between parent_block_hash and state.latest_block_hash.
        return ok()
