...
const MaxBlobs = SLOTS_PER_EPOCH * MAX_BLOBS_PER_BLOCK
...
func hasBlob*(
    quarantine: BlobQuarantine, slot: Slot, proposer_index: uint64, index: BlobIndex
): bool =
  for blob_sidecar in quarantine.blobs.values:
    template block_header(): untyped =
      blob_sidecar.signed_block_header.message

    if block_header.slot == slot and block_header.proposer_index == proposer_index and
        blob_sidecar.index == index:
      return true
  false

func popBlobs*(
    quarantine: var BlobQuarantine, digest: Eth2Digest, blck: deneb.SignedBeaconBlock
): seq[ref BlobSidecar] =
...
func hasBlobs*(quarantine: BlobQuarantine, blck: deneb.SignedBeaconBlock): bool =
  for idx, kzg_commitment in blck.message.body.blob_kzg_commitments:
...
func blobFetchRecord*(
    quarantine: BlobQuarantine, blck: deneb.SignedBeaconBlock
): BlobFetchRecord =
