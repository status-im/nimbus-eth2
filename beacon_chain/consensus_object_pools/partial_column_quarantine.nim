...
type
  ...
  PartialColumnKey* = object
    ## Composite key: block identifier plus column index.
    blockId*: gloas.PartialDataColumnGroupID
    columnIndex*: ColumnIndex

  PartialColumnQuarantine* = object
    ## Quarantine for partial data column messages,
    ##
    ## Stores group-ids and tracks which cells have been received for each
    ## (block_id, column_index) pair. Validation of a group-id on any subnet
...
func init*(T: typedesc[PartialColumnQuarantine]): T =
  T(
    headers: typeof(T.headers).init(MaxPartialHeaders),
    entries: LruCache[PartialColumnKey, PartialColumnEntry].init(
      MaxPartialEntries))

# --- Group-id management ---

func hasPartialGroupID*(
    quarantine: PartialColumnQuarantine,
    blockId: gloas.PartialDataColumnGroupID): bool =
  ## Returns true if a validated group-id exists for `blockId`.
...
func putPartialGroupID*(
  ...
  ## The group-id is both the key and the stored metadata.
...
func hasEntry*(
  ...
  let key = PartialColumnKey(blockId: blockId, columnIndex: columnIndex)
...
func putEntry*(
    quarantine: var PartialColumnQuarantine,
    blockId: gloas.PartialDataColumnGroupID,
    columnIndex: ColumnIndex, entry: PartialColumnEntry) =
...
func getOrCreateEntry*(
    quarantine: var PartialColumnQuarantine,
    blockId: gloas.PartialDataColumnGroupID, columnIndex: ColumnIndex,
  ...
  let entry = PartialColumnEntry(
    headerValidated: quarantine.hasPartialGroupID(blockId),
...
func markCellReceived*(
    quarantine: var PartialColumnQuarantine,
    blockId: gloas.PartialDataColumnGroupID,
    columnIndex: ColumnIndex, blobIndex: int, cell: KzgCell, proof: KzgProof) =
...
func hasCellReceived*(
    quarantine: var PartialColumnQuarantine,
    blockId: gloas.PartialDataColumnGroupID,
    columnIndex: ColumnIndex, blobIndex: int): bool =
...
func addCells*(
    quarantine: var PartialColumnQuarantine,
    blockId: gloas.PartialDataColumnGroupID, columnIndex: ColumnIndex,
    sidecar: ref gloas.PartialDataColumnSidecar) =
...
func isComplete*(
    quarantine: var PartialColumnQuarantine,
    blockId: gloas.PartialDataColumnGroupID, columnIndex: ColumnIndex): bool =
  ## Returns true if all cells have been received for the given
  ## (block_id, column_index) pair and the group-id has been validated.
  let key = PartialColumnKey(blockId: blockId, columnIndex: columnIndex)
  let entry = quarantine.entries.get(key).valueOr:
    return false
...
func removeHeader*(
    quarantine: var PartialColumnQuarantine,
    blockId: gloas.PartialDataColumnGroupID) =
  ## Remove the group-id associated with `blockId`.
