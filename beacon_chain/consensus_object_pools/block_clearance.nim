...
proc checkHeadBlock*(
  ...
  when typeof(signedBlock).kind >= ConsensusFork.Gloas:
    ...
    if parentFork >= ConsensusFork.Gloas and
        isParentBlockFull(dag, signedBlock, parent) and
        not dag.db.containsExecutionPayloadEnvelope(parent.root()):
      debug "Parent envelope missing",
        head = shortLog(dag.head),
        blckSlot = signedBlock.message.slot,
        blckRoot = signedBlock.root,
        pBlockHash = parent.executionBlockHash,
        pParentHash = parent.executionParentHash,
        isFull = isParentBlockFull(dag, signedBlock, parent),
        inDB = dag.db.containsExecutionPayloadEnvelope(parent.root())
