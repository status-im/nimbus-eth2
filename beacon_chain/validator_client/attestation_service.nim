...
proc serveAggregateAndProof*(
  ...
  let signedProof = SignedAggregateAndProof(message: proof, signature: signature)
  logScope:
...
proc produceAndPublishAttestations*(
  ...
  let data =
    await vc.produceAttestationData(slot, committee_index, ApiStrategyKind.Best)
...
proc produceAndPublishAggregates(
  ...
  let aggregateItems = block:
    ...
    for duty in duties:
      ...
      if (duty.data.slot != slot) or (duty.data.committee_index != committeeIndex):
  ...
  if len(aggregateItems) > 0:
    ...
    debug "No aggregate and proofs scheduled for slot",
...
proc publishAttestationsAndAggregates(
  ...
  let ad =
    try:
      ...
      error "Unexpected error while producing attestations",
        slot = slot,
        committee_index = committee_index,
        duties_count = len(duties),
        err_name = exc.name,
...
proc spawnAttestationTasks(service: AttestationServiceRef, slot: Slot) {.async.} =
  let vc = service.client
  let dutiesByCommittee = block:
...
): Future[AttestationServiceRef] {.async.} =
  logScope:
    service = ServiceName
