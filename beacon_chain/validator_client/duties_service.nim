...
proc pollForValidatorIndices*(
  ...
  for item in validators:
    ...
    if validator.isNone():
      ...
      vc.attachedValidators[].updateValidator(
        validator.get(),
                ...
                let res =
                  try:
                    await vc.getAttesterDuties(
                      epoch, chunk, vc.getMode()[FnKind.getAttesterDuties]
                    )
                  except ValidatorApiError as exc:
...
proc pollForSyncCommitteeDuties*(
  ...
  let
    vc = service.client
    indices = toSeq(vc.attachedValidators[].indices())
    epoch = max(period.start_epoch(), vc.getAltairEpoch())
        ...
        let res =
          try:
            await vc.getSyncCommitteeDuties(
              epoch, chunk, vc.getMode()[FnKind.getSyncCommitteeDuties]
            )
          except ValidatorApiError as exc:
    ...
    block:
      let
        ...
        sigres = await vc.fillAttestationSelectionProofs(
          currentSlot, currentSlot + AGGREGATION_PRE_COMPUTE_SLOTS
        )
...
proc pollForSyncCommitteeDuties*(
  ...
  if not vc.isPastAltairFork(currentEpoch):
    notice "Sync committee duties will not be queried before ALTAIR fork",
      epoch = currentEpoch
    # We are not going to poll for sync committee duties until `ALTAIR` epoch.
    return
...
proc pollForBeaconProposers*(
  ...
  if vc.attachedValidators[].count() != 0:
    try:
      let res =
        await vc.getProposerDuties(currentEpoch, vc.getMode()[FnKind.getProposerDuties])
      let
  ...
  let
    ...
    registrations =
      try:
        ...
        debug "Validator registration preparation was interrupted", slot = currentSlot
        raise exc
      ...
      if len(registrations) > 0:
        try:
          ...
          debug "Validator registration was interrupted",
            slot = currentSlot, version = genesis_fork_version
          raise exc
      else:
        0
...
proc slashingDatabasePruningLoop(
  ...
  while true:
    let slot = await vc.checkedWaitForSlot(
      vc.getNextEpochMiddleSlot(), vc.timeParams.aggregateSlotOffset, false
    )
    if slot.isNone():
      continue
