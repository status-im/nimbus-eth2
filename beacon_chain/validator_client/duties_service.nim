...
type DutiesServiceLoop* = enum
  AttesterLoop
  ProposerLoop
  IndicesLoop
  SyncCommitteeLoop
  ProposerPreparationLoop
  ValidatorRegisterLoop
  DynamicValidatorsLoop
  SlashPruningLoop
...
proc pollForValidatorIndices*(service: DutiesServiceRef) {.async.} =
  ...
  for item in validators:
    ...
    if validator.isNone():
      ...
      validator.get().updateValidator(
        Opt.some ValidatorAndIndex(index: item.index, validator: item.validator)
      )
  ...
  if len(updated) > 0:
    ...
    trace "Validator indices update dump",
      missing_validators = missing, updated_validators = updated
    vc.indicesAvailable.fire()

proc pollForAttesterDuties*(
    service: DutiesServiceRef, epoch: Epoch
): Future[int] {.async.} =
  var currentRoot: Opt[Eth2Digest]
  let
    vc = service.client
    indices = toSeq(vc.attachedValidators[].indices())
    relevantDuties = block:
        ...
        while true:
          block innerLoop:
            for chunk in indices.chunks(DutiesMaximumValidatorIds):
              let res =
                try:
                  await vc.getAttesterDuties(epoch, chunk, ApiStrategyKind.First)
                except ValidatorApiError as exc:
                  warn "Unable to get attester duties",
              ...
              for duty in res.data:
                if checkDuty(duty) and (duty.pubkey in vc.attachedValidators[]):
                  duties.add(duty)
              break mainLoop
      duties
      ...
      vc.attesters.withValue(duty.pubkey, map):
        map[].duties.withValue(epoch, epochDuty):
          ...
          if checkReorg(epochDuty[], currentRoot):
            ...
            res.add((epoch, duty))
      if not (dutyFound):
        info "Received new attester duty",
          duty, epoch = epoch, dependent_root = currentRoot.get()
        res.add((epoch, duty))
    res

  for item in addOrReplaceItems:
    let dap = DutyAndProof.init(
      item.epoch, currentRoot.get(), item.duty, Opt.none(ValidatorSig)
    )

    vc.attesters.mgetOrPut(dap.data.pubkey, default(EpochDuties)).duties[dap.epoch] =
      dap
...
proc pollForSyncCommitteeDuties*(
    service: DutiesServiceRef, period: SyncCommitteePeriod
): Future[int] {.async.} =
  let
    vc = service.client
    indices = toSeq(vc.attachedValidators[].indices())
    ...
    relevantDuties = block:
      ...
      for chunk in indices.chunks(DutiesMaximumValidatorIds):
        let res =
          try:
            await vc.getSyncCommitteeDuties(epoch, chunk, ApiStrategyKind.First)
          except ValidatorApiError as exc:
            warn "Unable to get sync committee duties",
              period = period, epoch = epoch, reason = exc.getFailureReason()
            return 0
          except CancelledError as exc:
            ...
            error "Unexpected error while getting sync committee duties",
              period = period, epoch = epoch, err_name = exc.name, err_msg = exc.msg
            return 0
  ...
  let addOrReplaceItems = block:
    var
      alreadyWarned = false
      res: seq[tuple[period: SyncCommitteePeriod, duty: RestSyncCommitteeDuty]]
    for duty in relevantDuties:
      var dutyFound = false
  ...
  for item in addOrReplaceItems:
    vc.syncCommitteeDuties.mgetOrPut(item.duty.pubkey, default(SyncPeriodDuties)).duties[
      item.period
    ] = item.duty
    ...
    for epochKey, epochDuty in item.duties:
      if (epochKey + HISTORICAL_DUTIES_EPOCHS) >= epoch:
        ...
        debug "Attester duties for the epoch has been pruned",
          validator = key, epoch = epochKey, loop = AttesterLoop
    attesters[key] = v
...
proc pollForAttesterDuties*(service: DutiesServiceRef) {.async.} =
  ...
  if vc.attachedValidators[].count() != 0:
    ...
    counts[0] = (currentEpoch, await service.pollForAttesterDuties(currentEpoch))
    counts[1] = (nextEpoch, await service.pollForAttesterDuties(nextEpoch))
    ...
    block:
      let
        moment = Moment.now()
        sigres = await vc.fillAttestationSelectionProofs(
            ...
            if currentSlot + SUBSCRIPTION_BUFFER_SLOTS < duty.data.slot:
              let isAggregator =
                if duty.slotSig.isSome():
                  is_aggregator(duty.data.committee_length, duty.slotSig.get())
                else:
                  false
...
proc pollForSyncCommitteeDuties*(service: DutiesServiceRef) {.async.} =
  ...
  if vc.attachedValidators[].count() != 0:
    ...
    counts[0] = (currentPeriod, await service.pollForSyncCommitteeDuties(currentPeriod))

    const
      numDelayEpochs = 4 # Chosen empirically
      numLookaheadEpochs =
        max(EPOCHS_PER_SYNC_COMMITTEE_PERIOD, numDelayEpochs) - numDelayEpochs + 1
    if (currentEpoch + numLookaheadEpochs) >= nextPeriod.start_epoch:
      counts[1] = (nextPeriod, await service.pollForSyncCommitteeDuties(nextPeriod))
    else:
    ...
    block:
      let
        moment = Moment.now()
        sigres = await vc.fillSyncCommitteeSelectionProofs(
          currentSlot, currentSlot + Epoch(AGGREGATION_PRE_COMPUTE_EPOCHS)
        )
    ...
    let
      periods = block:
        ...
        if service.syncSubscriptionEpoch.get(FAR_FUTURE_EPOCH) != currentEpoch:
          res.add((currentSlot, currentPeriod))
        let
          lookaheadSlot = currentSlot + SUBSCRIPTION_LOOKAHEAD_EPOCHS * SLOTS_PER_EPOCH
      ...
      subscriptions = block:
        ...
        for item in periods:
          let
            ...
            subscriptionsInfo = vc.syncMembersSubscriptionInfoForPeriod(item.period)
          for info in subscriptionsInfo:
            let sub = RestSyncCommitteeSubscription(
              validator_index: info.validator_index,
              sync_committee_indices: info.validator_sync_committee_indices,
...
proc pruneBeaconProposers(service: DutiesServiceRef, epoch: Epoch) =
  ...
  for epochKey, data in vc.proposers:
    if (epochKey + HISTORICAL_DUTIES_EPOCHS) >= epoch:
      ...
      debug "Proposer duty has been pruned", epoch = epochKey, loop = ProposerLoop
...
proc pollForBeaconProposers*(service: DutiesServiceRef) {.async.} =
  ...
  if vc.attachedValidators[].count() != 0:
    try:
      let res = await vc.getProposerDuties(currentEpoch, ApiStrategyKind.First)
      let
      ...
      if len(relevantDuties) > 0:
        ...
        debug "No relevant proposer duties received",
          slot = currentSlot, duties_count = len(duties)
    except ValidatorApiError as exc:
      notice "Unable to get proposer duties",
        slot = currentSlot, epoch = currentEpoch, reason = exc.getFailureReason()
    except CancelledError as exc:
      debug "Proposer duties processing was interrupted"
      raise exc
    except CatchableError as exc:
      debug "Unexpected error occured while getting proposer duties",
        slot = currentSlot, epoch = currentEpoch, err_name = exc.name, err_msg = exc.msg
  ...
  if len(proposers) > 0:
    let count =
      try:
        ...
        warn "Unable to prepare beacon proposers",
          slot = currentSlot,
          epoch = currentEpoch,
          err_name = exc.name,
          err_msg = exc.msg,
...
proc registerValidators*(service: DutiesServiceRef) {.async.} =
    ...
    registrations =
      try:
        ...
        error "Unexpected error occured while preparing validators " &
          "registration data",
          slot = currentSlot, fork = genesisFork, err_name = exc.name, err_msg = exc.msg
        default
    ...
    count =
      if len(registrations) > 0:
        try:
          ...
          warn "Unable to register validators",
            slot = currentSlot,
            fork = genesisFork,
            err_name = exc.name,
            err_msg = exc.msg,
  ...
  if count > 0:
    debug "Validators registered",
      slot = currentSlot,
      beacon_nodes_count = count,
...
proc attesterDutiesLoop(service: DutiesServiceRef) {.async.} =
  ...
  await allFutures(
    vc.preGenesisEvent.wait(), vc.indicesAvailable.wait(), vc.forksAvailable.wait()
  )
...
proc dynamicValidatorsLoop*(
    service: DutiesServiceRef, web3signerUrl: Web3SignerUrl, intervalInSeconds: int
) {.async.} =
  let vc = service.client
...
proc proposerPreparationsLoop(service: DutiesServiceRef) {.async.} =
  ...
  await allFutures(vc.preGenesisEvent.wait(), vc.indicesAvailable.wait())
  while true:
...
proc pruneSlashingDatabase(service: DutiesServiceRef) {.async.} =
  let
    ...
    blockHeader =
      try:
        ...
        debug "Finalized block header request was interrupted", slot = currentSlot
        raise exc
      except CatchableError as exc:
        error "Unexpected error occured while requesting " & "finalized block header",
          slot = currentSlot, err_name = exc.name, err_msg = exc.msg
  ...
  if blockHeader.isSome():
    ...
    if service.lastSlashingEpoch.get(FAR_FUTURE_EPOCH) != epoch:
      vc.attachedValidators[].slashingProtection.pruneAfterFinalization(epoch)
      service.lastSlashingEpoch = Opt.some(epoch)
      let finishTime = Moment.now()
      debug "Slashing database has been pruned",
        slot = currentSlot,
...
proc slashingDatabasePruningLoop(service: DutiesServiceRef) {.async.} =
  ...
  doAssert(len(vc.forks) > 0, "Fork schedule must not be empty at this point")
  while true:
    let slot = await vc.checkedWaitForSlot(
      vc.getNextEpochMiddleSlot(), aggregateSlotOffset, false
    )
    if slot.isNone():
      continue
...
template checkAndRestart(
    serviceLoop: DutiesServiceLoop, future: Future[void], body: untyped
): untyped =
  if future.finished():
    if future.failed():
      ...
      debug "The loop is finished unexpectedly without an error", loop = serviceLoop
    future = body
    ...
    let breakLoop =
      try:
        ...
        if not (isNil(registerFut)):
          futures.add(FutureBase(registerFut))
        ...
        checkAndRestart(SyncCommitteeLoop, syncFut, service.syncCommitteeDutiesLoop())
        checkAndRestart(
          ProposerPreparationLoop, prepareFut, service.proposerPreparationsLoop()
        )
        if not (isNil(registerFut)):
          checkAndRestart(
            ValidatorRegisterLoop, registerFut, service.validatorRegisterLoop()
          )
        ...
        checkAndRestart(
          SlashPruningLoop, slashPruningFut, service.slashingDatabasePruningLoop()
        )
        false
      except CancelledError:
        ...
        warn "Service crashed with unexpected error",
          err_name = exc.name, err_msg = exc.msg
        true
...
proc init*(
    t: typedesc[DutiesServiceRef], vc: ValidatorClientRef
): Future[DutiesServiceRef] {.async.} =
  logScope:
    service = ServiceName
  let res =
    DutiesServiceRef(name: ServiceName, client: vc, state: ServiceState.Initialized)
