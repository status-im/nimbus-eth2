...
proc serveContributionAndProof*(
  ...
  let signature = block:
    let res = await validator.getContributionAndProofSignature(fork, genesisRoot, proof)
    if res.isErr():
  ...
  let res =
    try:
      await vc.publishContributionAndProofs(@[restSignedProof], ApiStrategyKind.First)
    except ValidatorApiError as exc:
...
proc publishSyncMessagesAndContributions(
  ...
  block:
    ...
    debug "Producing sync committee messages",
  ...
  try:
    ...
    error "Unexpected error while producing sync committee messages",
      slot = slot, duties_count = len(duties), err_name = exc.name, err_msg = exc.msg
    return
...
proc processSyncCommitteeTasks(service: SyncCommitteeServiceRef, slot: Slot) {.async.} =
  let
    vc = service.client
...
): Future[SyncCommitteeServiceRef] {.async.} =
  logScope:
    service = ServiceName
