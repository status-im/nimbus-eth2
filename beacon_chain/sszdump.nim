...
proc dump*(dir: string, v: gloas.SignedExecutionPayloadEnvelope) =
  logErrors:
    let
      slot = v.message.payload.slot_number
      blck = shortLog(v.message.beacon_block_root)
    SSZ.saveFile(dir / &"envelope-{slot}-{blck}.ssz", v)
