...
import json_serialization/std/net, ./conf, ./nimbus_binary_common

export net, conf

type LightClientConf* = object # Config
  ...
  directPeers* {.
    desc:
      "The list of priviledged, secure and known peers to connect and maintain the connection to, this requires a not random netkey-file. In the complete multiaddress format like: /ip4/<address>/tcp/<port>/p2p/<peerId-public-key>. Peering agreements are established out of band and must be reciprocal.",
    name: "direct-peer"
  .}: seq[string]
  ...
  syncLightClientFinality* {.
    desc:
      "Whether the light client should including finality information when syncing execution layers",
    defaultValue: false,
    name: "sync-light-client-finality"
  .}: bool
