...
import
  ...
  json_serialization,
  web3/[primitives, confutils_defs],
  kzg4844/kzg_ex,
...
from fork_choice/fork_choice_types import ForkChoiceVersion
from consensus_object_pools/block_pools_types_light_client import
  LightClientDataImportMode

export
  uri, nat, enr, defaultEth2TcpPort, enabledLogLevel, defs, parseCmdArg, completeCmdArg,
  network_metadata, el_conf, network, BlockHashOrNumber, confTomlDefs, confTomlNet,
  confTomlUri, LightClientDataImportMode
...
type
  ...
  BeaconNodeConf* = object
    ...
    validatorsDirFlag* {.
      desc: "A directory containing validator keystores", name: "validators-dir"
    .}: Option[InputDir]

    verifyingWeb3Signers* {.
      desc: "Remote Web3Signer URL that will be used as a source of validators",
      name: "verifying-web3-signer-url"
    .}: seq[Uri]
    ...
    provenBlockProperties* {.
      ...
      name: "proven-block-property"
    .}: seq[string]

    web3Signers* {.
      desc: "Remote Web3Signer URL that will be used as a source of validators",
      name: "web3-signer-url"
    .}: seq[Uri]
    ...
    secretsDirFlag* {.
      desc: "A directory containing validator keystore passwords", name: "secrets-dir"
    .}: Option[InputDir]

    walletsDirFlag* {.desc: "A directory containing wallet files", name: "wallets-dir".}:
      Option[InputDir]

    eraDirFlag* {.hidden, desc: "A directory containing era files", name: "era-dir".}:
      Option[InputDir]

    web3ForcePolling* {.
      hidden,
      desc:
        "Force the use of polling when determining the head block of Eth1 (obsolete)",
      name: "web3-force-polling"
    .}: Option[bool]

    web3Urls* {.desc: "One or more execution layer Engine API URLs", name: "web3-url".}:
      seq[EngineApiUrlConfigValue]

    elUrls* {.desc: "One or more execution layer Engine API URLs", name: "el".}:
      seq[EngineApiUrlConfigValue]
    ...
    optimistic* {.
      ...
      desc:
        "Run the node in optimistic mode, allowing it to optimistically sync without an execution client (flag deprecated, always on)",
      name: "optimistic"
    .}: Option[bool]
    ...
    requireEngineAPI* {.
      ...
      desc:
        "Require Nimbus to be configured with an Engine API end-point after the Bellatrix fork epoch",
      name: "require-engine-api-in-bellatrix"
    .}: Option[bool]
    ...
    case cmd* {.command, defaultValue: BNStartUpCmd.noCommand.}: BNStartUpCmd
    of BNStartUpCmd.noCommand:
      ...
      syncLightClient* {.
        ...
        defaultValue: true,
        name: "sync-light-client"
      .}: bool

      trustedBlockRoot* {.
        desc:
          "Recent trusted finalized block root to sync from external " &
      ...
      trustedStateRoot* {.
        desc:
          "Recent trusted finalized state root to sync from external " &
          "beacon API (with `--external-beacon-api-url`)",
      ...
      finalizedCheckpointState* {.
        ...
        name: "finalized-checkpoint-state"
      .}: Option[InputFile]
      ...
      finalizedDepositTreeSnapshot* {.
        ...
        name: "finalized-deposit-tree-snapshot"
      .}: Option[InputFile]

      finalizedCheckpointBlock* {.
        hidden,
        desc: "SSZ file specifying a recent finalized block",
        name: "finalized-checkpoint-block"
      .}: Option[InputFile]

      nodeName* {.
        desc:
          "A name for this node that will appear in the logs. " &
      ...
      statusBarContents* {.
        ...
        defaultValue:
          "peers: $connected_peers;" & "finalized: $finalized_root:$finalized_epoch;" &
          "head: $head_root:$head_epoch:$head_epoch_slot;" &
          "time: $epoch:$epoch_slot ($slot);" & "sync: $sync_status|" &
          "ETH: $attached_validators_balance",
        defaultValueDesc: "",
        name: "status-bar-contents"
      .}: string
      ...
      restEnabled* {.desc: "Enable the REST server", defaultValue: false, name: "rest".}:
        bool
      ...
      keymanagerEnabled* {.
        desc: "Enable the REST keymanager API", defaultValue: false, name: "keymanager"
      .}: bool
      ...
      inProcessValidators* {.
        ...
        defaultValue: true,
          # the use of the nimbus_signing_process binary by default will be delayed until async I/O over stdin/stdout is developed for the child process.
        name: "in-process-validators"
      .}: bool
      ...
      syncHorizon* {.
        hidden,
        desc:
          "Number of empty slots to process before considering the client out of sync",
      ...
      suggestedFeeRecipient* {.
        desc: "Suggested fee recipient", name: "suggested-fee-recipient"
      .}: Option[Address]
      ...
      payloadBuilderUrl* {.
        desc: "Payload builder URL", defaultValue: "", name: "payload-builder-url"
      .}: string
      ...
      trustedSetupFile* {.
        ...
        name: "temporary-debug-trusted-setup-file"
      .}: Option[string]

      bandwidthEstimate* {.
        hidden,
        desc: "Bandwidth estimate for the node (bits per second)",
        name: "debug-bandwidth-estimate"
      .}: Option[Natural]

      forkChoiceVersion* {.
        hidden,
        desc: "Forkchoice version to use. " & "Must be one of: stable",
        name: "debug-forkchoice-version"
      .}: Option[ForkChoiceVersion]
    of BNStartUpCmd.wallets:
      ...
      of WalletsCmd.create:
        ...
        createdWalletNameFlag* {.
          desc: "An easy-to-remember name for the wallet of your choice", name: "name"
        .}: Option[WalletName]

        createdWalletFileFlag* {.desc: "Output wallet file", name: "out".}:
          Option[OutFile]
      of WalletsCmd.restore:
        ...
        restoredWalletFileFlag* {.desc: "Output wallet file", name: "out".}:
          Option[OutFile]

        restoredDepositsCount* {.
          desc:
            "Expected number of deposits to recover. If not specified, " &
    ...
    of BNStartUpCmd.deposits:
      ...
      of DepositsCmd.createTestnetDeposits:
        totalDeposits* {.
          desc: "Number of deposits to generate", defaultValue: 1, name: "count"
        .}: int
        ...
        outDepositsFile* {.
          desc: "The name of generated deposits file", name: "out-deposits-file"
        .}: Option[OutFile]
        ...
        newWalletFileFlag* {.desc: "Output wallet file", name: "new-wallet-file".}:
          Option[OutFile]
      ...
      of DepositsCmd.`import`:
        importedDepositsDir* {.argument, desc: "A directory with keystores to import".}:
          Option[InputDir]

        importMethod* {.
          desc: "Specifies which import method will be used (" & "normal, single-salt)",
      ...
      of DepositsCmd.exit:
        exitedValidators* {.
          desc:
            "One or more validator index, public key or a keystore path of " &
        ...
        exitAllValidatorsFlag* {.
          desc:
            "Exit all validators in the specified data directory or validators directory",
          defaultValue: false,
          name: "all"
        .}: bool

        exitAtEpoch* {.
          name: "epoch", defaultValueDesc: "immediately", desc: "The desired exit epoch"
        .}: Option[uint64]
    ...
    of BNStartUpCmd.record:
      ...
      of RecordCmd.create:
        ipExt* {.desc: "External IP address", name: "ip".}: IpAddress

        tcpPortExt* {.desc: "External TCP port", name: "tcp-port".}: Port

        udpPortExt* {.desc: "External UDP port", name: "udp-port".}: Port

        seqNumber* {.
          desc: "Record sequence number", defaultValue: 1, name: "seq-number"
        .}: uint
        ...
        .}: seq[(string)]
      of RecordCmd.print:
        recordPrint* {.argument, desc: "ENR URI of the record to print", name: "enr".}:
          Record
    of BNStartUpCmd.web3:
      case web3Cmd* {.command.}: Web3Cmd
      of Web3Cmd.test:
        web3TestUrl* {.argument, desc: "The web3 provider URL to test", name: "url".}:
          Uri
    of BNStartUpCmd.slashingdb:
      case slashingdbCmd* {.command.}: SlashProtCmd
      of SlashProtCmd.`import`:
        importedInterchangeFile* {.
          desc: "EIP-3076 slashing protection interchange file to import", argument
        .}: InputFile
      of SlashProtCmd.`export`:
        exportedValidators* {.
          desc:
            "Limit the export to specific validators " &
            "(specified as numeric indices or public keys)",
          abbr: "v",
          name: "validator"
        .}: seq[PubKey0x]
        exportedInterchangeFile* {.
          desc: "EIP-3076 slashing protection interchange file to export", argument
        .}: OutFile
    of BNStartUpCmd.trustedNodeSync:
      ...
      stateId* {.
        desc:
          "State id to sync to - this can be \"finalized\", a slot number or state hash or \"head\"",
        name: "state-id"
      .}: Option[string]
      ...
      blockId* {.
        hidden,
        desc:
          "Block id to sync to - this can be a block root, slot number, \"finalized\" or \"head\" (deprecated)"
      .}: Option[string]
      ...
      reindex* {.
        desc:
          "Recreate historical state index at end of backfill, allowing full history access (requires full backfill)",
        defaultValue: false
      .}: bool
  ...
  ValidatorClientConf* = object
    ...
    doppelgangerDetection* {.
      ...
      desc:
        "If enabled, the validator client prudently listens for 2 epochs " &
    ...
    suggestedGasLimit* {.
      desc: "Suggested gas limit", defaultValue: 30_000_000, name: "suggested-gas-limit"
    .}: uint64
    ...
    keymanagerTokenFile* {.
      desc:
        "A file specifying the authorizition token required for accessing the keymanager API",
      name: "keymanager-token-file"
    .}: Option[InputFile]

    metricsEnabled* {.
      desc: "Enable the metrics server (BETA)", defaultValue: false, name: "metrics"
    .}: bool
  ...
  SigningNodeConf* = object
    ...
    expectedFeeRecipient* {.
      desc:
        "Signatures for blocks will require proofs of the specified " & "fee recipient",
      name: "expected-fee-recipient"
    .}: Option[Address]
    ...
    requestTimeout* {.
      desc:
        "Request timeout, maximum time that node will wait for remote " &
    ...
    tlsCertificate* {.desc: "Path to SSL certificate file", name: "tls-cert".}:
      Option[InputFile]

    tlsPrivateKey* {.desc: "Path to SSL ceritificate's private key", name: "tls-key".}:
      Option[InputFile]
...
func parseCmdArg*(T: type GraffitiBytes, input: string): T {.raises: [ValueError].} =
...
func parseCmdArg*(
    T: type BlockHashOrNumber, input: string
): T {.raises: [ValueError].} =
...
func parseCmdArg*(T: type Uri, input: string): T {.raises: [ValueError].} =
  parseUri(input)

func completeCmdArg*(T: type Uri, input: string): seq[string] =
  return @[]

func parseCmdArg*(T: type PubKey0x, input: string): T {.raises: [ValueError].} =
  PubKey0x(hexToPaddedByteArray[RawPubKeySize](input))

func parseCmdArg*(T: type ValidatorPubKey, input: string): T {.raises: [ValueError].} =
  let res = ValidatorPubKey.fromHex(input)
  if res.isErr():
    raise (ref ValueError)(msg: $res.error())
  res.get()

func completeCmdArg*(T: type PubKey0x, input: string): seq[string] =
  return @[]

func parseCmdArg*(T: type Checkpoint, input: string): T {.raises: [ValueError].} =
...
func parseCmdArg*(T: type Epoch, input: string): T {.raises: [ValueError].} =
...
func parseCmdArg*(T: type WalletName, input: string): T {.raises: [ValueError].} =
  if input.len == 0:
    raise newException(ValueError, "The wallet name should not be empty")
  if input[0] == '_':
    raise
      newException(ValueError, "The wallet name should not start with an underscore")
  for rune in runes(input):
    if not rune.isPrintable:
      raise newException(
        ValueError, "The wallet name should consist only of printable characters"
      )
...
func outWalletName*(config: BeaconNodeConf): Option[WalletName] =
  ...
  of wallets:
    ...
    of WalletsCmd.create:
      config.createdWalletNameFlag
    of WalletsCmd.restore:
      config.restoredWalletNameFlag
    of WalletsCmd.list:
      fail()
  of deposits:
    case config.depositsCmd
    of DepositsCmd.createTestnetDeposits:
      config.newWalletNameFlag
    else:
      fail()
  else:
    fail()
...
func outWalletFile*(config: BeaconNodeConf): Option[OutFile] =
  ...
  of wallets:
    case config.walletsCmd
    of WalletsCmd.create:
      config.createdWalletFileFlag
    of WalletsCmd.restore:
      config.restoredWalletFileFlag
    of WalletsCmd.list:
      fail()
  of deposits:
    case config.depositsCmd
    of DepositsCmd.createTestnetDeposits:
      config.newWalletFileFlag
    else:
      fail()
  else:
    fail()
...
func runAsService*(config: BeaconNodeConf): bool =
  case config.cmd
  of noCommand: config.runAsServiceFlag
  else: false
...
func web3SignerUrls*(conf: AnyConf): seq[Web3SignerUrl] =
  ...
  for url in conf.verifyingWeb3Signers:
    result.add Web3SignerUrl(
      url: url, provenBlockProperties: conf.provenBlockProperties
    )
...
proc readValue*(
    r: var TomlReader, value: var Epoch
) {.raises: [SerializationError, IOError].} =
  value = Epoch r.parseInt(uint64)

proc readValue*(
    r: var TomlReader, value: var GraffitiBytes
) {.raises: [SerializationError, IOError].} =
  try:
    value = GraffitiBytes.init(r.readValue(string))
  except ValueError:
    r.raiseUnexpectedValue(
      "A printable string or 0x-prefixed hex-encoded raw bytes expected"
    )
...
proc readValue*(
    r: var TomlReader, a: var Eth2Digest
...
proc readValue*(
    reader: var TomlReader, value: var ValidatorPubKey
) {.raises: [SerializationError].} =
  let keyAsString =
    try:
...
proc readValue*(r: var TomlReader, a: var PubKey0x) {.raises: [SerializationError].} =
  try:
    a = parseCmdArg(PubKey0x, r.readValue(string))
  except CatchableError:
    r.raiseUnexpectedValue("a 0x-prefixed hex-encoded string expected")

proc readValue*(r: var TomlReader, a: var WalletName) {.raises: [SerializationError].} =
  try:
    a = parseCmdArg(WalletName, r.readValue(string))
  except CatchableError:
    r.raiseUnexpectedValue("string expected")

proc readValue*(r: var TomlReader, a: var Address) {.raises: [SerializationError].} =
  try:
...
proc loadJwtSecret(
  ...
  if jwtSecret.isSome or allowCreate:
    ...
    if secret.isErr:
      fatal "Specified a JWT secret file which couldn't be loaded", err = secret.error
      quit 1
...
proc loadJwtSecret*(
    rng: var HmacDrbgContext, config: BeaconNodeConf, allowCreate: bool
): Opt[seq[byte]] =
  rng.loadJwtSecret(
...
proc engineApiUrls*(config: BeaconNodeConf): seq[EngineApiUrl] =
  ...
  (elUrls & config.web3Urls).toFinalEngineApiUrls(config.jwtSecret.configJwtSecretOpt)

proc loadKzgTrustedSetup*(): Result[void, string] =
  const
    vendorDir = currentSourcePath.parentDir.replace('\\', '/') & "/../vendor"
    trustedSetup =
      staticRead(vendorDir & "/nim-kzg4844/kzg4844/csources/src/trusted_setup.txt")
