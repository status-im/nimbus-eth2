...
export
  ...
  network_metadata, el_conf, network, confTomlDefs, confTomlNet, confTomlUri, jsnet,
...
type
  ...
  BeaconNodeConf* = object
    ...
    web3ForcePolling* {.hidden, name: "web3-force-polling".}: Option[bool]
    ...
    case cmd* {.command, defaultValue: BNStartUpCmd.beaconNode.}: BNStartUpCmd
    of BNStartUpCmd.beaconNode:
      ...
      finalizedDepositTreeSnapshot* {.hidden, name: "finalized-deposit-tree-snapshot".}:
        Option[InputFile]
      ...
      longRangeSync* {.
        ...
        name: "debug-long-range-sync"
      .}: LongRangeSyncMode

      inProcessValidators* {.
        hidden, desc: "Deprecated for removal", name: "in-process-validators"
      .}: Option[bool]
      ...
      invalidBlockRoots* {.
        hidden,
        desc:
          "List of beacon block roots that, if the EL responds with SYNCING/ACCEPTED, are treated as if their execution payload was INVALID",
        name: "debug-invalidate-block-root"
      .}: seq[Eth2Digest]
      ...
      syncHorizon* {.
        hidden,
        desc:
          "Number of empty slots to process before considering the client out of sync. Defaults to the number of slots in 10 minutes",
        name: "sync-horizon"
      .}: Option[uint64]
...
template loadEth2Network*(
    config: BeaconNodeConf | SigningNodeConf
): Eth2NetworkMetadata =
...
func runAsService*(config: BeaconNodeConf): bool =
  case config.cmd
  of BNStartUpCmd.beaconNode: config.runAsServiceFlag
  else: false
...
proc readValue*(
    r: var TomlReader, a: var Eth1Address
) {.raises: [SerializationError].} =
  try:
...
proc loadJwtSecret*(
    rng: var HmacDrbgContext, config: auto, allowCreate: bool
): Opt[seq[byte]] =
  rng.loadJwtSecret(
