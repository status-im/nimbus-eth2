...
proc initLegacyLightClientHeadersStore(
...
): KvResult[LegacyLightClientHeadersStore] =
  ...
  ok LegacyLightClientHeadersStore(getStmt: getStmt, putStmt: putStmt)
...
proc initLightClientHeadersStore(
...
): KvResult[LightClientHeadersStore] =
  if not backend.readOnly:
    ?backend.exec(
      """
      CREATE TABLE IF NOT EXISTS `""" & name & """` (
  ...
  ok LightClientHeadersStore(getStmt: getStmt, putStmt: putStmt)

func close(store: var LightClientHeadersStore) =
  store.getStmt.disposeSafe()
  store.putStmt.disposeSafe()

proc getLatestFinalizedHeader*(db: LightClientDB): ForkedLightClientHeader =
      ...
      withAll(LightClientDataFork):
        when lcDataFork > LightClientDataFork.None:
          if header[0] == ord(lcDataFork).int64:
            return ForkedLightClientHeader.init(
              SSZ.decode(header[1], lcDataFork.LightClientHeader)
            )
      warn "Unsupported LC store kind", store = "headers", key, kind = header[0]
      return default(ForkedLightClientHeader)
    except SerializationError as exc:
      error "LC store corrupted",
        store = "headers", key, kind = header[0], exc = exc.msg
      return default(ForkedLightClientHeader)
...
func putLatestFinalizedHeader*(db: LightClientDB, header: ForkedLightClientHeader) =
    ...
    when lcDataFork > LightClientDataFork.None:
      block:
        ...
        when lcDataFork == LightClientDataFork.Altair:
          let res = db.legacyHeaders.putStmt.exec((key.int64, SSZ.encode(forkyHeader)))
          res.expect("SQL query OK")
        else:
      ...
      raiseAssert "Cannot store empty `LightClientHeader`"
...
func initSyncCommitteesStore(
  ...
  ok SyncCommitteeStore(getStmt: getStmt, putStmt: putStmt, keepFromStmt: keepFromStmt)
...
proc getSyncCommittee*(
  ...
  for res in db.syncCommittees.getStmt.exec(period.int64, syncCommittee):
    ...
    try:
      ...
      error "LC store corrupted", store = "syncCommittees", period, exc = exc.msg
      return Opt.none(altair.SyncCommittee)

func putSyncCommittee*(
    db: LightClientDB, period: SyncCommitteePeriod, syncCommittee: altair.SyncCommittee
) =
  doAssert not db.backend.readOnly # All `stmt` are non-nil
  doAssert period.isSupportedBySQLite
...
proc initLightClientDB*(
    backend: SqStoreRef, names: LightClientDBNames
): KvResult[LightClientDB] =
  let
    legacyHeaders =
      ?backend.initLegacyLightClientHeadersStore(names.legacyAltairHeaders)
    headers =
      ?backend.initLightClientHeadersStore(names.headers, names.legacyAltairHeaders)
