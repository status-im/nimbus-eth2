...
export toml_serialization, confTomlDefs, confTomlNet, confTomlUri
...
proc readValue*(
    reader: var TomlReader, value: var EngineApiRoles
...
proc writeValue*(writer: var JsonWriter, roles: EngineApiRoles) {.raises: [IOError].} =
...
proc parseCmdArg*(
    T: type EngineApiUrlConfigValue, input: string
): T {.raises: [ValueError].} =
  var
    uri = parseUri(input)
  ...
  if uri.anchor != "":
    for key, value in decodeQuery(uri.anchor):
      ...
      of "roles":
        ...
        raise newException(
          ValueError, "'" & key & "' is not a recognized Engine URL property"
        )
    uri.anchor = ""

  EngineApiUrlConfigValue(
    url: $uri, jwtSecret: jwtSecret, jwtSecretFile: jwtSecretFile, roles: roles
  )

proc readValue*(
    reader: var TomlReader, value: var EngineApiUrlConfigValue
) {.raises: [SerializationError, IOError].} =
  if reader.lex.readable and reader.lex.peekChar in ['\'', '"']:
    # If the input is a string, we'll reuse the command-line parsing logic
    value =
      try:
        parseCmdArg(EngineApiUrlConfigValue, reader.readValue(string))
...
proc fixupWeb3Urls*(web3Url: var string) =
  ...
  if not (
    normalizedUrl.startsWith("https://") or normalizedUrl.startsWith("http://") or
    normalizedUrl.startsWith("wss://") or normalizedUrl.startsWith("ws://")
  ):
    warn "The Web3 URL does not specify a protocol. Assuming a WebSocket server",
      web3Url
    web3Url = "ws://" & web3Url
...
proc toFinalUrl*(
    confValue: EngineApiUrlConfigValue, confJwtSecret: Opt[seq[byte]]
): Result[EngineApiUrl, cstring] =
  if confValue.jwtSecret.isSome and confValue.jwtSecretFile.isSome:
    return
      err "The options `jwtSecret` and `jwtSecretFile` should not be specified together"

  let jwtSecret =
    if confValue.jwtSecret.isSome:
  ...
  ok EngineApiUrl.init(
    url = url, jwtSecret = jwtSecret, roles = confValue.roles.get(defaultEngineApiRoles)
  )
...
proc toFinalEngineApiUrls*(
    elUrls: seq[EngineApiUrlConfigValue], confJwtSecret: Opt[InputFile]
): seq[EngineApiUrl] =
