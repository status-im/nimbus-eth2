...
const contractCode = hexToSeqByte staticRead "deposit_contract_code.txt"
...
proc sendDeposits*(
  ...
  let depositContract =
    web3.contractSender(DepositContract, Address depositContractAddress)
...
proc main() {.async.} =
  ...
  if conf.cmd == StartUpCommand.generateSimulationDeposits:
    let
      ...
      threshold = if conf.remoteSignersUrls.len > 0: conf.threshold else: 0

    if conf.remoteValidatorsCount > 0 and conf.remoteSignersUrls.len == 0:
