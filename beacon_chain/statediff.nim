...
import stew/assign2, ./spec/forks

func diffModIncEpoch[maxLength, T](
    hl: HashArray[maxLength, T], startSlot: uint64
): array[SLOTS_PER_EPOCH, T] =
  static:
    doAssert maxLength.uint64 mod SLOTS_PER_EPOCH == 0
...
func applyModIncrement[maxLength, T](
    ha: var HashArray[maxLength, T], hl: array[SLOTS_PER_EPOCH, T], slot: uint64
) =
  var indexSlot = slot
...
func applyValidatorIdentities(
    validators: var HashList[Validator, Limit VALIDATOR_REGISTRY_LIMIT], hl: auto
) =
  for item in hl:
...
func setValidatorStatusesNoWithdrawals(
  ...
  for i in 0 ..< hl.len:
    ...
    validator[].activation_eligibility_epoch = hl[i].activation_eligibility_epoch
...
func replaceOrAddDecodeEth1Votes[T, maxLength](
    votes0: var HashList[T, maxLength],
...
func getMutableValidatorStatuses(
  ...
  for i in 0 ..< state.validators.len:
    ...
    assign(
      result[i].activation_eligibility_epoch, validator.activation_eligibility_epoch
    )
  ...
  for i in 0 ..< state.validators.lenu64:
    if state.validators.item(i).has_eth1_withdrawal_credential and
      ...
      if not res.add IndexedWithdrawalCredentials(
        validator_index: i,
        withdrawal_credentials: state.validators.item(i).withdrawal_credentials,
      ):
        raiseAssert "same limit as validators"
...
func diffStates*(
  ...
  let
    ...
    (eth1_data_votes_replaced, eth1_data_votes) = replaceOrAddEncodeEth1Votes(
      state0.eth1_data_votes_recent, state0.eth1_data_votes_len, state1.eth1_data_votes
    )
  ...
  BeaconStateDiff(
    ...
    withdrawal_credential_changes: getValidatorWithdrawalChanges(state0, state1),
    balances: state1.balances.data,

    # RANDAO mixes gets updated every block, in place
    randao_mix: state1.randao_mixes[
      state0.slot.epoch.uint64 mod EPOCHS_PER_HISTORICAL_VECTOR.uint64
    ],
    slashing:
      state1.slashings[state0.slot.epoch.uint64 mod EPOCHS_PER_HISTORICAL_VECTOR.uint64],
...
func getBeaconStateDiffSummary*(
    state0: capella.BeaconState
  ...
  BeaconStateDiffPreSnapshot(
    eth1_data_votes_recent:
      if state0.eth1_data_votes.len > 0:
        ...
        # https://github.com/ethereum/consensus-specs/blob/v1.5.0-alpha.8/specs/phase0/beacon-chain.md#eth1-data
...
func applyDiff*(
  ...
  replaceOrAddDecodeEth1Votes(
    state.eth1_data_votes, stateDiff.eth1_data_votes_replaced, stateDiff.eth1_data_votes
  )
  ...
  setValidatorStatusesNoWithdrawals(state.validators, stateDiff.validator_statuses)
  for withdrawalUpdate in stateDiff.withdrawal_credential_changes:
    assign(
      state.validators.mitem(withdrawalUpdate.validator_index).withdrawal_credentials,
  ...
  # >=1 value from it
  let epochIndex = state.slot.epoch.uint64 mod EPOCHS_PER_HISTORICAL_VECTOR.uint64
  ...
  assign(state.previous_epoch_participation, stateDiff.previous_epoch_participation)
  assign(state.current_epoch_participation, stateDiff.current_epoch_participation)

  state.justification_bits = stateDiff.justification_bits
  assign(state.previous_justified_checkpoint, stateDiff.previous_justified_checkpoint)
  assign(state.current_justified_checkpoint, stateDiff.current_justified_checkpoint)
  ...
  assign(
    state.latest_execution_payload_header, stateDiff.latest_execution_payload_header
  )

  assign(state.next_withdrawal_index, stateDiff.next_withdrawal_index)
  assign(
    state.next_withdrawal_validator_index, stateDiff.next_withdrawal_validator_index
  )
