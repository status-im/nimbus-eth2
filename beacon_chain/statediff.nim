...
func diffModIncEpoch[maxLen, T](
    hl: HashArray[maxLen, T], startSlot: uint64
): array[SLOTS_PER_EPOCH, T] =
  static:
    doAssert maxLen.uint64 mod SLOTS_PER_EPOCH == 0
...
func applyDiff*(
  ...
  template assign[T, maxLen](tgt: var HashList[T, maxLen], src: List[T, maxLen]) =
    assign(tgt.data, src)
    tgt.resetCache()
