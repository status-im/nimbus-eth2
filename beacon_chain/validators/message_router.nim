...
proc routeAttestation*(
  ...
  let
    ...
    shufflingRef = router[].dag.getShufflingRef(
      ...
      return
    committee_index = shufflingRef.get_committee_index(attestation.data.index).valueOr:
  ...
  return await router.routeAttestation(attestation, subnet_id, checkSignature = true)

proc routeSignedAggregateAndProof*(
    router: ref MessageRouter, proof: SignedAggregateAndProof, checkSignature = true
): Future[SendResult] {.async.} =
  ...
  block:
    ...
    let res = await router[].processor.processSignedAggregateAndProof(
      MsgSource.api, proof, checkSignature = checkSignature, checkCover = false
    )
    if not res.isGoodForSending:
