...
proc unblindAndRouteBlockMEV*(
  ...
  if bundle.status != httpOk:
    ...
    return err(
      "submitBlindedBlock failed with HTTP error code " & $bundle.status & ": " &
        $shortLog(blindedBlock)
    )

  when consensusFork >= ConsensusFork.Deneb:
    template execution_payload(): untyped =
      bundle.data.data.execution_payload
  ...
  var signedBlock = consensusFork.SignedBeaconBlock(signature: blindedBlock.signature)
  copyFields(
    signedBlock.message,
  ...
  let newBlockRef = (await node.router.routeSignedBeaconBlock(signedBlock, blobsOpt)).valueOr:
