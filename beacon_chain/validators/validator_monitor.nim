...
declareCounter validator_monitor_block_hit,
  "Number of times a block proposed by the validator was included an epoch later",
  labels = ["validator"]
declareCounter validator_monitor_block_miss,
  "Number of times the validator was expected to propose a block but no block was included",
  labels = ["validator"]
...
func init*(
    T: type ValidatorMonitor, cfg: RuntimeConfig, autoRegister = false, totals = false
): T =
...
proc registerAttestation*(
  ...
  let
    ...
    delay =
      seen_timestamp - slot.attestation_deadline(self.cfg.timeParams, consensusFork)
...
proc registerAggregate*(
  ...
  let
    ...
    consensusFork = self.cfg.consensusForkAtEpoch(slot.epoch)
    delay = seen_timestamp - slot.aggregate_deadline(self.cfg.timeParams, consensusFork)
...
proc registerSyncCommitteeMessage*(
  ...
  self.withMonitor(sync_committee_message.validator_index):
    let
      ...
      delay =
        seen_timestamp -
        slot.sync_committee_message_deadline(self.cfg.timeParams, consensusFork)
...
proc registerSyncContribution*(
  ...
  let
    ...
    delay =
      seen_timestamp -
      slot.sync_contribution_deadline(self.cfg.timeParams, consensusFork)
