...
import
  ...
  # Local modules
  ../spec/[
    eth2_merkleization, forks, helpers, network, peerdas_helpers, signatures,
    state_transition, state_transition_block, validator,
  ],
...
func getValidator*(validators: auto, pubkey: ValidatorPubKey): Opt[ValidatorAndIndex] =
...
func blockConsensusValue(r: BlockRewards): UInt256 {.noinit.} =
  ...
  u256(r.attestations + r.sync_aggregate + r.proposer_slashings + r.attester_slashings) *
    u256(1000000000)
...
proc addValidatorsFromWeb3Signer(
  ...
  let dynamicStores =
    ...
    (await queryValidatorsSource(web3signerUrl)).valueOr(default(seq[KeystoreData]))
...
proc pollForDynamicValidators*(
  ...
  var timeout = seconds(intervalInSeconds)
...
proc getGraffitiBytes*(node: BeaconNode, validator: AttachedValidator): GraffitiBytes =
  getGraffiti(
    node.config.validatorsDir, node.config.defaultGraffitiBytes(), validator.pubkey
  )
...
proc isSynced*(node: BeaconNode, head: BlockRef): bool =
  ...
  #      else to do it
  not wallSlot.afterGenesis or head.slot + node.config.syncHorizon.get >= wallSlot.slot
...
proc createAndSendAttestation(
  ...
  if node.dag.cfg.consensusForkAtEpoch(epoch) >= ConsensusFork.Electra:
    discard await node.router.routeAttestation(
      registered.toSingleAttestation(signature),
      subnet_id,
    ...
    msg = block:
      ...
      if res.isErr():
        warn "Unable to sign committee message",
          validator = shortLog(validator), slot = slot, block_root = shortLog(head.root)
        return
      res.get()
...
proc sendSyncCommitteeContributions(node: BeaconNode, head: BlockRef, slot: Slot) =
  let syncCommittee = node.dag.syncCommitteeParticipants(slot + 1)
  ...
  if node.dag.cfg.consensusForkAtEpoch(slot.epoch) >= ConsensusFork.Electra:
    ...
    var msg = electra.SignedAggregateAndProof(
      message: electra.AggregateAndProof(
        aggregator_index: distinctBase validator_index, selection_proof: selectionProof
      )
    )
...
proc sendAggregatedAttestations(node: BeaconNode, head: BlockRef, slot: Slot) =
...
): Future[Result[SignedValidatorRegistrationV1, string]] {.
    async: (raises: [CancelledError])
.} =
  ...
  validatorRegistration.signature = ?await validator.getBuilderSignature(
...
proc registerValidatorsPerBuilder(
  ...
  let payloadBuilderClient = block:
    let
      flags = {RestClientFlag.CommaSeparatedArray, RestClientFlag.ResolveAlways}
      socketFlags = {SocketFlags.TcpNoDelay}
    RestClientRef.new(payloadBuilderAddress, flags = flags, socketFlags = socketFlags).valueOr:
  ...
  for key in attachedValidatorPubkeys:
    ...
    if validator.externalBuilderRegistration.isSome:
      ...
      let validatorRegistration = await node.getValidatorRegistration(validator, epoch)
      if validatorRegistration.isErr:
        error "registerValidators: validatorRegistration failed", validatorRegistration
        continue
...
proc registerValidators*(
  ...
  for pubkey in node.attachedValidators[].validators.keys:
    ...
    builderKeys.mgetOrPut(payloadBuilderAddress, default(seq[ValidatorPubKey])).add pubkey
...
proc handleValidatorDuties*(
    node: BeaconNode, lastSlot, slot: Slot
) {.async: (raises: [CancelledError]).} =
  ...
  if attestationCutoff.inFuture:
    ...
    if await node.consensusManager[].expectBlock(slot).withTimeout(
      ...
      await waitAfterBlockCutoff(node.beaconClock, slot, consensusFork, Opt.some(head))
  ...
  doAssert(
    node.dag.timeParams.aggregateSlotOffset ==
      node.dag.timeParams.syncContributionSlotOffset,
    "Timing change?",
  )
      ...
      for slot in epoch.slots():
        for validator_index in get_ptc(forkyState.data, slot, cache):
          if validator_index in validatorIndices:
            node.consensusManager[].actionTracker.registerPTCDuty(slot, validator_index)

            debug "PTC duty registered", slot = slot, epoch = epoch
