...
type
  ForkedBlockResult = Result[
    tuple[blck: ForkedBeaconBlock, blockValue: Wei, blobsBundleOpt: Opt[BlobsBundle]],
    string,
  ]
...
proc getValidator*(validators: auto, pubkey: ValidatorPubKey): Opt[ValidatorAndIndex] =
  ...
  if idx == -1:
    ...
    Opt.some ValidatorAndIndex(index: ValidatorIndex(idx), validator: validators[idx])
...
proc addValidators*(node: BeaconNode) =
  info "Loading validators",
    validatorsDir = node.config.validatorsDir(),
  ...
  try:
    ...
    waitFor allFutures(
      mapIt(node.config.web3SignerUrls, node.addValidatorsFromWeb3Signer(it, epoch))
    )
...
proc pollForDynamicValidators*(
    node: BeaconNode, web3signerUrl: Web3SignerUrl, intervalInSeconds: int
) {.async.} =
  ...
  proc addValidatorProc(keystore: KeystoreData) =
    let
      ...
      feeRecipient =
        node.consensusManager[].getFeeRecipient(keystore.pubkey, index, epoch)
      gasLimit = node.consensusManager[].getGasLimit(keystore.pubkey)
    discard node.attachedValidators[].addValidator(keystore, feeRecipient, gasLimit)
        ...
        timeout = block:
          ...
          if res.isOk():
            ...
            debug "Validators source has been polled for validators",
              keystores_found = len(keystores), web3signer_url = web3signerUrl.url
            node.attachedValidators.updateDynamicValidators(
              web3signerUrl, keystores, addValidatorProc
            )
...
proc getValidatorForDuties*(
    node: BeaconNode, idx: ValidatorIndex, slot: Slot, slashingSafe = false
): Opt[AttachedValidator] =
  let key = ?node.dag.validatorKey(idx)

  node.attachedValidators[].getValidatorForDuties(key.toPubKey(), slot, slashingSafe)
...
proc isSynced*(node: BeaconNode, head: BlockRef): bool =
  ...
  #      else to do it
  not wallSlot.afterGenesis or head.slot + node.config.syncHorizon >= wallSlot.slot

proc handleLightClientUpdates*(node: BeaconNode, slot: Slot) {.async.} =
  template pool(): untyped =
    node.lightClientPool[]

  static:
    doAssert lightClientFinalityUpdateSlotOffset == lightClientOptimisticUpdateSlotOffset
  let sendTime = node.beaconClock.fromNow(slot.light_client_finality_update_time())
    ...
    when lcDataFork > LightClientDataFork.None:
      ...
      if newFinality:
        ...
        if sendResult.isOk:
          ...
          warn "LC finality update failed to send", error = sendResult.error()
      ...
      if attested_slot > pool.latestForwardedOptimisticSlot:
        ...
        let sendResult = await node.network.broadcastLightClientOptimisticUpdate(msg)
        ...
        if sendResult.isOk:
          ...
          warn "LC optimistic update failed to send", error = sendResult.error()

proc createAndSendAttestation(
    node: BeaconNode,
    fork: Fork,
  ...
  try:
    ...
    let res =
      await node.router.routeAttestation(attestation, subnet_id, checkSignature = false)
    if not res.isOk():
      return

    if node.config.dumpEnabled:
      dump(node.config.dumpDirOutgoing, attestation.data, registered.validator.pubkey)
  except CatchableError as exc:
...
proc getBlockProposalEth1Data*(
    node: BeaconNode, state: ForkedHashedBeaconState
...
proc getGasLimit(node: BeaconNode, pubkey: ValidatorPubKey): uint64 =
...
proc getExecutionPayload(
  ...
  try:
    ...
    warn "Error creating non-empty execution payload", msg = exc.msg
    return Opt.none PayloadType
...
proc makeBeaconBlockForHeadAndSlot*(
  ...
  let blck = makeBeaconBlock(
    ...
    beacon_block_production_errors.inc()
    warn "Cannot create block for proposal", slot, head = shortLog(head), error
    $error
...
proc getBlindedExecutionPayload[
    EPH:
      capella.ExecutionPayloadHeader | deneb_mev.BlindedExecutionPayloadAndBlobsBundle
](
    node: BeaconNode,
  ...
  if blindedHeader.status != httpOk:
    ...
    if not verify_builder_signature(
      ...
      blindedHeader.data.data.message.pubkey, blindedHeader.data.data.signature,
    ):
    ...
    when EPH is capella.ExecutionPayloadHeader:
      ...
      template builderBid(): untyped =
        blindedHeader.data.data.message
...
from ./message_router_mev import copyFields, getFieldNames, unblindAndRouteBlockMEV

func constructSignableBlindedBlock[T: capella_mev.SignedBlindedBeaconBlock](
    blck: capella.BeaconBlock, executionPayloadHeader: capella.ExecutionPayloadHeader
): T =
  ...
  assign(blindedBlock.message.body.execution_payload_header, executionPayloadHeader)
...
proc constructSignableBlindedBlock[T: deneb_mev.SignedBlindedBeaconBlock](
  ...
  assign(
    blindedBlock.message.body.blob_kzg_commitments, blindedBundle.blob_kzg_commitments
  )

  blindedBlock

func constructPlainBlindedBlock[T: capella_mev.BlindedBeaconBlock](
    blck: ForkyBeaconBlock, executionPayloadHeader: capella.ExecutionPayloadHeader
): T =
  # https://github.com/nim-lang/Nim/issues/23020 workaround
  static:
    doAssert T is capella_mev.BlindedBeaconBlock
...
func constructPlainBlindedBlock[T: deneb_mev.BlindedBeaconBlock](
  ...
  # https://github.com/nim-lang/Nim/issues/23020 workaround
  static:
    doAssert T is deneb_mev.BlindedBeaconBlock
  ...
  copyFields(blindedBlock.body, blck.body, blckBodyFields)
  assign(
    blindedBlock.body.execution_payload_header, blindedBundle.execution_payload_header
  )
  assign(blindedBlock.body.blob_kzg_commitments, blindedBundle.blob_kzg_commitments)

  blindedBlock

proc blindedBlockCheckSlashingAndSign[
    T: capella_mev.SignedBlindedBeaconBlock | deneb_mev.SignedBlindedBeaconBlock
](
    node: BeaconNode,
    slot: Slot,
    validator: AttachedValidator,
    validator_index: ValidatorIndex,
    nonsignedBlindedBlock: T,
  ...
  let
    ...
    signingRoot =
      compute_block_signing_root(fork, genesis_validators_root, slot, blockRoot)
    notSlashable = node.attachedValidators.slashingProtection.registerBlock(
      validator_index, validator.pubkey, slot, signingRoot
    )

  if notSlashable.isErr:
    warn "Slashing protection activated for MEV block",
      blockRoot = shortLog(blockRoot),
      blck = shortLog(nonsignedBlindedBlock),
  ...
  withBlck(forkedBlock):
    when consensusFork >= ConsensusFork.Capella:
      when not (
        ...
        return ok constructSignableBlindedBlock[T](forkyBlck, executionPayloadHeader)
    else:
      return err(
        "getUnsignedBlindedBeaconBlock: attempt to construct pre-Capella blinded block"
      )
...
proc getBlindedBlockParts[
  ...
  let
    ...
    executionPayloadHeader =
      try:
        ...
        BlindedBlockResult[EPH].err("getBlindedExecutionPayload error: " & exc.msg)

  if executionPayloadHeader.isErr:
    warn "Could not obtain blinded execution payload header",
      error = executionPayloadHeader.error, slot, validator_index, head = shortLog(head)
  ...
  when EPH is capella.ExecutionPayloadHeader:
    ...
    let
      ...
      kzg_commitments =
        Opt.some(executionPayloadHeader.get.blindedBlckPart.blob_kzg_commitments)
  ...
  return ok(
    (
      executionPayloadHeader.get.blindedBlckPart, executionPayloadHeader.get.blockValue,
      forkedBlck.blck,
    )
  )

proc getBuilderBid[
    SBBB: capella_mev.SignedBlindedBeaconBlock | deneb_mev.SignedBlindedBeaconBlock
](
    node: BeaconNode,
    payloadBuilderClient: RestClientRef,
    head: BlockRef,
  ...
  let blindedBlockParts = await getBlindedBlockParts[EPH](
    node, payloadBuilderClient, head, validator_pubkey, slot, randao, validator_index,
    node.graffitiBytes,
  )
...
proc proposeBlockMEV(
    ...
    payloadBuilderClient: RestClientRef,
    blindedBlock:
      capella_mev.SignedBlindedBeaconBlock | deneb_mev.SignedBlindedBeaconBlock,
): Future[Result[BlockRef, string]] {.async.} =
  let unblindedBlockRef =
...
proc makeBlindedBeaconBlockForHeadAndSlot*[BBB: ForkyBlindedBeaconBlock](
  ...
  let
    pubkey =
      ...
      withState(node.dag.headState):
        if node.dag.cfg.isEFMainnet and
    ...
    blindedBlockParts = await getBlindedBlockParts[EPH](
      node, payloadBuilderClient, head, pubkey, slot, randao_reveal, validator_index,
      graffiti,
    )
  if blindedBlockParts.isErr:
  ...
  withBlck(forkedBlck):
    when consensusFork >= ConsensusFork.Capella:
      when (
        (
          consensusFork == ConsensusFork.Deneb and
        ...
        return ok (
          constructPlainBlindedBlock[BBB](forkyBlck, executionPayloadHeader), bidValue
        )
      else:
        return
          err("makeBlindedBeaconBlockForHeadAndSlot: mismatched block/payload types")
    else:
...
proc collectBidFutures(
    ...
    slot: Slot,
    randao: ValidatorSig,
): Future[BlockProposalBidFutures[SBBB]] {.async.} =
  let usePayloadBuilder =
    if not payloadBuilderClient.isNil:
      withState(node.dag.headState):
        ...
        (not node.dag.cfg.isEFMainnet) or (
    ...
    payloadBuilderBidFut =
      if usePayloadBuilder:
        when not (EPS is bellatrix.ExecutionPayloadForSigning):
          ...
          fut.complete(
            BlindedBlockResult[SBBB].err("Bellatrix Builder API unsupported")
          )
          fut
      else:
        let fut = newFuture[BlindedBlockResult[SBBB]]("builder-bid")
        fut.complete(
          BlindedBlockResult[SBBB].err(
...
proc proposeBlockAux(
  ...
  let payloadBuilderClientMaybe =
    node.getPayloadBuilderClient(validator_index.distinctBase)
  ...
  let useBuilderBlock =
    if collectedBids.builderBidAvailable:
      (not collectedBids.engineBidAvailable) or
        builderBetterBid(
  ...
  if collectedBids.engineBidAvailable:
    ...
    if collectedBids.builderBidAvailable:
      info "Compared engine and builder block bids",
        localBlockValueBoost,
        useBuilderBlock,
        builderBlockValue = collectedBids.payloadBuilderBidFut.read.get().blockValue,
  ...
  if useBuilderBlock:
    let
      blindedBlock = (
        await blindedBlockCheckSlashingAndSign(
          node, slot, validator, validator_index,
          collectedBids.payloadBuilderBidFut.read.get.blindedBlckPart,
        )
      ).valueOr:
        return head
    ...
    let
      ...
      blobsOpt =
        when consensusFork >= ConsensusFork.Deneb:
          ...
          Opt.some(
            signedBlock.create_blob_sidecars(blobsBundle.proofs, blobsBundle.blobs)
          )
        else:
          Opt.none(seq[BlobSidecar])
      newBlockRef = (await node.router.routeSignedBeaconBlock(signedBlock, blobsOpt)).valueOr:
        return head # Errors logged in router
  ...
  return withConsensusFork(node.dag.cfg.consensusForkAtEpoch(slot.epoch)):
    when consensusFork >= ConsensusFork.Capella:
      proposeBlockContinuation(
        consensusFork.SignedBlindedBeaconBlock, consensusFork.ExecutionPayloadForSigning
      )
    else:
...
proc handleAttestations(node: BeaconNode, head: BlockRef, slot: Slot) =
  ...
  if slot + SLOTS_PER_EPOCH < head.slot:
    ...
    warn "Skipping attestation, head is too recent",
      head = shortLog(head), slot = shortLog(slot)
    return
  ...
  if head != attestationHead.blck:
    ...
    notice "Attesting to a state in the past, falling behind?",
      attestationHead = shortLog(attestationHead), head = shortLog(head)
  ...
  let
    epochRef = node.dag.getEpochRef(attestationHead.blck, slot.epoch, false).valueOr:
    ...
    registeredRes = node.attachedValidators.slashingProtection.withContext:
      ...
      for committee_index in get_committee_indices(committees_per_slot):
        let
          committee = get_beacon_committee(epochRef.shufflingRef, slot, committee_index)
          subnet_id =
        ...
        for index_in_committee, validator_index in committee:
          ...
          let
            ...
            registered = registerAttestationInContext(
              validator_index, validator.pubkey, data.source.epoch, data.target.epoch,
              signingRoot,
            )
          if registered.isErr():
...
proc createAndSendSyncCommitteeMessage(
    node: BeaconNode,
    validator: AttachedValidator,
    slot: Slot,
...
proc handleSyncCommitteeMessages(node: BeaconNode, head: BlockRef, slot: Slot) =
  ...
  for subcommitteeIdx in SyncSubcommitteeIndex:
    for valIdx in syncSubcommittee(syncCommittee, subcommitteeIdx):
      let validator = node.getValidatorForDuties(valIdx, slot, slashingSafe = true).valueOr:
        continue
      asyncSpawn createAndSendSyncCommitteeMessage(
        node, validator, slot, subcommitteeIdx, head
      )

proc signAndSendContribution(
    node: BeaconNode,
    validator: AttachedValidator,
    subcommitteeIdx: SyncSubcommitteeIndex,
    head: BlockRef,
  ...
  try:
    ...
    var msg = SignedContributionAndProof(
      message: ContributionAndProof(
        aggregator_index: uint64 validator.index.get, selection_proof: selectionProof
      )
    )

    if not node.syncCommitteeMsgPool[].produceContribution(
      slot, head.bid, subcommitteeIdx, msg.message.contribution
    ):
      return
...
proc handleSyncCommitteeContributions(
  ...
  for subcommitteeIdx in SyncSubcommitteeIndex:
    for valIdx in syncSubcommittee(syncCommittee, subcommitteeIdx):
      ...
      asyncSpawn signAndSendContribution(node, validator, subcommitteeIdx, head, slot)
...
proc signAndSendAggregate(
  ...
  try:
    let
      ...
      selectionProof = block:
        let res = await validator.getSlotSignature(fork, genesis_validators_root, slot)
        if res.isErr():
          warn "Unable to create slot signature",
            validator = shortLog(validator), slot, error = res.error()
          return
        res.get()
    ...
    var msg = SignedAggregateAndProof(
      message: AggregateAndProof(
        aggregator_index: uint64 validator_index, selection_proof: selectionProof
      )
    )
    ...
    # Logged in the router
    discard await node.router.routeSignedAggregateAndProof(msg, checkSignature = false)
  except CatchableError as exc:
    # An error could happen here when the signature task fails - we must
    # not leak the exception because this is an asyncSpawn task
...
proc sendAggregatedAttestations(
  ...
  let
    shufflingRef = node.dag.getShufflingRef(head, slot.epoch, false).valueOr:
      warn "Cannot construct EpochRef for head, report bug", head = shortLog(head), slot
      return
    committees_per_slot = get_committee_count_per_slot(shufflingRef)

  for committee_index in get_committee_indices(committees_per_slot):
    for _, validator_index in get_beacon_committee(shufflingRef, slot, committee_index):
      let validator = node.getValidatorForDuties(validator_index, slot).valueOr:
        continue
...
proc updateValidatorMetrics*(node: BeaconNode) =
  ...
  for _, v in node.attachedValidators[].validators:
    let balance =
      if v.index.isNone():
        0.Gwei
      elif v.index.get().uint64 >= getStateField(node.dag.headState, balances).lenu64:
...
proc registerValidatorsPerBuilder(
    node: BeaconNode,
    payloadBuilderAddress: string,
    epoch: Epoch,
    attachedValidatorPubkeys: seq[ValidatorPubKey],
) {.async.} =
  const
    HttpOk = 200
  ...
  try:
    let payloadBuilderClient = RestClientRef.new(payloadBuilderAddress).valueOr:
      debug "Unable to initialize payload builder client while registering validators",
        payloadBuilderAddress, epoch, err = error
      return
      ...
      withState(node.dag.headState):
        ...
        for i in 0 ..< forkyState.data.validators.len:
          ...
          if pubkey in node.externalBuilderRegistrations and
            ...
            let signedValidatorRegistration = node.externalBuilderRegistrations[pubkey]
    ...
    for key in attachedValidatorPubkeys:
      ...
      # pending validator
      withState(node.dag.headState):
        if distinctBase(validator.index.get) >= forkyState.data.validators.lenu64:
          continue
    ...
    for chunkIdx in 0 ..< validatorRegistrations.len:
      let registerValidatorResult = awaitWithTimeout(
        payloadBuilderClient.registerValidator(validatorRegistrations[chunkIdx]),
    ...
    warn "registerValidators: exception", error = exc.msg
...
proc updateValidators(node: BeaconNode, validators: openArray[Validator]) =
  ...
  for i in node.dutyValidatorCount .. validators.high:
    let v = node.attachedValidators[].getValidator(validators[i].pubkey).valueOr:
      continue
    ...
    if validator.needsUpdate and validator.index.isSome():
      ...
      if index < validators.lenu64:
        validator.updateValidator(
          Opt.some(ValidatorAndIndex(index: index, validator: validators[int index]))
        )
...
proc handleValidatorDuties*(node: BeaconNode, lastSlot, slot: Slot) {.async.} =
  ...
  if attestationCutoff.inFuture:
    debug "Waiting to send attestations",
      head = shortLog(head), attestationCutoff = shortLog(attestationCutoff.offset)

    # Wait either for the block or the attestation cutoff time to arrive
    if await node.consensusManager[].expectBlock(slot).withTimeout(
      attestationCutoff.offset
    ):
  ...
  let aggregateCutoff = node.beaconClock.fromNow(slot.aggregate_deadline())
  ...
  let sendAggregatedAttestationsFut = sendAggregatedAttestations(node, head, slot)
...
proc registerDuties*(node: BeaconNode, wallSlot: Slot) {.async.} =
  ...
  if node.attachedValidators[].count() == 0 or not node.isSynced(node.dag.head) or
      not node.dag.head.executionValid:
    # Nothing to do because we have no validator attached
    return
  ...
  for slot in wallSlot ..< wallSlot + SUBNET_SUBSCRIPTION_LEAD_TIME_SLOTS:
    let shufflingRef = node.dag.getShufflingRef(head, slot.epoch, false).valueOr:
      ...
      for index_in_committee, validator_index in committee:
        let
          ...
          subnet_id =
            compute_subnet_for_attestation(committees_per_slot, slot, committee_index)
          slotSigRes =
            await validator.getSlotSignature(fork, genesis_validators_root, slot)
        if slotSigRes.isErr():
          error "Unable to create slot signature",
            validator = shortLog(validator), error_msg = slotSigRes.error()
          continue
...
proc makeMaybeBlindedBeaconBlockForHeadAndSlotImpl[ResultType](
    ...
    head: BlockRef,
    slot: Slot,
): Future[ResultType] {.async.} =
  let
    proposer = node.dag.getProposer(head, slot).valueOr:
      return ResultType.err("Unable to get proposer for specific head and slot")
    proposerKey = node.dag.validatorKey(proposer).get().toPubKey()

    payloadBuilderClient = node.getPayloadBuilderClient(proposer.distinctBase).valueOr:
      nil
    localBlockValueBoost = node.config.localBlockValueBoost

    collectedBids = await collectBidFutures(
      consensusFork.SignedBlindedBeaconBlock, consensusFork.ExecutionPayloadForSigning,
      node, payloadBuilderClient, proposerKey, proposer, graffiti, head, slot,
      randao_reveal,
    )
    useBuilderBlock =
      if collectedBids.builderBidAvailable:
        ...
        return ResultType.ok(
          (
            blck: consensusFork.MaybeBlindedBeaconBlock(
              isBlinded: true, blindedData: blindedResult.get().blindedBlckPart.message
            ),
  ...
  template forkyBlck(): untyped =
    blockResult.blck.forky(consensusFork)
  ...
  when consensusFork >= ConsensusFork.Deneb:
      ...
      (
        blck: consensusFork.MaybeBlindedBeaconBlock(
          ...
          data: deneb.BlockContents(
            `block`: forkyBlck, kzg_proofs: blobsBundle.proofs, blobs: blobsBundle.blobs
          ),
        ),
    ...
    ResultType.ok(
      (
        blck: consensusFork.MaybeBlindedBeaconBlock(isBlinded: false, data: forkyBlck),
        executionValue: Opt.some(blockResult.blockValue),
        consensusValue: Opt.none(UInt256),
      )
    )
