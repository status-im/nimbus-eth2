...
export keystore, validator_pool, crypto, rand, Web3SignerUrl
...
type
  ...
  ValidatorPubKeyToDataFn* =
    proc(pubkey: ValidatorPubKey): Opt[ValidatorAndIndex] {.raises: [], gcsafe.}

  GetCapellaForkVersionFn* = proc(): Opt[Version] {.raises: [], gcsafe.}
  GetDenebForkEpochFn* = proc(): Opt[Epoch] {.raises: [], gcsafe.}
  GetForkFn* = proc(epoch: Epoch): Opt[Fork] {.raises: [], gcsafe.}
  GetGenesisFn* = proc(): Eth2Digest {.raises: [], gcsafe.}
...
const
  ...
  mostCommonPasswords = wordListArray(
    nimbusSecurityResourcesPath / "passwords" / "10-million-password-list-top-100000.txt",
...
func init(
    T: type KeystoreData, keystore: RemoteKeystore, handle: FileLockHandle
...
func init(
    T: type AddValidatorFailure, status: AddValidatorStatus, msg = ""
): AddValidatorFailure {.raises: [].} =
  AddValidatorFailure(status: status, message: msg)

func toKeystoreKind(kind: ValidatorKind): KeystoreKind {.raises: [].} =
  case kind
  of ValidatorKind.Local: KeystoreKind.Local
  of ValidatorKind.Remote: KeystoreKind.Remote
    ...
    if fileAccessible(dataDir, amask):
      ...
      if cres.isErr():
        fatal "Could not check data folder's ACL",
          path = dataDir, errorCode = $cres.error, errorMsg = ioErrorMsg(cres.error)
        return false
      else:
        if cres.get() == false:
  ...
  when defined(windows):
    ...
    if cres.isErr():
      fatal "Could not check file's ACL",
        key_path = filePath, errorCode = $cres.error, errorMsg = ioErrorMsg(cres.error)
      return false
    else:
      if cres.get() == false:
...
proc keyboardCreatePassword(
    prompt: string, confirm: string, allowEmpty = false
): KsResult[string] =
  while true:
    let password =
      try:
...
proc keyboardGetPassword[T](
    prompt: string,
    attempts: int,
    pred: proc(p: string): KsResult[T] {.gcsafe, raises: [].},
): KsResult[T] =
  var
...
proc loadSecretFile(path: string): KsResult[KeystorePass] {.raises: [].} =
  let res = readAllChars(path)
  if res.isErr():
    return err(ioErrorMsg(res.error()))
  ok(KeystorePass.init(res.get()))

proc loadRemoteKeystoreImpl(validatorsDir, keyName: string): Opt[KeystoreData] =
  let keystorePath = validatorsDir / keyName / RemoteKeystoreFileName

  if not (checkSensitiveFilePermissions(keystorePath)):
    error "Remote keystorage file has insecure permissions", key_path = keystorePath
    return Opt.none(KeystoreData)

  let handle = block:
    let res = openLockedFile(keystorePath)
    if res.isErr():
      error "Unable to lock keystore file",
        key_path = keystorePath, error_msg = ioErrorMsg(res.error())
      return Opt.none(KeystoreData)
    res.get()
  ...
  let keystore = block:
    ...
    if gres.isErr():
      error "Could not read remote keystore file",
        key_path = keystorePath, error_msg = ioErrorMsg(gres.error())
      return Opt.none(KeystoreData)
    let buffer = gres.get()
    let data =
      try:
        ...
        error "Invalid remote keystore file",
          key_path = keystorePath, error_msg = e.formatMsg(keystorePath)
        return Opt.none(KeystoreData)
    let kres = KeystoreData.init(data, handle)
    if kres.isErr():
      error "Invalid remote keystore file",
        key_path = keystorePath, error_msg = kres.error()
      return Opt.none(KeystoreData)
    kres.get()
...
proc loadLocalKeystoreImpl(
    validatorsDir, secretsDir, keyName: string,
    nonInteractive: bool,
  ...
  let keystore = block:
    let gres = handle.getData(MaxKeystoreFileSize)
    if gres.isErr():
      error "Could not read local keystore file",
        key_path = keystorePath, error_msg = ioErrorMsg(gres.error())
      return Opt.none(KeystoreData)
    let buffer = gres.get()
    let data =
      try:
        ...
        error "Invalid local keystore file",
          key_path = keystorePath, error_msg = e.formatMsg(keystorePath)
        return Opt.none(KeystoreData)
    data
  ...
  if fileExists(passphrasePath):
    ...
    let passphrase = block:
      let res = loadSecretFile(passphrasePath)
      if res.isErr():
        error "Failed to read passphrase file",
          error_msg = res.error(), path = passphrasePath
        return Opt.none(KeystoreData)
      res.get()
    ...
    if res.isOk():
      ...
      error "Failed to decrypt keystore",
        key_path = keystorePath, secure_path = passphrasePath
      return Opt.none(KeystoreData)

  if nonInteractive:
    error "Unable to load validator key store. Please ensure matching " &
      "passphrase exists in the secrets dir",
  ...
  let prompt = "Please enter passphrase for key \"" & (validatorsDir / keyName) & "\": "
  let res = keyboardGetPassword[ValidatorPrivKey](
    prompt,
    3,
    proc(password: string): KsResult[ValidatorPrivKey] =
      let decrypted = decryptKeystore(keystore, KeystorePass.init password, cache)
      if decrypted.isErr():
...
proc loadKeystore*(
  ...
  if fileExists(localKeystorePath):
    loadLocalKeystoreImpl(validatorsDir, secretsDir, keyName, nonInteractive, cache)
...
proc removeValidatorFiles*(
    validatorsDir, secretsDir, keyName: string, kind: KeystoreKind
): KmResult[RemoveValidatorStatus] {.raises: [].} =
  let
...
proc existsKeystore(keystoreDir: string, keyKind: KeystoreKind): bool {.raises: [].} =
  case keyKind
  of KeystoreKind.Local:
...
proc existsKeystore(
    keystoreDir: string, keysMask: set[KeystoreKind]
): bool {.raises: [].} =
...
proc queryValidatorsSource*(
  ...
  let
    ...
    prestoFlags = {RestClientFlag.CommaSeparatedArray, RestClientFlag.ResolveAlways}
    socketFlags = {SocketFlags.TcpNoDelay}
    client = block:
      let res = RestClientRef.new(
        $web3signerUrl.url, prestoFlags, httpFlags, socketFlags = socketFlags
      )
      if res.isErr():
    ...
    keys =
      try:
        ...
        if response.status != 200:
          warn "Remote validator's source responded with error", error = response.status
          return QueryResult.err(
            "Remote validator's source responded with error [" & $response.status & "]"
          )

        let res =
          decodeBytes(Web3SignerKeysResponse, response.data, response.contentType)
        if res.isErr():
          warn "Unable to obtain validator's source response", reason = res.error
          return QueryResult.err($res.error)
        res.get()
    ...
    remoteType =
      if web3signerUrl.provenBlockProperties.len == 0:
        RemoteSignerType.Web3Signer
      else:
        RemoteSignerType.VerifyingWeb3Signer

    provenBlockProperties = mapIt(
      web3signerUrl.provenBlockProperties,
      block:
  ...
  for pubkey in keys:
    keystores.add(
      KeystoreData(
        ...
        pubkey: pubkey,
        remotes:
          @[RemoteSignerInfo(url: HttpHostUri(web3signerUrl.url), pubkey: pubkey)],
...
iterator listLoadableKeys*(
    validatorsDir, secretsDir: string, keysMask: set[KeystoreKind]
): CookedPubKey =
      ...
      if kind == pcDir:
        ...
        if not (existsKeystore(keystoreDir, keysMask)):
          notice "Incorrect keystore directory, ignoring",
            reason =
              "Missing keystore files ('keystore.json' or " & "'remote_keystore.json')"
          continue
        ...
        let cres = publicKey.load().valueOr:
          let reason =
            "Directory name should be correct validators public " &
...
iterator listLoadableKeystores*(
    validatorsDir, secretsDir: string,
    nonInteractive: bool,
  ...
  try:
    ...
    for kind, file in walkDir(validatorsDir):
      if kind == pcDir:
        ...
        let keystore = loadKeystore(
          validatorsDir, secretsDir, keyName, nonInteractive, cache
        ).valueOr:
...
iterator listLoadableKeystores*(
    config: AnyConf, cache: KeystoreCacheRef
): KeystoreData =
  for el in listLoadableKeystores(
...
type ValidatorConfigFileStatus* = enum
  noSuchValidator
  noConfigFile
  malformedConfigFile

func validatorKeystoreDir(validatorsDir: string, pubkey: ValidatorPubKey): string =
  validatorsDir / pubkey.fsName

proc checkValidatorKeystoreDir(validatorsDir: string, pubkey: ValidatorPubKey): bool =
  dirExists(validatorsDir.validatorKeystoreDir(pubkey))

func configFilePath*(
    validatorsDir: string, kind: ConfigFileKind, pubkey: ValidatorPubKey
): string =
  case kind
...
proc getSuggestedFeeRecipient*(
    validatorsDir: string, pubkey: ValidatorPubKey, defaultFeeRecipient: Eth1Address
  ...
  try:
    ...
    ok Eth1Address.fromHex(
      strutils.strip(readFile(feeRecipientPath), leading = false, trailing = true)
    )
    ...
    warn "Failed to load fee recipient file; falling back to default fee recipient",
      feeRecipientPath, defaultFeeRecipient, err = exc.msg
    err malformedConfigFile

proc getSuggestedGasLimit*(
    validatorsDir: string, pubkey: ValidatorPubKey, defaultGasLimit: uint64
  ...
  let gasLimitPath = validatorsDir.configFilePath(ConfigFileKind.GasLimitFile, pubkey)
  if not fileExists(gasLimitPath):
    return ok defaultGasLimit
  try:
    ok parseBiggestUInt(
      strutils.strip(readFile(gasLimitPath), leading = false, trailing = true)
    )
  except SerializationError as e:
    warn "Invalid local gas limit file", gasLimitPath, err = e.formatMsg(gasLimitPath)
    err malformedConfigFile
  except CatchableError as exc:
    warn "Failed to load gas limit file; falling back to default gas limit",
      gasLimitPath, defaultGasLimit, err = exc.msg
    err malformedConfigFile

proc getSuggestedGraffiti*(
    validatorsDir: string, pubkey: ValidatorPubKey, defaultGraffitiBytes: GraffitiBytes
  ...
  let graffitiPath = validatorsDir.configFilePath(ConfigFileKind.GraffitiFile, pubkey)
  ...
  try:
    ...
    warn "Invalid local graffiti file", graffitiPath, reason = exc.msg
    return err malformedConfigFile

type BuilderConfig = object
  payloadBuilderEnable: bool
  payloadBuilderUrl: string

proc getBuilderConfig*(
    validatorsDir: string, pubkey: ValidatorPubKey, defaultBuilderAddress: Opt[string]
  ...
  let builderConfig =
    try:
      Json.loadFile(builderConfigPath, BuilderConfig, requireAllFields = true)
    except IOError as err:
      # Any exception must be in the presence of such a file, and therefore
      # an actual error worth logging
      error "Failed to read payload builder configuration",
...
type
  ...
  KeystoreGenerationError* = object
    ...
    of FailedToCreateKeystoreDir, FailedToCreateValidatorsDir, FailedToCreateSecretsDir,
        FailedToCreateSecretFile, FailedToCreateKeystoreFile, DuplicateKeystoreDir,
        DuplicateKeystoreFile:
      error*: string

func mapErrTo*[T, E](
    r: Result[T, E], v: static KeystoreGenerationErrorKind
): Result[T, KeystoreGenerationError] =
  r.mapErr(
    proc(e: E): KeystoreGenerationError =
      KeystoreGenerationError(kind: v, error: $e)
  )

proc loadNetKeystore*(
    keystorePath: string, insecurePwd: Opt[string]
): Opt[lcrypto.PrivateKey] =
  if not (checkSensitiveFilePermissions(keystorePath)):
    error "Network keystorage file has insecure permissions", key_path = keystorePath
    return

  let keyStore =
    try:
      Json.loadFile(
        keystorePath, NetKeystore, requireAllFields = true, allowUnknownFields = true
      )
    except IOError as err:
      error "Failed to read network keystore", err = err.msg, path = keystorePath
      return
    except SerializationError as err:
  ...
  if insecurePwd.isSome():
    ...
    let res = keyboardGetPassword[lcrypto.PrivateKey](
      prompt,
      3,
...
proc saveNetKeystore*(
    rng: var HmacDrbgContext,
    keystorePath: string,
    netKey: lcrypto.PrivateKey,
    insecurePwd: Opt[string],
): Result[void, KeystoreGenerationError] =
  let password =
    if insecurePwd.isSome():
      warn "Using insecure password to lock networking key", key_path = keystorePath
      insecurePwd.get()
    else:
      let prompt = "Please enter NEW password to lock network key storage: "
      let confirm = "Please confirm, network key storage password: "
      ?keyboardCreatePassword(prompt, confirm).mapErrTo(FailedToCreateKeystoreFile)

  let keyStore = createNetKeystore(kdfScrypt, rng, netKey, KeystorePass.init password)
...
proc createLocalValidatorFiles*(
    secretsDir, validatorsDir, keystoreDir, secretFile, passwordAsString, keystoreFile,
...
proc createLockedLocalValidatorFiles(
  ...
  # keystoreFile:
  let lock = ?secureWriteLockedFile(keystoreFile, encodedStorage).mapErrTo(
    FailedToCreateKeystoreFile
  )
  ...
  if dirExists(keystoreDir):
    return err(
      KeystoreGenerationError(
        kind: DuplicateKeystoreDir, error: "Keystore directory already exists"
      )
    )
  if fileExists(keystoreFile):
    return err(
      KeystoreGenerationError(
        kind: DuplicateKeystoreFile, error: "Keystore file already exists"
      )
    )

  let keyStore = createKeystore(
    kdfPbkdf2, rng, signingKey, keypass, signingKeyPath, mode = mode, salt = salt
  )
  let encodedStorage = Json.encode(keyStore)

  ?createLocalValidatorFiles(
    secretsDir,
    validatorsDir,
    keystoreDir,
    secretsDir / keyName,
...
proc saveLockedKeystore(
  ...
  let keyStore =
    createKeystore(kdfPbkdf2, rng, signingKey, keypass, signingKeyPath, mode = mode)

  let encodedStorage = Json.encode(keyStore)

  let lock = ?createLockedLocalValidatorFiles(
    secretsDir,
    validatorsDir,
    keystoreDir,
    secretsDir / keyName,
...
proc saveKeystore(
  ...
  ?createRemoteValidatorFiles(validatorsDir, keystoreDir, keystoreFile, encodedStorage)
  ok()
...
proc saveLockedKeystore(
  ...
  let lock = ?createLockedRemoteValidatorFiles(
    validatorsDir, keystoreDir, keystoreFile, encodedStorage
  )
  ok(lock)

proc saveKeystore*(
    validatorsDir: string, publicKey: ValidatorPubKey, url: HttpHostUri
): Result[void, KeystoreGenerationError] {.raises: [].} =
  let remoteInfo = RemoteSignerInfo(url: url, id: 0)
  saveKeystore(validatorsDir, publicKey, @[remoteInfo], 1)

proc importKeystore*(
    pool: var ValidatorPool, validatorsDir: string, keystore: RemoteKeystore
): ImportResult[KeystoreData] {.raises: [].} =
  let
  ...
  let cookedKey = publicKey.load().valueOr:
    return err(
      AddValidatorFailure.init(
        AddValidatorStatus.failed, "Invalid validator's public key"
      )
    )
  ...
  let res =
    saveLockedKeystore(validatorsDir, publicKey, keystore.remotes, keystore.threshold)
  if res.isErr():
    return err(AddValidatorFailure.init(AddValidatorStatus.failed, $res.error()))
  ok(KeystoreData.init(cookedKey, keystore.remotes, keystore.threshold, res.get()))
...
proc importKeystore*(
  ...
  let res = saveLockedKeystore(
    rng, validatorsDir, secretsDir, privateKey, publicKey, keystore.path, password
  )
...
func validatorKeystoreDir(host: KeymanagerHost, pubkey: ValidatorPubKey): string =
  host.validatorsDir.validatorKeystoreDir(pubkey)

proc checkValidatorKeystoreDir*(host: KeymanagerHost, pubkey: ValidatorPubKey): bool =
  host.validatorsDir.checkValidatorKeystoreDir(pubkey)

proc checkConfigFile*(
    host: KeymanagerHost, kind: ConfigFileKind, pubkey: ValidatorPubKey
): bool =
  fileExists(host.validatorsDir.configFilePath(kind, pubkey))

func feeRecipientPath(host: KeymanagerHost, pubkey: ValidatorPubKey): string =
  host.validatorsDir.configFilePath(ConfigFileKind.FeeRecipientFile, pubkey)

func gasLimitPath(host: KeymanagerHost, pubkey: ValidatorPubKey): string =
  host.validatorsDir.configFilePath(ConfigFileKind.GasLimitFile, pubkey)

func graffitiPath(host: KeymanagerHost, pubkey: ValidatorPubKey): string =
  host.validatorsDir.configFilePath(ConfigFileKind.GraffitiFile, pubkey)

proc removeFeeRecipientFile*(
...
proc setFeeRecipient*(
  ...
  ?secureCreatePath(validatorKeystoreDir).mapErr(
    proc(e: auto): string =
      "Could not create wallet directory [" & validatorKeystoreDir & "]: " & $e
  )

  let res = io2
    .writeFile(validatorKeystoreDir / FeeRecipientFilename, $feeRecipient)
    .mapErr(
      proc(e: auto): string =
        "Failed to write fee recipient file: " & $e
    )
...
proc setGasLimit*(
    host: KeymanagerHost, pubkey: ValidatorPubKey, gasLimit: uint64
): Result[void, string] =
  let validatorKeystoreDir = host.validatorKeystoreDir(pubkey)
  ...
  let res = io2.writeFile(validatorKeystoreDir / GasLimitFilename, $gasLimit).mapErr(
      proc(e: auto): string =
        "Failed to write gas limit file: " & $e
    )
...
proc setGraffiti*(
    host: KeymanagerHost, pubkey: ValidatorPubKey, graffiti: GraffitiBytes
): Result[void, string] =
  let
  ...
  ?secureCreatePath(validatorKeystoreDir).mapErr(
    proc(e: auto): string =
      "Could not create wallet directory [" & validatorKeystoreDir & "], " & "reason: (" &
        $int(e) & ") " & ioErrorMsg(e)
  )

  io2.writeFile(path, to0xHex(distinctBase(graffiti))).mapErr(
    proc(e: auto): string =
      "Failed to write graffiti file," & " reason: (" & $int(e) & ") " & ioErrorMsg(e)
  )
...
proc getValidatorWithdrawalAddress*(
  ...
  if host.getValidatorAndIdxFn.isNil:
    ...
    if validatorAndIndex.isNone:
      ...
      template validator(): auto =
        validatorAndIndex.get.validator
...
func getPerValidatorDefaultFeeRecipient*(
    defaultFeeRecipient: Opt[Eth1Address], withdrawalAddress: Opt[Eth1Address]
): Eth1Address =
...
proc getSuggestedFeeRecipient*(
    host: KeymanagerHost, pubkey: ValidatorPubKey, defaultFeeRecipient: Eth1Address
): Result[Eth1Address, ValidatorConfigFileStatus] =
  let res = getSuggestedFeeRecipient(host.validatorsDir, pubkey, defaultFeeRecipient).valueOr:
...
proc getSuggestedFeeRecipient(
    host: KeymanagerHost, pubkey: ValidatorPubKey, withdrawalAddress: Opt[Eth1Address]
): Eth1Address =
  # Enforce the gsfr(foo).valueOr(foo) pattern where feasible
  let perValidatorDefaultFeeRecipient =
    getPerValidatorDefaultFeeRecipient(host.defaultFeeRecipient, withdrawalAddress)
  host.getSuggestedFeeRecipient(pubkey, perValidatorDefaultFeeRecipient).valueOr:
    perValidatorDefaultFeeRecipient

proc getSuggestedGasLimit*(
    host: KeymanagerHost, pubkey: ValidatorPubKey
): Result[uint64, ValidatorConfigFileStatus] =
  let res = getSuggestedGasLimit(host.validatorsDir, pubkey, host.defaultGasLimit).valueOr:
    if error == ValidatorConfigFileStatus.noSuchValidator:
      # Dynamic validators do not have directories.
      if host.validatorPool[].isDynamic(pubkey):
...
proc addValidator*(
  ...
  let
    feeRecipient = host.getSuggestedFeeRecipient(keystore.pubkey, withdrawalAddress)
    gasLimit = host.getSuggestedGasLimit(keystore.pubkey).valueOr(host.defaultGasLimit)
...
proc generateDeposits*(
    ...
    mode = Secure,
): Result[seq[DepositData], KeystoreGenerationError] =
...
proc saveWallet(wallet: Wallet, outWalletPath: string): Result[void, string] =
  ...
  ?secureCreatePath(walletDir).mapErr(
    proc(e: auto): string =
      "Could not create wallet directory [" & walletDir & "]: " & $e
  )

  ?secureWriteFile(outWalletPath, encodedWallet).mapErr(
    proc(e: auto): string =
...
proc importKeystoreFromFile*(
    decryptor: var MultipleKeystoresDecryptor, fileName: string
  ...
  let
    ...
    keystore =
      try:
        ...
        return err("Invalid keystore file format [" & e.formatMsg(fileName) & "]")

  var firstDecryptionAttempt = true
  while true:
    var secret: seq[byte]
    let status = decryptCryptoField(
      keystore.crypto, KeystorePass.init(decryptor.previouslyUsedPassword), secret
    )
    case status
    ...
    of DecryptionStatus.InvalidPassword:
      ...
      if not (readPasswordInput("Password: ", decryptor.previouslyUsedPassword)):
        ...
        if len(decryptor.previouslyUsedPassword) == 0:
          break

proc importKeystoresFromDir*(
    rng: var HmacDrbgContext,
    meth: ImportMethod,
  ...
  var (singleSaltPassword, singleSaltSalt) =
    ...
    of ImportMethod.SingleSalt:
      (KeystorePass.init(ncrutils.toHex(rng.generateBytes(32))), rng.generateBytes(32))
  ...
  try:
    for file in walkDirRec(importedDir):
      ...
      let keystore =
        try:
          Json.loadFile(
            file, Keystore, requireAllFields = true, allowUnknownFields = true
          )
        except SerializationError as e:
      ...
      while true:
        var secret: seq[byte]
        let status =
          decryptCryptoField(keystore.crypto, KeystorePass.init password, secret)
        case status
        of DecryptionStatus.Success:
          ...
          if privKey.isOk:
            ...
            var (password, salt) =
              ...
              of ImportMethod.Normal:
                var defaultSeq: seq[byte]
                (KeystorePass.init ncrutils.toHex(rng.generateBytes(32)), defaultSeq)
              of ImportMethod.SingleSalt:
            ...
            let status = saveKeystore(
              rng, validatorsDir, secretsDir, privKey.value, pubkey, keystore.path,
              password.str, salt,
            )
            if status.isOk:
        ...
        of DecryptionStatus.InvalidPassword:
          if firstDecryptionAttempt:
            try:
              const msg =
                "Please enter the password for decrypting '$1' " &
...
proc pickPasswordAndSaveWallet(
    rng: var HmacDrbgContext, config: BeaconNodeConf, seed: KeySeed
): Result[WalletPathPair, string] =
...
proc createWalletInteractively*(
    rng: var HmacDrbgContext, config: BeaconNodeConf
  ...
  while true:
    ...
    if answer.len > 0 and answer[0] == 'q':
      quit 1
    if answer == "continue":
      break
  ...
  echoP "To confirm that you've saved the seed phrase, please enter the " &
    ...
    "seek phrase in your clipboard, we strongly advice clearing the " & "clipboard now."
  echo ""
  ...
  for i in countdown(2, 0):
    ...
    if parts.len == 2:
      if count(parts[1], ' ') == 2 and mnemonic.string.startsWith(parts[0]) and
...
proc restoreWalletInteractively*(rng: var HmacDrbgContext, config: BeaconNodeConf) =
  var
...
proc unlockWalletInteractively*(wallet: Wallet): Result[KeySeed, string] =
  ...
  let res = keyboardGetPassword[KeySeed](
    "Password: ",
    3,
...
proc findWallet*(
    config: BeaconNodeConf, name: WalletName
): Result[Opt[WalletPathPair], string] =
  ...
  try:
    ...
    return err(
      "Error accessing the wallets directory \"" & config.walletsDir & "\": " & err.msg
    )
...
func init*(T: type LaunchPadDeposit, cfg: RuntimeConfig, d: DepositData): T =
  T(
    pubkey: d.pubkey,
