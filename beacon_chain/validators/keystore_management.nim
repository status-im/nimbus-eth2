...
func feeRecipientPath(validatorsDir: string, pubkey: ValidatorPubKey): string =
  validatorsDir.validatorKeystoreDir(pubkey) / FeeRecipientFilename

func gasLimitPath(validatorsDir: string, pubkey: ValidatorPubKey): string =
  validatorsDir.validatorKeystoreDir(pubkey) / GasLimitFilename

func builderConfigPath(validatorsDir: string, pubkey: ValidatorPubKey): string =
...
proc loadNetKeystore*(
    keystorePath: string, insecurePwd: Option[string]
...
proc saveNetKeystore*(
  ...
  try:
    ...
    return
      err(KeystoreGenerationError(kind: FailedToCreateKeystoreFile, error: exc.msg))
...
proc createLockedLocalValidatorFiles(
  ...
  # keystoreFile:
  let lock =
    ?secureWriteLockedFile(keystoreFile, encodedStorage).mapErrTo(
      FailedToCreateKeystoreFile
    )
...
proc saveKeystore*(
  ...
  let encodedStorage =
    try:
      ...
      return
        err(KeystoreGenerationError(kind: FailedToCreateKeystoreFile, error: e.msg))
...
proc saveLockedKeystore(
  ...
  let lock =
    ?createLockedLocalValidatorFiles(
      secretsDir,
      validatorsDir,
      keystoreDir,
      secretsDir / keyName,
...
proc saveLockedKeystore(
  ...
  let lock =
    ?createLockedRemoteValidatorFiles(
...
proc setFeeRecipient*(
  ...
  io2.writeFile(validatorKeystoreDir / FeeRecipientFilename, $feeRecipient).mapErr(
    proc(e: auto): string =
...
proc setGasLimit*(
  ...
  io2.writeFile(validatorKeystoreDir / GasLimitFilename, $gasLimit).mapErr(
    proc(e: auto): string =
...
proc generateDeposits*(
  ...
  for i in 0 ..< localValidatorsCount:
    ...
    deposits.add prepareDeposit(cfg, withdrawalPubKey, derivedKey, signingPubKey)
...
): Result[Option[WalletPathPair], string] =
