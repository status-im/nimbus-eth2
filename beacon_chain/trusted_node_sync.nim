...
proc doTrustedNodeSync*(
  ...
  var client = createNewRestClient(restUrl).valueOr:
  ...
  let
    validatorMonitor = newClone(ValidatorMonitor.init(cfg, false, false))
...
when isMainModule:
  ...
  waitFor db.doTrustedNodeSync(
    cfg, databaseDir, os.paramStr(3), os.paramStr(4), syncTarget, backfill, false
  )
  db.close()
