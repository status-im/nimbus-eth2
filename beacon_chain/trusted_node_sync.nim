...
proc doTrustedNodeSync*(
  ...
  if head.isNone:
    ...
    # For Gloas states, we also need to download the execution payload envelope
    # for the head block. Without it, the clearance state's latest_block_hash
    # won't match the next block's parent_block_hash, blocking sync.
