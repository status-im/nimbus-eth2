...
import
  ...
  ./spec/[
    beaconstate, eth2_merkleization, forks, light_client_sync, network, presets,
...
const
  ...
  smallRequestsTimeout = 30.seconds
    # Downloading smaller items such as blocks and deposit snapshots.
...
proc fetchDepositSnapshot(
  ...
  let resp =
    try:
      ...
      return err(
        "The trusted node likely does not support the /eth/v1/beacon/deposit_snapshot end-point:" &
          e.msg
      )
  ...
  let snapshot = DepositTreeSnapshot(
    ...
    depositContractState: DepositContractState(
      branch: data.finalized, deposit_count: depositCountBytes(data.deposit_count)
    ),
...
chronicles.formatIt(TrustedNodeSyncTarget):
  shortLog(it)
...
proc doTrustedNodeSync*(
  ...
  notice "Starting trusted node sync", databaseDir, backfill, reindex
  ...
  let genesisState =
    if (let genesisRoot = db.getGenesisBlock(); genesisRoot.isSome()):
      ...
      if not db.getState(consensusFork, genesisStateRoot, tmp[], noRollback):
        error "Cannot load genesis state from database", genesisStateRoot
        quit 1

      if (genesisState != nil) and (getStateRoot(tmp[]) != getStateRoot(genesisState[])):
        error "Unexpected genesis state in database, is this the same network?",
          databaseRoot = getStateRoot(tmp[]), genesisRoot = getStateRoot(genesisState[])
        quit 1
      tmp
    else:
      let tmp =
        if genesisState != nil:
          ...
          of TrustedNodeSyncKind.StateId:
            ...
            try:
              ...
              info "Unable to download genesis state", error = exc.msg, restUrl
              nil
  ...
  let
    ...
    head =
      if dbHead.isSome():
        let bid = db.getBlockId(dbHead.get()).valueOr:
  ...
  if head.isNone:
      ...
      of TrustedNodeSyncKind.TrustedBlockRoot:
        ...
        while true:
          let
            ...
            current = getBeaconTime().slotOrZero().sync_committee_period
    ...
    if stateRoot.isSome:
      if state[].getStateRoot() != stateRoot.get:
        ...
        quit 1
      info "Checkpoint state validated against LC data", stateRoot = stateRoot.get
    ...
    if backfillSlot <= horizon:
        ...
        for i in 0 ..< 3:
          try:
            return awaitWithTimeout(
              client.getBlockV2(BlockIdent.init(slot), cfg), smallRequestsTimeout
            ):
      ...
      proc processBlock(blck: Option[ref ForkedSignedBeaconBlock]) =
        ...
        if blck.isSome():
          ...
          withBlck(data[]):
            ...
            if res.isErr():
              case res.error()
              of VerifierError.Invalid, VerifierError.MissingParent,
      ...
      try:
        ...
        for i in 0.uint64 .. missingSlots + gets.lenu64:
          if i >= gets.lenu64():
            let fut = gets[int(i mod gets.lenu64)]
        ...
        notice "Backfilling incomplete - blocks will be downloaded when starting the node",
          msg = exc.msg
        false
    else:
...
when isMainModule:
  import std/[os], networking/network_metadata

  let
    syncTarget =
      TrustedNodeSyncTarget(kind: TrustedNodeSyncKind.StateId, stateId: os.paramStr(5))
  ...
  waitFor db.doTrustedNodeSync(
    getRuntimeConfig(some os.paramStr(1)),
    os.paramStr(2),
    os.paramStr(3),
    os.paramStr(4),
    syncTarget,
