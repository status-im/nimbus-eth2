...
func voting_period_start_time*(state: BeaconState): uint64 =
  let eth1_voting_period_start_slot = state.slot - state.slot mod SLOTS_PER_ETH1_VOTING_PERIOD
...
method close*(p: Web3DataProviderRef): Future[void] {.async.} =
  if p.subscription != nil:
    await p.subscription.unsubscribe()
  await p.web3.close()

method fetchDepositData*(p: Web3DataProviderRef,
                         web3Block: BlockObject): Future[Eth1Block] {.async.} =
  let
