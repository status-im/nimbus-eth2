...
import chronos, chronicles, confutils, stew/[base10, results, io2], blscurve, presto
  ...
  for item in validatorPool:
    if item.kind == ValidatorKind.Remote and item.data.remotes.len == 1:
      validators.add RemoteKeystoreInfo(
        pubkey: item.pubkey, url: HttpHostUri(item.data.remotes[0].url)
      )
  validators
...
proc installKeymanagerHandlers*(router: var RestRouter, host: KeymanagerHost) =
  ...
  router.api(MethodPost, "/eth/v1/keystores") do(
    ...
    if request.slashing_protection.isSome():
      ...
      let res = inclSPDIR(host.validatorPool[].slashingProtection, slashing_protection)
      if res == siFailure:
        return keymanagerApiError(
  ...
  router.api(MethodDelete, "/eth/v1/validator/{pubkey}/feerecipient") do(
    ...
    return
      if res.isOk:
        ...
        keymanagerApiError(Http500, "Failed to remove fee recipient file: " & res.error)
  ...
  router.api(MethodDelete, "/eth/v1/validator/{pubkey}/gas_limit") do(
    ...
    return
      if res.isOk:
        ...
        keymanagerApiError(Http500, "Failed to remove gas limit file: " & res.error)
