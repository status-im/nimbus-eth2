...
logScope:
  topics = "rest_rewardsapi"

func isGenesis(node: BeaconNode, blockId: BlockIdent, genesisBsid: BlockSlotId): bool =
  case blockId.kind
...
proc installRewardsApiHandlers*(router: var RestRouter, node: BeaconNode) =
  let
    genesisBlockRewardsResponse = RestApiResponse.prepareJsonResponseFinalized(
      (
        proposer_index: "0",
        total: "0",
        attestations: "0",
  ...
  router.api2(MethodGet, "/eth/v1/beacon/rewards/blocks/{block_id}") do(
    ...
    let
      ...
      targetBlock = withBlck(bdata):
        let parentBid = node.dag.getBlockId(forkyBlck.message.parent_root).valueOr:
    ...
    let
      rewards = withBlck(bdata):
        state_transition_block(
          node.dag.cfg, tmpState[], forkyBlck, cache, node.dag.updateFlags, rollbackProc
        ).valueOr:
          return RestApiResponse.jsonError(Http400, BlockInvalidError)
      total =
        rewards.attestations + rewards.sync_aggregate + rewards.proposer_slashings +
        rewards.attester_slashings
      proposerIndex = withBlck(bdata):
  ...
  router.api2(MethodPost, "/eth/v1/beacon/rewards/sync_committee/{block_id}") do(
    block_id: BlockIdent, contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let
      idents = block:
    ...
    let response = withState(tmpState[]):
      ...
      when consensusFork > ConsensusFork.Phase0:
        let
          ...
          committeeKeys = toHashSet(forkyState.data.current_sync_committee.pubkeys.data)
          ...
          reward = block:
            ...
            if res > uint64(high(int64)):
              return RestApiResponse.jsonError(Http500, RewardOverflowError)
            res
          ...
          if (len(idents) == 0) or (pubkey in keys):
            resp.add(
              RestSyncCommitteeReward(
                validator_index: RestValidatorIndex(vindex), reward: RestReward(vreward)
              )
            )
    ...
    RestApiResponse.jsonResponseFinalized(
      response, node.getBlockOptimistic(bdata), node.dag.isFinalized(bid)
    )
