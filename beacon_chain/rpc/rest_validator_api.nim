...
import
  ...
  ".."/consensus_object_pools/
    [blockchain_dag, spec_cache, attestation_pool, sync_committee_msg_pool],
...
proc installValidatorApiHandlers*(router: var RestRouter, node: BeaconNode) =
  ...
  router.api(MethodPost, "/eth/v1/validator/duties/attester/{epoch}") do(
    epoch: Epoch, contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let indexList = block:
      ...
      if dres.isErr():
        return RestApiResponse.jsonError(
          Http400, InvalidValidatorIndexValueError, $dres.error()
        )
      ...
      for item in items:
        ...
        res.incl(vres.get())
      if len(res) == 0:
        return RestApiResponse.jsonError(Http400, EmptyValidatorIndexArrayError)
      res
    let qepoch = block:
      if epoch.isErr():
        return
          RestApiResponse.jsonError(Http400, InvalidEpochValueError, $epoch.error())
      let
        res = epoch.get()
      ...
      if res > wallEpoch + 1:
        return RestApiResponse.jsonError(
          Http400, InvalidEpochValueError, "Cannot request duties past next epoch"
        )
      res
    let qhead = block:
    ...
    let optimistic = node.getShufflingOptimistic(
      shufflingRef.attester_dependent_slot, shufflingRef.attester_dependent_root
    )
  ...
  router.api(MethodGet, "/eth/v1/validator/duties/proposer/{epoch}") do(
    ...
    let optimistic = node.getShufflingOptimistic(
      epochRef.proposer_dependent_slot, epochRef.proposer_dependent_root
    )
  ...
  router.api(MethodPost, "/eth/v1/validator/duties/sync/{epoch}") do(
    ...
    proc produceResponse(
        requestedValidatorIndices: openArray[ValidatorIndex],
      ...
      for requestedValidatorIdx in requestedValidatorIndices:
        ...
        let requestedValidatorPubkey = stateValidators[requestedValidatorIdx].pubkey
    ...
    if qSyncPeriod == headSyncPeriod:
      let res = withState(node.dag.headState):
        when consensusFork >= ConsensusFork.Altair:
          produceResponse(
            indexList, forkyState.data.current_sync_committee.pubkeys.data,
      ...
      let res = withState(node.dag.headState):
        when consensusFork >= ConsensusFork.Altair:
          produceResponse(
            indexList, forkyState.data.next_sync_committee.pubkeys.data,
            forkyState.data.validators.asSeq,
          )
        else:
          emptyResponse()
      ...
      let earliestSlotInQSyncPeriod =
        max(node.dag.cfg.ALTAIR_FORK_EPOCH.start_slot, qSyncPeriod.start_slot)
  ...
  router.api(MethodGet, "/eth/v1/validator/blocks/{slot}") do(
    slot: Slot, randao_reveal: Option[ValidatorSig], graffiti: Option[GraffitiBytes]
  ) -> RestApiResponse:
    return RestApiResponse.jsonError(Http410, DeprecatedRemovalValidatorBlocksV1)
  ...
  router.api(MethodGet, "/eth/v2/validator/blocks/{slot}") do(
    ...
    let contentType = preferredContentType(jsonMediaType, sszMediaType).valueOr:
      return RestApiResponse.jsonError(Http406, ContentNotAcceptableError)
    let message = block:
      let qslot = block:
        if slot.isErr():
          return
            RestApiResponse.jsonError(Http400, InvalidSlotValueError, $slot.error())
        let res = slot.get()
      ...
      let qskip_randao_verification =
        if skip_randao_verification.isNone():
          ...
          if res.isErr() or res.get() != "":
            return
              RestApiResponse.jsonError(Http400, InvalidSkipRandaoVerificationValue)
          true
      let qrandao =
        if randao_reveal.isNone():
          ...
          if res.isErr():
            return
              RestApiResponse.jsonError(Http400, InvalidRandaoRevealValue, $res.error())
          res.get()
      let qgraffiti =
        if graffiti.isNone():
          ...
          if res.isErr():
            return RestApiResponse.jsonError(
              Http400, InvalidGraffitiBytesValue, $res.error()
            )
          res.get()
      let qhead = block:
      ...
      if not node.verifyRandao(qslot, proposer, qrandao, qskip_randao_verification):
        return RestApiResponse.jsonError(Http400, InvalidRandaoRevealValue)

      let res = withConsensusFork(node.dag.cfg.consensusForkAtEpoch(qslot.epoch)):
        when consensusFork >= ConsensusFork.Bellatrix:
          await makeBeaconBlockForHeadAndSlot(
            consensusFork.ExecutionPayloadForSigning, node, qrandao, proposer,
  ...
  router.api(MethodGet, "/eth/v1/validator/blinded_blocks/{slot}") do(
    ...
    let qslot = block:
      if slot.isErr():
        return RestApiResponse.jsonError(Http400, InvalidSlotValueError, $slot.error())
      let res = slot.get()
    ...
    let qskip_randao_verification =
      if skip_randao_verification.isNone():
        ...
        let res = skip_randao_verification.get()
        if res.isErr() or res.get() != "":
          return RestApiResponse.jsonError(Http400, InvalidSkipRandaoVerificationValue)
        true
    let qrandao =
      if randao_reveal.isNone():
        return RestApiResponse.jsonError(Http400, MissingRandaoRevealValue)
    ...
    let qgraffiti =
      if graffiti.isNone():
        ...
        if res.isErr():
          return
            RestApiResponse.jsonError(Http400, InvalidGraffitiBytesValue, $res.error())
        res.get()
    let qhead = block:
      let res = node.getSyncedHead(qslot)
      if res.isErr():
    ...
    template responseVersioned(response: untyped, contextFork: ConsensusFork): untyped =
      if contentType == sszMediaType:
    ...
    withConsensusFork(contextFork):
      when consensusFork >= ConsensusFork.Capella:
        ...
        let res = await makeBeaconBlockForHeadAndSlot(
          bellatrix.ExecutionPayloadForSigning, node, qrandao, proposer, qgraffiti,
          qhead, qslot,
        )
        if res.isErr():
          return RestApiResponse.jsonError(Http400, res.error())
  ...
  router.api(MethodGet, "/eth/v3/validator/blocks/{slot}") do(
    ...
    withConsensusFork(node.dag.cfg.consensusForkAtEpoch(qslot.epoch)):
      when consensusFork >= ConsensusFork.Capella:
        let
          message = (
            await node.makeMaybeBlindedBeaconBlockForHeadAndSlot(
          ...
          headers = consensusFork.getMaybeBlindedHeaders(
            message.blck.isBlinded, message.executionValue, message.consensusValue
          )
          ...
          let forked =
            if message.blck.isBlinded:
              ForkedMaybeBlindedBeaconBlock.init(
                message.blck.blindedData, message.executionValue, message.consensusValue
              )
            else:
              ForkedMaybeBlindedBeaconBlock.init(
                message.blck.data, message.executionValue, message.consensusValue
              )
        ...
        let
          message = (
            await PayloadType.makeBeaconBlockForHeadAndSlot(
        ...
        template forkyBlck(): untyped =
          message.blck.forky(consensusFork)
  ...
  router.api(MethodGet, "/eth/v1/validator/attestation_data") do(
    ...
    let adata = block:
      let qslot = block:
        ...
        let res = slot.get()
        if res.isErr():
          return RestApiResponse.jsonError(Http400, InvalidSlotValueError, $res.error())
        res.get()
      ...
      if qslot + SLOTS_PER_EPOCH < (wallTime - MAXIMUM_GOSSIP_CLOCK_DISPARITY).slotOrZero:
        return RestApiResponse.jsonError(
      ...
      let qindex = block:
        ...
        if res.isErr():
          return RestApiResponse.jsonError(
            Http400, InvalidCommitteeIndexValueError, $res.error()
          )
        res.get()
      let qhead = block:
        let res = node.getSyncedHead(qslot)
  ...
  router.api(MethodGet, "/eth/v1/validator/aggregate_attestation") do(
    attestation_data_root: Option[Eth2Digest], slot: Option[Slot]
  ) -> RestApiResponse:
    let attestation = block:
      ...
      let qroot = block:
        if attestation_data_root.isNone():
          return
            RestApiResponse.jsonError(Http400, MissingAttestationDataRootValueError)
        let res = attestation_data_root.get()
        if res.isErr():
          return RestApiResponse.jsonError(
            Http400, InvalidAttestationDataRootValueError, $res.error()
          )
        res.get()
      let res = node.attestationPool[].getAggregatedAttestation(qslot, qroot)
      if res.isNone():
        return RestApiResponse.jsonError(Http400, UnableToGetAggregatedAttestationError)
      res.get()
  ...
  router.api(MethodPost, "/eth/v1/validator/aggregate_and_proofs") do(
    ...
    let proofs = block:
      ...
      if dres.isErr():
        return RestApiResponse.jsonError(
          Http400, InvalidAggregateAndProofObjectError, $dres.error()
        )
      dres.get()
    ...
    for future in pending:
      if future.completed():
        ...
        if res.isErr():
          return RestApiResponse.jsonError(
            Http400, AggregateAndProofValidationError, $res.error()
          )
      else:
        return RestApiResponse.jsonError(
          Http500, "Unexpected server failure, while sending aggregate and proof"
        )
  ...
  router.api(MethodPost, "/eth/v1/validator/beacon_committee_subscriptions") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let requests = block:
      if contentBody.isNone():
        return RestApiResponse.jsonError(Http400, EmptyRequestBodyError)
      let dres = decodeBody(seq[RestCommitteeSubscription], contentBody.get())
      if dres.isErr():
        return RestApiResponse.jsonError(
          Http400, InvalidSubscriptionRequestValueError, $dres.error()
        )
      dres.get()
    ...
    template getAndCacheShufflingRef(
        shufflingRefVar: var Opt[ShufflingRef], epoch: Epoch
    ): ShufflingRef =
    ...
    for request in requests:
      if uint64(request.committee_index) >= uint64(MAX_COMMITTEES_PER_SLOT):
        return RestApiResponse.jsonError(Http400, InvalidCommitteeIndexValueError)
      if uint64(request.validator_index) >=
          lenu64(getStateField(node.dag.headState, validators)):
        return RestApiResponse.jsonError(Http400, InvalidValidatorIndexValueError)
      ...
      let
        ...
        shufflingRef =
          if epoch == wallEpoch:
            ...
            return RestApiResponse.jsonError(Http400, SlotNotInNextWallSlotEpochError)
      ...
      if not is_active_validator(
        getStateField(node.dag.headState, validators).item(request.validator_index),
        request.slot.epoch,
      ):
        return RestApiResponse.jsonError(Http400, ValidatorNotActive)

      node.consensusManager[].actionTracker.registerDuty(
        request.slot, subnet_id, request.validator_index, request.is_aggregator
      )

      let validator_pubkey = getStateField(node.dag.headState, validators)
        .item(request.validator_index).pubkey
  ...
  router.api(MethodPost, "/eth/v1/validator/sync_committee_subscriptions") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let subscriptions = block:
      var res: seq[RestSyncCommitteeSubscription]
      if contentBody.isNone():
        return RestApiResponse.jsonError(Http400, EmptyRequestBodyError)
      let dres = decodeBody(seq[RestSyncCommitteeSubscription], contentBody.get())
      if dres.isErr():
        return RestApiResponse.jsonError(
          Http400, InvalidSyncCommitteeSubscriptionRequestError
        )
      let subs = dres.get()
      for item in subs:
    ...
    for item in subscriptions:
      let validator_pubkey =
        getStateField(node.dag.headState, validators).item(item.validator_index).pubkey
  ...
  router.api(MethodGet, "/eth/v1/validator/sync_committee_contribution") do(
    slot: Option[Slot],
    subcommittee_index: Option[SyncSubCommitteeIndex],
    beacon_block_root: Option[Eth2Digest]
  ) -> RestApiResponse:
    let qslot = block:
      if slot.isNone():
        return RestApiResponse.jsonError(Http400, MissingSlotValueError)
    ...
    let qindex =
      if subcommittee_index.isNone():
        return RestApiResponse.jsonError(Http400, MissingSubCommitteeIndexValueError)
      else:
        let res = subcommittee_index.get()
        if res.isErr():
          return RestApiResponse.jsonError(
            Http400, InvalidSubCommitteeIndexValueError, $res.error()
          )
        res.get()
    let qroot =
      if beacon_block_root.isNone():
        return RestApiResponse.jsonError(Http400, MissingBeaconBlockRootValueError)
      else:
        let res = beacon_block_root.get()
        if res.isErr():
          return RestApiResponse.jsonError(
            Http400, InvalidBeaconBlockRootValueError, $res.error()
          )
        res.get()
  ...
  router.api(MethodPost, "/eth/v1/validator/contribution_and_proofs") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let proofs = block:
      if contentBody.isNone():
        return RestApiResponse.jsonError(Http400, EmptyRequestBodyError)
      let dres = decodeBody(seq[SignedContributionAndProof], contentBody.get())
      if dres.isErr():
        return
          RestApiResponse.jsonError(Http400, InvalidContributionAndProofMessageError)
      dres.get()
    ...
    if len(failures) > 0:
      ...
      return RestApiResponse.jsonMsgResponse(ContributionAndProofValidationSuccess)

  # https://ethereum.github.io/beacon-APIs/#/ValidatorRequiredApi/prepareBeaconProposer
  router.api(MethodPost, "/eth/v1/validator/prepare_beacon_proposer") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let
      body = block:
        ...
        if dres.isErr():
          return RestApiResponse.jsonError(Http400, InvalidPrepareBeaconProposerError)
        dres.get()
      currentEpoch = node.beaconClock.now.slotOrZero.epoch

    for proposerData in body:
      node.dynamicFeeRecipientsStore[].addMapping(
        proposerData.validator_index, proposerData.fee_recipient, currentEpoch
      )
  ...
  router.api(MethodPost, "/eth/v1/validator/register_validator") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let body = block:
      if contentBody.isNone():
    ...
    let
      ...
      indexList = block:
        ...
        for item in items:
          ...
          let index = vres.get()
          if index in dupset:
            return RestApiResponse.jsonError(Http400, DuplicateValidatorIndexArrayError)
          dupset.incl(index)
          res.add(index)
        if len(res) == 0:
          return RestApiResponse.jsonError(Http400, EmptyValidatorIndexArrayError)
      ...
      response = indexList.mapIt(
        RestLivenessItem(
          index: it, is_live: node.attestationPool[].validatorSeenAtEpoch(qepoch, it)
        )
      )
  ...
  router.api(MethodPost, "/eth/v1/validator/beacon_committee_selections") do(
    ...
    return RestApiResponse.jsonError(Http501, AggregationSelectionNotImplemented)
