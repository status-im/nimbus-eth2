...
proc installValidatorApiHandlers*(router: var RestRouter, node: BeaconNode) =
  ...
  router.api2(MethodGet, "/eth/v1/validator/blocks/{slot}") do(
    ...
    RestApiResponse.jsonError(Http410, DeprecatedRemovalValidatorBlocksV1)
  ...
  router.api(MethodGet, "/eth/v2/validator/blocks/{slot}") do(
    ...
    RestApiResponse.jsonError(Http410, DeprecatedRemovalValidatorBlocksV2)
  ...
  router.api(MethodGet, "/eth/v1/validator/blinded_blocks/{slot}") do(
    ...
    RestApiResponse.jsonError(Http410, DeprecatedRemovalProduceBlindedBlockV1)
  ...
  router.api(MethodGet, "/eth/v3/validator/blocks/{slot}") do(
    ...
    graffiti: Option[GraffitiBytes],
    skip_randao_verification: Option[string],
    builder_boost_factor: Option[uint64]
  ) -> RestApiResponse:
    let
      ...
      when consensusFork >= ConsensusFork.Gloas:
        ...
        let message = (
          await node.makeMaybeBlindedBeaconBlockForHeadAndSlot(
            consensusFork, proposer, qrandao, qgraffiti, qhead, qslot, qboostFactor
          )
        ).valueOr:
        ...
        if contentType == sszMediaType:
          ...
          RestApiResponse.jsonResponsePlain(
            forked, consensusFork, message.blck.isBlinded, message.executionValue,
        ...
        let message = (
          await node.makeBeaconBlockForHeadAndSlot(
  ...
  router.api2(MethodGet, "/eth/v2/validator/aggregate_attestation") do(
    ...
    let root = block:
      if attestation_data_root.isNone():
        return RestApiResponse.jsonError(Http400, MissingAttestationDataRootValueError)
    ...
    let
      ...
      forked =
        if qfork >= ConsensusFork.Electra:
          let electra_attestation = node.attestationPool[].getElectraAggregatedAttestation(
            qslot, root, committee_index
          ).valueOr:
            return
              RestApiResponse.jsonError(Http404, UnableToGetAggregatedAttestationError)
          ForkedAttestation.init(electra_attestation, qfork)
        else:
          let phase0_attestation = node.attestationPool[].getPhase0AggregatedAttestation(
            qslot, root
          ).valueOr:
            return
              RestApiResponse.jsonError(Http404, UnableToGetAggregatedAttestationError)
  ...
  router.api2(MethodPost, "/eth/v1/validator/beacon_committee_subscriptions") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let requests = block:
      if contentBody.isNone():
    ...
    for request in requests:
      ...
      node.validatorMonitor[].addAutoMonitor(validator_pubkey, request.validator_index)
  ...
  router.api2(MethodPost, "/eth/v1/validator/sync_committee_subscriptions") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    ...
    for item in subscriptions:
      ...
      node.validatorMonitor[].addAutoMonitor(validator_pubkey, item.validator_index)
  ...
  router.api2(MethodGet, "/eth/v1/validator/sync_committee_contribution") do(
    slot: Option[Slot],
  ...
  router.api2(MethodPost, "/eth/v1/validator/contribution_and_proofs") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let proofs = block:
      if contentBody.isNone():
  ...
  router.api2(MethodPost, "/eth/v1/validator/prepare_beacon_proposer") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let
      body = block:
    ...
    info "Prepared beacon proposers",
      numUpdatedFeeRecipients = numUpdated, numRefreshedFeeRecipients = numRefreshed
  ...
  router.api2(MethodPost, "/eth/v1/validator/register_validator") do(
    contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    if contentBody.isNone():
      return RestApiResponse.jsonError(Http400, EmptyRequestBodyError)
    let body = decodeBodyJsonOrSsz(
    ...
    for registration in body:
      ...
      node.externalBuilderRegistrations[registration.message.pubkey] = registration
