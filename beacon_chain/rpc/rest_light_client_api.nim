...
proc installLightClientApiHandlers*(router: var RestRouter, node: BeaconNode) =
  ...
  router.api2(MethodGet, "/eth/v1/beacon/light_client/bootstrap/{block_root}") do(
  ...
  router.api2(MethodGet, "/eth/v1/beacon/light_client/updates") do(
  ...
  router.api2(MethodGet, "/eth/v1/beacon/light_client/finality_update") do() -> RestApiResponse:
    doAssert node.dag.lcDataStore.serve
    let contentType = block:
      let res = preferredContentType(jsonMediaType, sszMediaType)
      if res.isErr():
  ...
  router.api2(MethodGet, "/eth/v1/beacon/light_client/optimistic_update") do() -> RestApiResponse:
    doAssert node.dag.lcDataStore.serve
    let contentType = block:
      let res = preferredContentType(jsonMediaType, sszMediaType)
      if res.isErr():
