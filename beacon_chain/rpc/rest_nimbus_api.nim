...
RestJson.useDefaultSerializationFor(
  BlockProposalEth1Data, Eth1BlockObj, RestChronosMetricsInfo, RestConnectionInfo,
  RestFutureInfo, RestPeerInfo, RestPeerInfoTuple, RestPeerStats, RestPeerStatus,
  RestPubSubPeer, RestSimplePeer,
)
...
proc toNode(v: PubSubPeer, backoff: Moment): RestPubSubPeer =
  RestPubSubPeer(
    ...
    agent: when defined(libp2p_agents_metrics): v.shortAgent else: "unknown",
  )
...
proc installNimbusApiHandlers*(router: var RestRouter, node: BeaconNode) =
  ...
  router.api(MethodGet, "/nimbus/v1/chain/head") do() -> RestApiResponse:
    let
      ...
      justified = getStateField(node.dag.headState, current_justified_checkpoint)
  ...
  router.api(MethodGet, "/nimbus/v1/syncmanager/status") do() -> RestApiResponse:
    return RestApiResponse.jsonResponse(node.syncManager.inProgress)

  router.api(MethodGet, "/nimbus/v1/node/peerid") do() -> RestApiResponse:
    return RestApiResponse.jsonResponse((peerid: $node.network.peerId()))

  router.api(MethodGet, "/nimbus/v1/node/version") do() -> RestApiResponse:
    return RestApiResponse.jsonResponse((version: "Nimbus/" & fullVersionStr))

  router.api(MethodGet, "/nimbus/v1/network/ids") do() -> RestApiResponse:
    var res: seq[PeerId]
    for peerId, peer in node.network.peerPool:
      res.add(peerId)
    return RestApiResponse.jsonResponse((peerids: res))

  router.api(MethodGet, "/nimbus/v1/network/peers") do() -> RestApiResponse:
    ...
    template setGraffitiAux(node: BeaconNode, graffitiStr: string): RestApiResponse =
      node.graffitiBytes =
        try:
          ...
          return RestApiResponse.jsonError(Http400, InvalidGraffitiBytesValue, err.msg)
    ...
    if body.contentType == ApplicationJsonMediaType:
      ...
      if graffitiBytes.isErr():
        return RestApiResponse.jsonError(
          Http400, InvalidGraffitiBytesValue, $graffitiBytes.error()
        )
  ...
  router.api(MethodGet, "/nimbus/v1/graffiti") do() -> RestApiResponse:
  ...
  router.api(MethodPost, "/nimbus/v1/chronicles/settings") do(
    ...
    if log_level.isSome():
      let level = block:
        ...
        if res.isErr():
          return
            RestApiResponse.jsonError(Http400, InvalidLogLevelValueError, $res.error())
        res.get()
      {.gcsafe.}:
        updateLogLevel(level)
    return RestApiResponse.jsonResponse((result: true))

  router.api(MethodGet, "/nimbus/v1/eth1/chain") do() -> RestApiResponse:
    let res = mapIt(node.elManager.eth1ChainBlocks, it)
    return RestApiResponse.jsonResponse(res)

  router.api(MethodGet, "/nimbus/v1/eth1/proposal_data") do() -> RestApiResponse:
    ...
    node.dag.withUpdatedState(
      proposalState[], head.atSlot(wallSlot).toBlockSlotId().expect("not nil")
    ):
      return RestApiResponse.jsonResponse(node.getBlockProposalEth1Data(updatedState))
    do:
      return RestApiResponse.jsonError(Http400, PrunedStateError)

  router.api(MethodGet, "/nimbus/v1/debug/chronos/futures") do() -> RestApiResponse:
    when defined(chronosFutureTracking):
      ...
      return RestApiResponse.jsonError(
        Http503, "Compile with '-d:chronosFutureTracking' to get this request working"
      )

  router.api(MethodGet, "/nimbus/v1/debug/chronos/metrics") do() -> RestApiResponse:
    ...
    let res = RestChronosMetricsInfo(
      ...
      http_server_secure_connections: getCount(HttpServerSecureConnectionTrackerName),
  ...
  router.api(MethodGet, "/nimbus/v1/debug/chronos/restserver/connections") do() -> RestApiResponse:
  ...
  router.api(MethodGet, "/nimbus/v1/debug/gossip/peers") do() -> RestApiResponse:
    let gossipPeers = block:
  ...
  router.api(MethodPost, "/nimbus/v1/timesync") do(
    ...
    let
      ...
      timestamp1 = block:
        ...
        if dres.isErr():
          return
            RestApiResponse.jsonError(Http400, InvalidTimestampValue, $dres.error())
        dres.get().timestamp1
    let
      delay = node.processingDelay.valueOr:
        ZeroDuration
