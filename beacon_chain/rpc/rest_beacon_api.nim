...
proc installBeaconApiHandlers*(router: var RestRouter, node: BeaconNode) =
  ...
  router.api(MethodGet, "/eth/v1/beacon/states/{state_id}/finality_checkpoints") do(
    state_id: StateIdent
  ) -> RestApiResponse:
    let
      sid = state_id.valueOr:
  ...
  router.api(MethodPost, "/eth/v1/beacon/states/{state_id}/validators") do(
    state_id: StateIdent, contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let
  ...
  router.api(MethodGet, "/eth/v1/beacon/states/{state_id}/validators/{validator_id}") do(
    state_id: StateIdent, validator_id: ValidatorIdent
  ) -> RestApiResponse:
    let
      sid = state_id.valueOr:
        return RestApiResponse.jsonError(Http400, InvalidStateIdValueError, $error)
  ...
  router.api(MethodGet, "/eth/v1/beacon/states/{state_id}/validator_balances") do(
  ...
  router.api(MethodPost, "/eth/v1/beacon/states/{state_id}/validator_balances") do(
    state_id: StateIdent, contentBody: Option[ContentBody]
  ) -> RestApiResponse:
    let
      validatorIds = block:
  ...
  router.api(MethodGet, "/eth/v1/beacon/states/{state_id}/committees") do(
    state_id: StateIdent,
    epoch: Option[Epoch],
    ...
    let vslot =
      if slot.isSome():
        ...
        if rslot.isErr():
          return
            RestApiResponse.jsonError(Http400, InvalidSlotValueError, $rslot.error())
        let res = rslot.get()
        if vepoch.isSome():
  ...
  router.api(MethodGet, "/eth/v1/beacon/states/{state_id}/sync_committees") do(
    state_id: StateIdent, epoch: Option[Epoch]
  ) -> RestApiResponse:
    let
      sid = state_id.valueOr:
        return RestApiResponse.jsonError(Http400, InvalidStateIdValueError, $error)
  ...
  router.api(MethodGet, "/eth/v1/beacon/states/{state_id}/randao") do(
    state_id: StateIdent, epoch: Option[Epoch]
  ) -> RestApiResponse:
    let
      sid = state_id.valueOr:
    ...
    if bsi.isSome:
      ...
      if mix.isSome:
        return RestApiResponse.jsonResponseWOpt(
          RestEpochRandao(randao: mix.get), node.getBidOptimistic(bsi.get.bid)
        )
  ...
  router.api(MethodGet, "/eth/v1/beacon/headers") do(
    ...
    if parent_root.isSome():
      ...
      if rroot.isErr():
        return RestApiResponse.jsonError(
          Http400, InvalidParentRootValueError, $rroot.error()
        )
      return RestApiResponse.jsonError(Http500, NoImplementationError)
  ...
  router.api(MethodPost, "/eth/v1/beacon/blocks") do(
    ...
    let res = block:
      ...
      of ConsensusFork.Phase0:
        ...
        await node.router.routeSignedBeaconBlock(blck, Opt.none(seq[BlobSidecar]))
    ...
    let res = block:
      let
        ...
        body = block:
          ...
          contentBody.get()
      var
        restBlock = decodeBodyJsonOrSsz(RestPublishedSignedBlockContents, body, version).valueOr:
          return RestApiResponse.jsonError(error)
        forked = ForkedSignedBeaconBlock.init(restBlock)
  ...
  router.api(MethodGet, "/eth/v1/beacon/blocks/{block_id}") do(
    ...
    return RestApiResponse.jsonError(Http410, DeprecatedRemovalBeaconBlocksDebugStateV1)
    ...
    return
      if contentType == sszMediaType:
        ...
        RestApiResponse.jsonResponseBlock(
          bdata.asSigned(), node.getBlockOptimistic(bdata), node.dag.isFinalized(bid)
        )
      else:
  ...
  router.api(MethodGet, "/eth/v1/beacon/blocks/{block_id}/attestations") do(
    block_id: BlockIdent
  ) -> RestApiResponse:
    let
      blockIdent = block_id.valueOr:
  ...
  router.api(MethodPost, "/eth/v1/beacon/pool/attestations") do(
    ...
    let attestations = block:
      ...
      if dres.isErr():
        return RestApiResponse.jsonError(
          Http400, InvalidAttestationObjectError, $dres.error()
        )
      dres.get()
  ...
  router.api(MethodGet, "/eth/v1/beacon/pool/attester_slashings") do() -> RestApiResponse:
    return
      RestApiResponse.jsonResponse(toSeq(node.validatorChangePool.attester_slashings))
  ...
  router.api(MethodGet, "/eth/v1/beacon/pool/proposer_slashings") do() -> RestApiResponse:
    return
  ...
  router.api(MethodPost, "/eth/v1/beacon/pool/proposer_slashings") do(
    ...
    if res.isErr():
      return RestApiResponse.jsonError(
        Http400, ProposerSlashingValidationError, $res.error()
      )
  ...
  router.api(MethodGet, "/eth/v1/beacon/pool/bls_to_execution_changes") do() -> RestApiResponse:
    return RestApiResponse.jsonResponse(
  ...
  router.api(MethodPost, "/eth/v1/beacon/pool/bls_to_execution_changes") do(
    ...
    let bls_to_execution_changes = block:
      if contentBody.isNone():
        return RestApiResponse.jsonError(Http400, EmptyRequestBodyError)
    ...
    for individual_res in res:
      ...
      if individual_res.failed():
        return RestApiResponse.jsonError(
          Http400, BlsToExecutionChangeValidationError, $individual_res.error[].msg
        )
      let fut_result = individual_res.read()
      if fut_result.isErr():
        return RestApiResponse.jsonError(
          Http400, BlsToExecutionChangeValidationError, $fut_result.error()
        )
  ...
  router.api(MethodPost, "/eth/v1/beacon/pool/sync_committees") do(
    ...
    let failures = block:
      ...
      res
    if len(failures) > 0:
      return RestApiResponse.jsonErrorList(
        Http400, SyncCommitteeMessageValidationError, failures
      )
    else:
      return RestApiResponse.jsonMsgResponse(SyncCommitteeMessageValidationSuccess)

  # https://ethereum.github.io/beacon-APIs/#/Beacon/getPoolVoluntaryExits
  router.api(MethodGet, "/eth/v1/beacon/pool/voluntary_exits") do() -> RestApiResponse:
  ...
  router.api(MethodGet, "/eth/v1/beacon/blob_sidecars/{block_id}") do(
    ...
    if indices.isErr:
      return RestApiResponse.jsonError(Http400, InvalidSidecarIndexValueError)
