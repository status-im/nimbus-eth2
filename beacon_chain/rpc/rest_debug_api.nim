...
import
  ".."/beacon_node, ".."/spec/forks, "."/[rest_beacon_api, rest_utils, state_ttl_cache]
...
proc installDebugApiHandlers*(router: var RestRouter, node: BeaconNode) =
  ...
  router.api2(MethodGet, "/eth/v1/debug/beacon/data_column_sidecars/{block_id}") do(
    ...
    ](node, preferredContentType(jsonMediaType, sszMediaType), block_id, indices)
  ...
  router.api2(MethodGet, "/eth/v1/debug/beacon/heads") do() -> RestApiResponse:
    RestApiResponse.jsonError(Http410, DeprecatedRemovalGetDebugChainHeadsV1)

  # https://ethereum.github.io/beacon-APIs/#/Debug/getDebugChainHeadsV2
  router.api2(MethodGet, "/eth/v2/debug/beacon/heads") do() -> RestApiResponse:
  ...
  router.api2(MethodGet, "/eth/v1/debug/fork_choice") do() -> RestApiResponse:
    ...
    for item in forkChoice.backend.proto_array:
      ...
      response.fork_choice_nodes.add RestNode(
        ...
        execution_block_hash: node.dag.loadExecutionBlockHash(item.bid).get(ZERO_HASH),
