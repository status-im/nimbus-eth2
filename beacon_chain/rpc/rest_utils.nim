...
export
  results, eth2_rest_serialization, blockchain_dag, rest_types, rest_constants,
  rest_common, route
...
template withStateForBlockSlotId*(
  ...
  block:
    ...
    if isState(node.dag.headState):
      ...
      if node.dag.updateState(
        stateToAdvance[], blockSlotId, false, cache, node.dag.updateFlags
      ):
...
proc verifyRandao*(
  ...
  let proposer_pubkey = node.dag.validatorKey(proposer).valueOr:
    return false
