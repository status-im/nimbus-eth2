...
func nodeIsViableForHead(self: var ProtoArray, node: ProtoNode, nodeIdx: Index): bool
func nodeLeadsToViableHead(
  self: var ProtoArray, node: ProtoNode, nodeIdx: Index
): FcResult[bool]
  ...
  return
    if not correctJustified:
      ...
      if ancestor.isNone:
        ...
        var idx = nodeIdx
        template mutableNode(): untyped =
          self.nodes.buf[idx - self.nodes.offset]
