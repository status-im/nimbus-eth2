...
proc init*(T: type ForkChoice, epochRef: EpochRef, blck: BlockRef): T =
  ...
  ## is used here
  debug "Initializing fork choice", epoch = epochRef.epoch, blck = shortLog(blck)

  let checkpoint = Checkpoint(root: blck.root, epoch: epochRef.epoch)
  ForkChoice(
    backend: ForkChoiceBackend.init(
      FinalityCheckpoints(justified: checkpoint, finalized: checkpoint)
    ),
...
proc update_justified(
  ...
  trace "Updating justified", store = self.justified.checkpoint, state = justified
...
proc update_justified(
    self: var Checkpoints, dag: ChainDAGRef, justified: Checkpoint
): FcResult[void] =
  let blck = dag.getBlockRef(justified.root).valueOr:
    return err ForkChoiceError(kind: fcJustifiedNodeUnknown, blockRoot: justified.root)
...
proc update_checkpoints(
    self: var Checkpoints, dag: ChainDAGRef, checkpoints: FinalityCheckpoints
): FcResult[void] =
  ...
  if checkpoints.finalized.epoch > self.finalized.epoch:
    trace "Updating finalized", store = self.finalized, state = checkpoints.finalized
...
proc on_tick(self: var ForkChoice, dag: ChainDAGRef, time: BeaconTime): FcResult[void] =
...
func process_attestation(
  ...
  template vote(): untyped =
    self.votes[validator_index]
  ...
  if target_epoch > vote.next_epoch or vote.next_root.isZero:
    ...
    trace "Integrating vote in fork choice",
      validator_index = validator_index, new_vote = shortLog(vote)
...
proc update_time*(
    self: var ForkChoice, dag: ChainDAGRef, time: BeaconTime
): FcResult[void] =
...
proc on_attestation*(
  ...
  ?self.update_time(
    dag, max(wallTime, attestation_slot.start_beacon_time(dag.timeParams))
  )
...
func process_equivocation*(self: var ForkChoice, validator_index: ValidatorIndex) =
  self.backend.votes.extend(validator_index.int + 1)

  # Disallow future votes
  template vote(): untyped =
    self.backend.votes[validator_index]
...
proc process_block*(
  ...
  ?update_time(self, dag, max(wallTime, blckRef.slot.start_beacon_time(dag.timeParams)))
  ...
  if slot == blck.slot and
      ...
      slot.attestation_deadline(dag.timeParams, typeof(blck).kind) and
      self.checkpoints.proposer_boost_root == ZERO_HASH:
  ...
  if unrealized_is_better:
    if epochRef.epoch < slot.epoch:
      ...
      ?process_block(self.backend, blckRef.bid, blck.parent_root, unrealized)
    else:
      ?process_block(
        self.backend,
        blckRef.bid,
        blck.parent_root,
        epochRef.checkpoints,
        some unrealized,
      ) # Realized in `on_tick`
  else:
    ?process_block(self.backend, blckRef.bid, blck.parent_root, epochRef.checkpoints)
...
func find_head(
  ...
  ?self.proto_array.applyScoreChanges(
    deltas, current_epoch, checkpoints, justified_total_active_balance,
    proposer_boost_root,
  )
  ...
  trace "Fork choice requested", checkpoints, fork_choice_head = shortLog(new_head)
...
func mark_root_invalid*(self: var ForkChoice, root: Eth2Digest) =
  try:
    let nodePhysicalIdx =
      self.backend.proto_array.indices[root] - self.backend.proto_array.nodes.offset
  ...
  for val_index, vote in votes.mpairs():
    ...
    if vote.current_root != vote.next_root or old_balance != new_balance:
      ...
      if vote.current_root in indices:
        ...
        if index >= deltas.len:
          return err ForkChoiceError(kind: fcInvalidNodeDelta, index: index)
        deltas[index] -=
          Delta old_balance
...
when isMainModule:
  ...
  proc tMove_out_of_tree() =
    ...
    votes.add VoteTracker(
      current_root: fakeHash(1), next_root: default(Eth2Digest), next_epoch: Epoch(0)
    )

    # One validator moves their vote from the block to something outside of the tree
    votes.add VoteTracker(
      current_root: fakeHash(1), next_root: fakeHash(1337), next_epoch: Epoch(0)
    )
  ...
  proc tValidator_appears() =
    ...
    for _ in 0 ..< 2:
      votes.add VoteTracker(
        current_root: fakeHash(1), next_root: fakeHash(2), next_epoch: Epoch(0)
      )
