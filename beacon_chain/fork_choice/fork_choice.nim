...
proc init*(
    T: type ForkChoice, epochRef: EpochRef, blck: BlockRef, version: ForkChoiceVersion
): T =
  ...
  ForkChoice(
    backend: ForkChoiceBackend.init(
      FinalityCheckpoints(justified: checkpoint, finalized: checkpoint), version
    ),
...
proc process_block*(
  ...
  if slot == blck.slot and self.checkpoints.time < slot.attestation_deadline and
      self.checkpoints.proposer_boost_root == ZERO_HASH:
