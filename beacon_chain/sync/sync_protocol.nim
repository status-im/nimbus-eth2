...
p2pProtocol BeaconSync(version = 1,
  ...
  proc blobSidecarsByRoot(
    ...
    #      will be generated by the libp2p macro - we guarantee that seq items
    #      are `not-nil` in the implementation
    trace "got v1 blobs range request", peer, len = blobIds.len
    if blobIds.len == 0:
    ...
    debug "Blob root v1 request done",
  ...
  proc blobSidecarsByRange(
    ...
    trace "got v1 blobs range request", peer, startSlot, count = reqCount
    if reqCount == 0:
      raise newException(InvalidInputsError, "Empty range requested")
    ...
    debug "BlobSidecar v1 range request done",
  ...
  proc blobSidecarsByRoot_v2(
    ...
    #      will be generated by the libp2p macro - we guarantee that seq items
    #      are `not-nil` in the implementation
    trace "got v2 blobs range request", peer, len = blobIds.len
    if blobIds.len == 0:
      raise newException(InvalidInputsError, "No blobs requested")
    ...
    for i in 0..<count:
      ...
      if dag.db.getBlobSidecarSZ(blockRef.bid.root, index, bytes):
        ...
        peer.awaitQuota(blobResponseCost, "blob_sidecars_by_root/2")
        peer.network.awaitQuota(blobResponseCost, "blob_sidecars_by_root/2")
    ...
    debug "Blob root v2 request done",
      peer, roots = blobIds.len, count, found
  ...
  proc blobSidecarsByRange_v2(
    ...
    trace "got v2 blobs range request", peer, startSlot, count = reqCount
    if reqCount == 0:
      raise newException(InvalidInputsError, "Empty range requested")
    ...
    var blockIds: array[int(MAX_REQUEST_BLOB_SIDECARS_ELECTRA), BlockId]
    let
      count = int min(reqCount, blockIds.lenu64)
      endIndex = count - 1
      startIndex =
    ...
    for i in startIndex..endIndex:
      for j in 0..<MAX_BLOBS_PER_BLOCK:
        if dag.db.getBlobSidecarSZ(blockIds[i].root, BlobIndex(j), bytes):
          ...
          # TODO extract from libp2pProtocol
          peer.awaitQuota(blobResponseCost, "blobs_sidecars_by_range/2")
          peer.network.awaitQuota(blobResponseCost, "blobs_sidecars_by_range/2")
    ...
    debug "BlobSidecar v2 range request done",
      peer, startSlot, count = reqCount, found
