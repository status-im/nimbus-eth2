...
func cmpSidecarIdentifier(x: BlobIdentifier,
                          y: ref ForkyBlobSidecar): int =
  cmp(x.index, y.index)

func cmpSidecarIdentifier(x: DataColumnIdentifier,
                          y: ref DataColumnSidecar): int =
  cmp(x.index, y[].index)
...
func checkResponseSanity(idList: seq[BlobIdentifier],
  ...
  var i = 0
  while i < blobs.len:
    withForkyBlob(blobs[i]):
      func cmpIdentifier(x: BlobIdentifier, y: auto): int =
        cmpSidecarIdentifier(x, y)

      let
        block_root = hash_tree_root(forkyBlob[].signed_block_header.message)
        id = idList[i]
        idListKey = binarySearch(idList, forkyBlob, cmpIdentifier)
...
func checkResponseSubset(idList: seq[BlobIdentifier],
  ...
  for blb in blobs:
    withForkyBlob(blb):
      ...
      if binarySearch(idList, forkyBlob, cmpIdentifier) == -1:
        return false
  true
...
func cmpSidecarIndexes(x, y: ForkedBlobSidecar): int =
  let
    xIndex = withForkyBlob(x):
      forkyBlob.index
    yIndex = withForkyBlob(y):
      forkyBlob.index
  cmp(xIndex, yIndex)

func cmpSidecarIndexes(x, y: ref DataColumnSidecar): int =
  cmp(x[].index, y[].index)
