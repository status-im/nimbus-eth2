...
proc checkResponse(idList: seq[BlobIdentifier],
  ...
  while i < blobs.len:
    withForkyBlob(blobs[i]):
      ...
      func cmpSidecarIdentifier(
          x: BlobIdentifier, y: ref ForkyBlobSidecar): int =
        cmp(x.index, y.index)
      ...
      if id.block_root != block_root or id.index != forkyBlob[].index:
        return false
...
func checkResponse(idList: seq[DataColumnIdentifier],
  ...
  while i < columns.len:
    ...
    func cmpSidecarIdentifier(
        x: DataColumnIdentifier, y: ref DataColumnSidecar): int =
      cmp(x.index, y.index)
