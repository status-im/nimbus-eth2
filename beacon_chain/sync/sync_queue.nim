...
proc getShortMap*[T](req: SyncRequest[T],
  ...
  for i in 0 ..< req.data.count:
    if last < len(data):
      var counter = 0
      for k in last ..< len(data):
        let blobSlot = withForkyBlob(data[k]):
          forkyBlob[].signed_block_header.message.slot
        if slider < blobSlot:
          break
        elif slider == blobSlot:
          inc(counter)
      last = last + counter
      if counter == 0:
        res.add('.')
...
proc getShortMap*[T](
  ...
  for i in 0 ..< int(req.data.count):
    ...
    if len(blobs[i]) > 0:
      let slot = withForkyBlob(blobs[i][0]):
        forkyBlob[].signed_block_header.message.slot
      if not(notFirst):
...
proc getShortMap*[T](
    req: SyncRequest[T],
    data: Opt[seq[ForkedBlobSidecars]]
): string =
  if data.isNone():
...
): (ref ForkedSignedBeaconBlock, Opt[ForkedBlobSidecars]) =
  case kind
  of SyncQueueKind.Forward:
