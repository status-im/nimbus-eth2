...
import
  ...
  # Status libraries
  results,
  stew/[bitops2, base10, io2, endians2],
  stew/shims/macros,
  eth/keyfile/uuid,
  blscurve,
  json_serialization,
...
func validateMnemonic*(inputWords: string, outputMnemonic: var Mnemonic): bool =
...
func deriveChildKey*(parentKey: ValidatorPrivKey, index: Natural): ValidatorPrivKey =
  let success =
...
func deriveMasterKey*(mnemonic: Mnemonic, password: KeystorePass): ValidatorPrivKey =
  deriveMasterKey(getSeed(mnemonic, password))

func deriveChildKey*(masterKey: ValidatorPrivKey, path: KeyPath): ValidatorPrivKey =
  result = masterKey
...
# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.2/specs/phase0/validator.md#bls_withdrawal_prefix
func makeWithdrawalCredentials*(k: ValidatorPubKey): Eth2Digest =
  var bytes = eth2digest(k.toRaw())
  bytes.data[0] = BLS_WITHDRAWAL_PREFIX.uint8
  bytes

# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.0/specs/phase0/deposit-contract.md#withdrawal-credentials
