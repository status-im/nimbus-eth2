...
declareCounter nbc_remote_signer_requests, "Number of remote signer requests"

declareCounter nbc_remote_signer_signatures, "Number of remote signer signatures"

declareCounter nbc_remote_signer_failures, "Number of remote signer signatures"

declareCounter nbc_remote_signer_200_responses, "Number of 200 responses (signature)"
...
declareCounter nbc_remote_signer_communication_errors, "Number of communication errors"
...
proc getUpcheck*(): RestResponse[Web3SignerStatusResponse] {.
  rest, endpoint: "/upcheck", meth: MethodGet, accept: "application/json"
.} ## https://consensys.github.io/web3signer/web3signer-eth2.html#tag/Server-Status

proc reload*(): RestPlainResponse {.
  rest, endpoint: "/reload", meth: MethodPost, accept: "application/json"
.}
...
proc getKeys*(): RestResponse[Web3SignerKeysResponse] {.
  rest, endpoint: "/api/v1/eth2/publicKeys", meth: MethodGet, accept: "application/json"
...
proc signDataPlain*(
  identifier: ValidatorPubKey, body: Web3SignerRequest
): RestPlainResponse {.
  rest,
  endpoint: "/api/v1/eth2/sign/{identifier}",
  meth: MethodPost,
  accept: "application/json"
.} # https://consensys.github.io/web3signer/web3signer-eth2.html#tag/Signing

proc init(
    t: typedesc[Web3SignerError], kind: Web3SignerErrorKind, message: string
): Web3SignerError =
  Web3SignerError(kind: kind, message: message)

proc signData*(
    client: RestClientRef, identifier: ValidatorPubKey, body: Web3SignerRequest
  ...
  let
    ...
    response =
      try:
        let
          res = await client.signDataPlain(
            identifier, body, restAcceptType = "application/json"
          )
          duration = Moment.now() - startSignMoment
        nbc_remote_signer_duration.observe(float(milliseconds(duration)) / 1000.0)
        res
  ...
  return
      ...
      let sig =
        if response.contentType.isNone() or
          ...
          if mediaType == TextPlainMediaType:
            ...
            let res = decodeBytes(
              Web3SignerSignatureResponse, response.data, response.contentType
            ).valueOr:
              inc(nbc_remote_signer_failures)
              return Web3SignerDataResponse.err(
                Web3SignerError.init(
    ...
    of 400:
      ...
      let message = block:
        let res =
          decodeBytes(Web3SignerErrorResponse, response.data, response.contentType)
        if res.isErr():
  ...
  while true:
    ...
    if not (operationFut.finished()):
      ...
      if lastError.isSome():
        ...
        return Web3SignerDataResponse.err(
          Web3SignerError.init(Web3SignerErrorKind.TimeoutError, "Operation timed out")
        )
    else:
      ...
      of Web3SignerErrorKind.Error404, Web3SignerErrorKind.Error412,
          Web3SignerErrorKind.Error500, Web3SignerErrorKind.CommError,
        ...
        if attempt == attemptsCount:
          ...
          if currentTimeout < len(BackoffTimeouts) - 1:
            inc currentTimeout
      of Web3SignerErrorKind.Error400, Web3SignerErrorKind.UknownStatus,
          Web3SignerErrorKind.InvalidContentType, Web3SignerErrorKind.InvalidPlain,
          Web3SignerErrorKind.InvalidContent, Web3SignerErrorKind.InvalidSignature:
        # Critical errors
        return resp
