...
import std/[sequtils, strutils, tables], stew/[base10, byteutils], ../forks
...
func getOrDefault*(info: VCRuntimeConfig, name: string, defaultValue: uint64): uint64 =
  let numstr =
    try:
      info[name]
    except KeyError:
...
func getConsensusForkConfig*(
    info: VCRuntimeConfig, optionalForks: set[ConsensusFork] = {}
  ...
  if len(presence) != (int(high(ConsensusFork)) + 1):
    ...
    return err(
      "Some forks missing in configuration [" & missingForks.mapIt($it).join(", ") & "]"
    )
    ...
    for index, fork in forks.pairs():
      if index > 0:
        if config[forks[index]].epoch < config[forks[index - 1]].epoch:
          return err(
            "Fork epochs are inconsistent, " & $forks[index] & " is scheduled at epoch " &
