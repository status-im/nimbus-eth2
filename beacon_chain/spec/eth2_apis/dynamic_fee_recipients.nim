...
import std/tables, stew/results, chronicles, ../datatypes/base

logScope:
  topics = "fee_recipient"
...
proc addMapping*(
    store: var DynamicFeeRecipientsStore,
    validator: ValidatorIndex,
    feeRecipient: Eth1Address,
    currentEpoch: Epoch,
) =
  var updated = false
  store.mappings.withValue(validator, entry):
    updated = not (entry[].recipient == feeRecipient)
    entry[] = Entry(recipient: feeRecipient, addedAt: currentEpoch)
  do:
    updated = true
    store.mappings[validator] = Entry(recipient: feeRecipient, addedAt: currentEpoch)
  if updated:
...
func getDynamicFeeRecipient*(
    store: var DynamicFeeRecipientsStore, validator: ValidatorIndex, currentEpoch: Epoch
): Opt[Eth1Address] =
  store.mappings.withValue(validator, entry):
...
func pruneOldMappings*(store: var DynamicFeeRecipientsStore, currentEpoch: Epoch) =
