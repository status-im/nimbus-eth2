...
from ./datatypes/capella import
  BeaconState, MAX_WITHDRAWALS_PER_PAYLOAD, SignedBLSToExecutionChange, Withdrawal
...
proc check_attester_slashing*(
    state: ForkyBeaconState, attester_slashing: SomeAttesterSlashing, flags: UpdateFlags
...
func findValidatorIndex*(
    state: ForkyBeaconState, pubkey: ValidatorPubKey
): Opt[ValidatorIndex] =
    ...
    if verify_deposit_signature(cfg, deposit.data):
      ...
      when state is altair.BeaconState or state is bellatrix.BeaconState or
        ...
        if not state.previous_epoch_participation.add(ParticipationFlags(0)):
          return
            err("process_deposit: too many validators (previous_epoch_participation)")
        if not state.current_epoch_participation.add(ParticipationFlags(0)):
          return
            err("process_deposit: too many validators (current_epoch_participation)")
      ...
      # index correct
      trace "Skipping deposit with invalid signature", deposit = shortLog(deposit.data)
...
func process_withdrawals*(
  ...
  if not (len(payload.withdrawals) == len(expected_withdrawals)):
    return
      err("process_withdrawals: different numbers of payload and expected withdrawals")
  ...
  for i in 0 ..< len(expected_withdrawals):
    ...
    let validator_index = ValidatorIndex.init(expected_withdrawals[i].validator_index).valueOr:
      return err("process_withdrawals: invalid validator index")
    decrease_balance(state, validator_index, expected_withdrawals[i].amount)
...
func kzg_commitment_to_versioned_hash*(kzg_commitment: KzgCommitment): VersionedHash =
