...
proc init*(
    T: type BatchVerifier, rng: ref HmacDrbgContext, taskpool: TaskPoolPtr
): BatchVerifier =
  BatchVerifier(
...
proc collectSignatureSets*(
  ...
  sigs.add block_signature_set(
    ...
    signed_block.root,
    proposer_key,
    signed_block.signature.load.valueOr do:
  ...
  for i in 0 ..< signed_block.message.body.proposer_slashings.len:
    ...
    # Alias
    template slashing(): untyped =
      signed_block.message.body.proposer_slashings[i]
    ...
    block:
      ...
      sigs.add block_signature_set(
        ...
        header.message,
        key,
        header.signature.load().valueOr do:
  ...
  for i in 0 ..< signed_block.message.body.attester_slashings.len:
    ...
    # Alias
    template slashing(): untyped =
      signed_block.message.body.attester_slashings[i]
  ...
  for i in 0 ..< signed_block.message.body.voluntary_exits.len:
    ...
    # due to https://github.com/nim-lang/Nim/issues/14421
    # fixed in 1.4.2
    template volex(): untyped =
      signed_block.message.body.voluntary_exits[i]
    ...
    sigs.add voluntary_exit_signature_set(
      (
        ...
        if state.kind >= ConsensusFork.Capella: capella_fork else: fork
      ),
      genesis_validators_root,
      volex.message,
      key,
        ...
        when consensusFork >= ConsensusFork.Capella:
          for bls_change in signed_block.message.body.bls_to_execution_changes:
            let sig = bls_change.signature.load.valueOr:
              return err(
                "collectSignatureSets: cannot load BLS to execution change signature"
              )
            ...
            let validator_pubkey = bls_change.message.from_bls_pubkey.loadWithCache.valueOr:
              return
                err("collectSignatureSets: cannot load BLS to execution change pubkey")
