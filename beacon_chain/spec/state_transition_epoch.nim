...
# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.4/specs/bellatrix/beacon-chain.md#epoch-processing
...
import
  stew/assign2, chronicles, ../extras, "."/[beaconstate, eth2_merkleization, validator]
...
func init*(info: var phase0.EpochInfo, state: phase0.BeaconState) =
  ...
  for i in 0 ..< state.validators.len:
    ...
    info.validators[i] =
      RewardStatus(current_epoch_effective_balance: v[].effective_balance, flags: flags)
...
func process_attestation(
  ...
  if a.data.target.epoch == state.get_current_epoch():
    ...
    is_previous_epoch_attester = Opt.some(
      InclusionInfo(delay: a.inclusion_delay, proposer_index: a.proposer_index)
    )
  ...
  for validator_index in get_attesting_indices_iter(
    ...
    template v(): untyped =
      info.validators[validator_index]
...
func is_eligible_validator*(validator: RewardStatus): bool =
  validator.flags.contains(RewardFlags.isActiveInPreviousEpoch) or (
    validator.flags.contains(RewardFlags.isSlashed) and
    not (validator.flags.contains RewardFlags.canWithdrawInCurrentEpoch)
  )
...
# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.4/specs/altair/beacon-chain.md#get_unslashed_participating_indices
  ...
  for validator_index in 0'u64 ..< state.validators.lenu64:
    let
      is_active_current_epoch =
        is_active_validator(state.validators[validator_index], current_epoch)
      validator_effective_balance = state.validators[validator_index].effective_balance
    ...
    let
      is_active_previous_epoch =
        is_active_validator(state.validators[validator_index], previous_epoch)
      previous_epoch_participation = state.previous_epoch_participation[validator_index]
    ...
    # and merge
    if is_active_current_epoch and
        has_flag(
          state.current_epoch_participation[validator_index], TIMELY_TARGET_FLAG_INDEX
        ):
...
type JustificationAndFinalizationInfo = object
  previous_justified_checkpoint: Checkpoint
  current_justified_checkpoint: Checkpoint
  finalized_checkpoint: Checkpoint
  justification_bits: JustificationBits
...
proc weigh_justification_and_finalization(
  ...
  res.justification_bits = JustificationBits(
    (uint8(res.justification_bits) shl 1) and uint8((2 ^ JUSTIFICATION_BITS_LENGTH) - 1)
  )

  if previous_epoch_target_balance * 3 >= total_active_balance * 2:
    res.current_justified_checkpoint =
      Checkpoint(epoch: previous_epoch, root: get_block_root(state, previous_epoch))
  ...
  if current_epoch_target_balance * 3 >= total_active_balance * 2:
    res.current_justified_checkpoint =
      Checkpoint(epoch: current_epoch, root: get_block_root(state, current_epoch))
...
proc process_justification_and_finalization*(
    state: var phase0.BeaconState, balances: TotalBalances, flags: UpdateFlags = {}
) =
  ...
  let jfRes = weigh_justification_and_finalization(
    state, balances.current_epoch, balances.previous_epoch_target_attesters,
    balances.current_epoch_target_attesters, flags,
  )
  assign(state.previous_justified_checkpoint, jfRes.previous_justified_checkpoint)
  assign(state.current_justified_checkpoint, jfRes.current_justified_checkpoint)
...
proc compute_unrealized_finality*(
    state: phase0.BeaconState, cache: var StateCache
): FinalityCheckpoints =
  if get_current_epoch(state) <= GENESIS_EPOCH + 1:
  ...
  template balances(): auto =
    info.balances

  var finalityState = state.toFinalityState()
  let jfRes = weigh_justification_and_finalization(
    finalityState, balances.current_epoch, balances.previous_epoch_target_attesters,
    balances.current_epoch_target_attesters,
  )
  FinalityCheckpoints(
    justified: jfRes.current_justified_checkpoint, finalized: jfRes.finalized_checkpoint
  )
...
# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.4/specs/phase0/beacon-chain.md#helpers
func get_base_reward_sqrt*(
    state: phase0.BeaconState, index: ValidatorIndex, total_balance_sqrt: auto
): Gwei =
  # Spec function recalculates total_balance every time, which creates an
  # O(n^2) situation.
  let effective_balance = state.validators[index].effective_balance
  effective_balance * BASE_REWARD_FACTOR div total_balance_sqrt div
...
func get_attestation_component_delta(
  ...
  if is_unslashed_attester:
    RewardDelta(
      rewards: get_attestation_component_reward(
        attesting_balance, total_balance, base_reward, finality_delay
      )
    )
  else:
...
func get_inclusion_delay_delta*(
    validator: RewardStatus, base_reward: Gwei
): (RewardDelta, Opt[(uint64, RewardDelta)]) =
  ## Return proposer and inclusion delay micro-rewards/penalties for each validator.
  if validator.is_previous_epoch_attester.isSome() and
      ((not validator.flags.contains(RewardFlags.isSlashed))):
    let
...
func get_inactivity_penalty_delta*(
    validator: RewardStatus, base_reward: Gwei, finality_delay: uint64
): RewardDelta =
...
# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.4/specs/phase0/beacon-chain.md#get_attestation_deltas
func get_attestation_deltas(state: phase0.BeaconState, info: var phase0.EpochInfo) =
  ...
  for index, validator in info.validators.mpairs():
    ...
    let
      source_delta =
        get_source_delta(validator, base_reward, info.balances, finality_delay)
      target_delta =
        get_target_delta(validator, base_reward, info.balances, finality_delay)
      head_delta = get_head_delta(validator, base_reward, info.balances, finality_delay)
    ...
    if proposer_delta.isSome:
      ...
      if proposer_index < info.validators.lenu64:
        info.validators[proposer_index].delta.add(proposer_delta.get()[1])
...
func get_unslashed_participating_increment*(
    info: altair.EpochInfo, flag_index: TimelyFlag
): uint64 =
...
func get_active_increments*(info: altair.EpochInfo): uint64 =
...
template get_flag_and_inactivity_delta(
  ...
  (
    vidx,
    reward(TIMELY_SOURCE_FLAG_INDEX),
    reward(TIMELY_TARGET_FLAG_INDEX),
    reward(TIMELY_HEAD_FLAG_INDEX),
    penalty(TIMELY_SOURCE_FLAG_INDEX),
    penalty(TIMELY_TARGET_FLAG_INDEX),
    inactivity_penalty,
  )
...
iterator get_flag_and_inactivity_deltas*(
  ...
  static:
    doAssert ord(high(TimelyFlag)) == 2

  let
    previous_epoch = get_previous_epoch(state)
    active_increments = get_active_increments(info)
    penalty_denominator = cfg.INACTIVITY_SCORE_BIAS * INACTIVITY_PENALTY_QUOTIENT
    epoch_participation =
...
func process_registry_updates*(
  ...
  for vidx in state.validators.vindices:
    if is_eligible_for_activation_queue(typeof(state).kind, state.validators.item(vidx)):
    ...
    if is_active_validator(state.validators.item(vidx), get_current_epoch(state)) and
      ...
      maybe_exit_queue_info =
        Opt.some (?initiate_validator_exit(cfg, state, vidx, exit_queue_info, cache))
  ...
  for i in 0 ..< activation_queue.len:
    ...
    state.validators.mitem(high(distinctBase(ValidatorIndex)) - vidx_complement).activation_epoch =
...
func get_adjusted_total_slashing_balance*(
  ...
  const multiplier =
    ...
    when state is phase0.BeaconState:
      ...
      state is fulu.BeaconState or state is gloas.BeaconState:
...
func get_slashing_penalty*(
    ...
    validator: Validator,
    adjusted_total_slashing_balance, total_balance: Gwei,
): Gwei =
  ...
  when consensusFork <= ConsensusFork.Deneb:
    let penalty_numerator =
      validator.effective_balance div increment * adjusted_total_slashing_balance
    penalty_numerator div total_balance * increment
  elif consensusFork in [ConsensusFork.Electra, ConsensusFork.Fulu, ConsensusFork.Gloas]:
    let
...
# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.0/specs/phase0/beacon-chain.md#slashings-balances-updates
...
# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.4/specs/phase0/beacon-chain.md#randao-mixes-updates
...
func compute_historical_root*(state: var ForkyBeaconState): Eth2Digest =
  ...
  hash_tree_root([hash_tree_root(state.block_roots), hash_tree_root(state.state_roots)])
...
# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.4/specs/altair/beacon-chain.md#participation-flags-updates
...
template compute_inactivity_update(
  ...
  if not_in_inactivity_leak:
    inactivity_score -= min(cfg.INACTIVITY_SCORE_RECOVERY_RATE, inactivity_score)
  inactivity_score

# https://github.com/ethereum/consensus-specs/blob/v1.5.0-beta.4/specs/altair/beacon-chain.md#inactivity-scores
...
func apply_pending_deposit(
    ...
    validator_index: Opt[ValidatorIndex],
): Result[void, cstring] =
...
func process_pending_deposits*(
  ...
  for deposit in state.pending_deposits:
    ...
    let index = findValidatorIndex(state.validators.asSeq, bsv[], deposit.pubkey)
    if index.isOk:
...
func process_pending_consolidations*(
  ...
  for pending_consolidation in state.pending_consolidations:
    ...
    let
      source_validator_index = ValidatorIndex.init(pending_consolidation.source_index).valueOr:
        return err("process_pending_consolidations: source index out of range")
      target_validator_index = ValidatorIndex.init(pending_consolidation.target_index).valueOr:
  ...
  state.pending_consolidations = HashList[
    PendingConsolidation, Limit PENDING_CONSOLIDATIONS_LIMIT
  ].init(state.pending_consolidations.asSeq[next_pending_consolidation ..^ 1])
...
func process_proposer_lookahead*(
  ...
  let
    ...
    new_proposers = get_beacon_proposer_indices(state, next_epoch)

  for i in 0 ..< SLOTS_PER_EPOCH:
    if new_proposers[i].isSome():
      mitem(state.proposer_lookahead, last_epoch_start + i) =
        new_proposers[i].get.uint64
...
func get_builder_payment_quorum_threshold(
    state: gloas.BeaconState, cache: var StateCache
): uint64 =
  ## Calculate the quorum threshold for builder payments.
  let quorum = (
    get_total_active_balance(state, cache) div SLOTS_PER_EPOCH *
...
func process_builder_pending_payments*(
  ...
  for index in 0 ..< min(state.builder_pending_payments.len, SLOTS_PER_EPOCH.int):
    ...
    if payment.weight.distinctBase > quorum:
      ...
      if not state.builder_pending_withdrawals.add(payment.withdrawal):
        return err(
          "process_builder_pending_payments: couldn't add to builder_pending_withdrawals"
        )
