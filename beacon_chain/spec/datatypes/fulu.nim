...
from ./electra import Attestation, AttesterSlashing, AggregateAndProof, ElectraCommitteeValidatorsBits,
  SignedAggregateAndProof, AttestationCommitteeBits, PendingDeposit, PendingPartialWithdrawal,
  WithdrawalRequest, PendingConsolidation, ConsolidationRequest, DepositRequest,
  IndexedAttestation, ExecutionRequests, FINALIZED_ROOT_GINDEX_ELECTRA, 
  CURRENT_SYNC_COMMITTEE_GINDEX_ELECTRA, NEXT_SYNC_COMMITTEE_GINDEX_ELECTRA
...
type
  ...
  TrustedAttestation* = object
    ...
    # It DOES NOT imply that the state transition was verified.
    # Currently the code MUST verify the state transition as soon as the signature is verified
    aggregation_bits*: electra.ElectraCommitteeValidatorsBits
    data*: AttestationData
    signature*: TrustedSig
    committee_bits*: electra.AttestationCommitteeBits  # [New in Electra:EIP7549]

  SomeIndexedAttestation* = electra.IndexedAttestation | TrustedIndexedAttestation
  SomeAttesterSlashing* = electra.AttesterSlashing | TrustedAttesterSlashing
...
func is_valid_light_client_header*(
  ...
  if epoch < cfg.CAPELLA_FORK_EPOCH:
    return
      header.execution == static(default(fulu.ExecutionPayloadHeader)) and
...
iterator getValidatorIndices*(attester_slashing: electra.AttesterSlashing | TrustedAttesterSlashing): uint64 =
...
func init*(
    T: type electra.Attestation,
    committee_index: CommitteeIndex,
    indices_in_committee: openArray[uint64],
    committee_len: int,
    data: AttestationData,
    signature: ValidatorSig): Result[T, cstring] =
  var committee_bits: electra.AttestationCommitteeBits
  committee_bits[int(committee_index)] = true

  var bits = electra.ElectraCommitteeValidatorsBits.init(committee_len)
