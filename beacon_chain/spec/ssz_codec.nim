...
template toSszType*(v: Gwei): auto =
  uint64(v)
...
template toSszType*(v: Eth1Address): auto =
  v.data()
...
func fromSszBytes*(
    T: type Eth1Address, bytes: openArray[byte]
): T {.raises: [SszError].} =
...
template toSszType*(v: HashedValidatorPubKey): auto =
  toRaw(v.pubkey)
