...
func upgrade_to_verge*(cfg: RuntimeConfig, pre: capella.BeaconState):
  ...
  let
    ...
    latest_execution_payload_header = electra.ExecutionPayloadHeader(
      ...
      execution_witness_root: Eth2Digest()  # [New in Verge]
    )
  ...
  (ref electra.BeaconState)(
    ...
    fork: Fork(
      previous_version: pre.fork.current_version,
      current_version: cfg.VERGE_FORK_VERSION, # [Modified in Verge]
      epoch: epoch
    ),
