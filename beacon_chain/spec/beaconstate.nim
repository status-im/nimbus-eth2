...
func get_validator_from_deposit*(deposit: DepositData): Validator =
  let
    amount = deposit.amount
    effective_balance =
      min(amount - amount mod EFFECTIVE_BALANCE_INCREMENT, MAX_EFFECTIVE_BALANCE)
...
func get_attestation_participation_flag_indices(
    state: deneb.BeaconState, data: AttestationData, inclusion_delay: uint64
): set[TimelyFlag] =
  ## Return the flag indices that are satisfied by an attestation.
  let justified_checkpoint =
    if data.target.epoch == get_current_epoch(state):
...
func is_partially_withdrawable_validator(validator: Validator, balance: Gwei): bool =
...
proc initialize_beacon_state_from_eth1(
  ...
  state.eth1_data.deposit_root = mixInLength(merkleizer.getFinalHash(), deposits.len)
  ...
  for idx, deposit in deposits:
    ...
    do:
      if skipBlsValidation in flags or verify_deposit_signature(cfg, deposit):
  ...
  for vidx in state.validators.vindices:
    ...
    validator.effective_balance =
      min(balance - balance mod EFFECTIVE_BALANCE_INCREMENT, MAX_EFFECTIVE_BALANCE)
...
proc initialize_beacon_state_from_eth1*(
  ...
  let
    ...
    fork = Fork(
      previous_version: forkVersion, current_version: forkVersion, epoch: GENESIS_EPOCH
    )
...
func upgrade_to_altair*(
    cfg: RuntimeConfig, pre: phase0.BeaconState
...
func upgrade_to_bellatrix*(
    cfg: RuntimeConfig, pre: altair.BeaconState
...
func upgrade_to_capella*(
    cfg: RuntimeConfig, pre: bellatrix.BeaconState
...
func upgrade_to_deneb*(
    cfg: RuntimeConfig, pre: capella.BeaconState
