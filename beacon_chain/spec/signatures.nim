...
func compute_block_signing_root*(
    ...
    slot: Slot,
    blck:
      Eth2Digest | SomeForkyBeaconBlock | BeaconBlockHeader |
...
func compute_deposit_signing_root(
    version: Version, deposit_message: DepositMessage
): Eth2Digest =
  let
...
func get_deposit_signature*(
    preset: RuntimeConfig, deposit: DepositData, privkey: ValidatorPrivKey
): CookedSig =
...
func get_deposit_signature*(
    message: DepositMessage, version: Version, privkey: ValidatorPrivKey
): CookedSig =
...
proc verify_deposit_signature(
    preset: RuntimeConfig, deposit: DepositData, pubkey: CookedPubKey
): bool =
  let
    deposit_message = deposit.getDepositMessage()
    signing_root =
      compute_deposit_signing_root(preset.GENESIS_FORK_VERSION, deposit_message)

  blsVerify(pubkey, signing_root.data, deposit.signature)

proc verify_deposit_signature*(preset: RuntimeConfig, deposit: DepositData): bool =
...
func compute_builder_signing_root(
    fork: Fork,
    msg: capella_mev.BuilderBid | deneb_mev.BuilderBid | ValidatorRegistrationV1,
): Eth2Digest =
  # Uses genesis fork version regardless
  doAssert fork.current_version == fork.previous_version

  let domain = get_domain(fork, DOMAIN_APPLICATION_BUILDER, GENESIS_EPOCH, ZERO_HASH)
  compute_signing_root(msg, domain)
...
func compute_bls_to_execution_change_signing_root*(
    genesisFork: Fork, genesis_validators_root: Eth2Digest, msg: BLSToExecutionChange
): Eth2Digest =
  ...
  let domain = get_domain(
    genesisFork, DOMAIN_BLS_TO_EXECUTION_CHANGE, GENESIS_EPOCH, genesis_validators_root
  )
  compute_signing_root(msg, domain)
