...
import ./datatypes/[phase0, altair, bellatrix], ./helpers, ./eth2_merkleization
...
template withTrust(sig: SomeSig, body: untyped): bool =
  when sig is TrustedSig: true else: body
...
func compute_slot_signing_root*(
  ...
  let
    epoch = epoch(slot)
    domain = get_domain(fork, DOMAIN_SELECTION_PROOF, epoch, genesis_validators_root)
...
func compute_block_signing_root*(
  ...
  let
    epoch = epoch(slot)
    domain = get_domain(fork, DOMAIN_BEACON_PROPOSER, epoch, genesis_validators_root)
...
func get_block_signature*(
  ...
  let signing_root =
    compute_block_signing_root(fork, genesis_validators_root, slot, root)
...
func compute_aggregate_and_proof_signing_root*(
  ...
  let
    ...
    domain =
      get_domain(fork, DOMAIN_AGGREGATE_AND_PROOF, epoch, genesis_validators_root)
...
func compute_attestation_signing_root*(
    fork: Fork, genesis_validators_root: Eth2Digest, attestation_data: AttestationData
): Eth2Digest =
  let
    epoch = attestation_data.target.epoch
    domain = get_domain(fork, DOMAIN_BEACON_ATTESTER, epoch, genesis_validators_root)
...
func compute_deposit_signing_root(
    genesis_fork_version: Version, deposit_message: DepositMessage
): Eth2Digest =
  let
...
func get_deposit_signature*(
    genesis_fork_version: Version, message: DepositMessage, privkey: ValidatorPrivKey
): CookedSig =
...
func get_deposit_signature*(
    genesis_fork_version: Version, deposit: DepositData, privkey: ValidatorPrivKey
): CookedSig =
  get_deposit_signature(genesis_fork_version, deposit.getDepositMessage(), privkey)

proc verify_deposit_signature(
    genesis_fork_version: Version, deposit: DepositData, pubkey: CookedPubKey
): bool =
  let
...
proc verify_deposit_signature*(
    genesis_fork_version: Version, deposit: DepositData
): bool =
...
func compute_voluntary_exit_signing_root*(
    fork: Fork, genesis_validators_root: Eth2Digest, voluntary_exit: VoluntaryExit
): Eth2Digest =
  let
    epoch = voluntary_exit.epoch
    domain = get_domain(fork, DOMAIN_VOLUNTARY_EXIT, epoch, genesis_validators_root)
  compute_signing_root(voluntary_exit, domain)

func voluntary_exit_signature_fork(
    is_post_deneb: static bool, state_fork: Fork, capella_fork_version: Version
): Fork =
  when is_post_deneb:
    ...
    Fork(
      ...
      epoch: GENESIS_EPOCH,
    ) # irrelevant when current/previous identical
  else:
    state_fork

func voluntary_exit_signature_fork*(
    consensusFork: static ConsensusFork, state_fork: Fork, capella_fork_version: Version
): Fork =
...
func compute_sync_committee_message_signing_root*(
  ...
  let domain =
    get_domain(fork, DOMAIN_SYNC_COMMITTEE, slot.epoch, genesis_validators_root)
...
func compute_contribution_and_proof_signing_root*(
    fork: Fork, genesis_validators_root: Eth2Digest, msg: ContributionAndProof
): Eth2Digest =
  let domain = get_domain(
    fork, DOMAIN_CONTRIBUTION_AND_PROOF, msg.contribution.slot.epoch,
...
func is_sync_committee_aggregator*(signature: ValidatorSig): bool =
  let
    ...
    modulo = max(
      1'u64,
      (SYNC_COMMITTEE_SIZE div SYNC_COMMITTEE_SUBNET_COUNT) div
...
func compute_builder_signing_root(
    ...
    msg: electra_mev.BuilderBid | fulu_mev.BuilderBid | ValidatorRegistrationV1,
): Eth2Digest =
  ...
  let domain =
    compute_domain(DOMAIN_APPLICATION_BUILDER, genesis_fork_version, ZERO_HASH)
  compute_signing_root(msg, domain)
...
func compute_execution_payload_bid_signing_root(
  ...
  let domain = get_domain(fork, DOMAIN_BEACON_BUILDER, epoch, genesis_validators_root)
  compute_signing_root(msg, domain)
...
func compute_payload_attestation_message_signing_root*(
    fork: Fork, genesis_validators_root: Eth2Digest, msg: PayloadAttestationMessage
): Eth2Digest =
  let
    epoch = msg.data.slot.epoch
    domain = get_domain(fork, DOMAIN_PTC_ATTESTER, epoch, genesis_validators_root)
  compute_signing_root(msg, domain)
...
func get_payload_attestation_message_signature*(
  ...
  let signing_root =
    compute_payload_attestation_message_signing_root(fork, genesis_validators_root, msg)
  blsSign(privkey, signing_root.data)
