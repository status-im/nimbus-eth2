...
proc getStatus*(): RestPlainResponse {.
  rest, endpoint: "/eth/v1/builder/status", meth: MethodGet
...
proc getHeader*(
    client: RestClientRef, slot: Slot, parent_hash: Eth2Digest, pubkey: ValidatorPubKey
...
proc submitBlindedBlockV2Plain*(
  ...
  rest,
  endpoint: "/eth/v2/builder/blinded_blocks",
  meth: MethodPost,
  connection: {Dedicated, Close}
.}
...
proc submitBlindedBlock*(
    client: RestClientRef, body: electra_mev.SignedBlindedBeaconBlock
): Future[RestPlainResponse] {.
    async: (
      raises:
        [CancelledError, RestEncodingError, RestDnsResolveError, RestCommunicationError],
...
proc submitBlindedBlock*(
    client: RestClientRef, body: fulu_mev.SignedBlindedBeaconBlock
): Future[RestPlainResponse] {.
    async: (
      raises:
        [CancelledError, RestEncodingError, RestDnsResolveError, RestCommunicationError],
