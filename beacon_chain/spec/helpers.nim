...
export eth2_merkleization, forks, rlp, ssz_codec
...
func create_blob_sidecars*(
    forkyBlck: deneb.SignedBeaconBlock, kzg_proofs: KzgProofs, blobs: Blobs
): seq[BlobSidecar] =
...
func is_execution_block*(blck: SomeForkyBeaconBlock): bool =
  when typeof(blck).kind >= ConsensusFork.Bellatrix:
    const defaultExecutionPayload = default(typeof(blck.body.execution_payload))
...
func is_merge_transition_block(
    state: bellatrix.BeaconState | capella.BeaconState | deneb.BeaconState,
    body:
      bellatrix.BeaconBlockBody | bellatrix.TrustedBeaconBlockBody |
      bellatrix.SigVerifiedBeaconBlockBody | capella.BeaconBlockBody |
      capella.TrustedBeaconBlockBody | capella.SigVerifiedBeaconBlockBody |
...
proc computeTransactionsTrieRoot*(payload: ForkyExecutionPayload): ExecutionHash256 =
...
func toExecutionWithdrawal*(withdrawal: capella.Withdrawal): ExecutionWithdrawal =
  ExecutionWithdrawal(
