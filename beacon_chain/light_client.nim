...
proc installMessageValidators*(
  ...
  template validate[T: SomeForkyLightClientObject](
      msg: T, contextFork: ConsensusFork, validatorProcName: untyped
  ): ValidationResult =
    msg.logReceived()
...
proc updateGossipStatus*(
  ...
  let
    ...
    currentEpochTargetGossipState = getTargetGossipState(
      epoch, cfg.ALTAIR_FORK_EPOCH, cfg.BELLATRIX_FORK_EPOCH, cfg.CAPELLA_FORK_EPOCH,
      cfg.DENEB_FORK_EPOCH, isBehind,
    )
    targetGossipState =
