...
proc getBlockSZ*(f: EraFile, slot: Slot, bytes: var seq[byte]): Result[void, string] =
  ...
  let pos = f[].blockIdx.offsets[slot - f[].blockIdx.startSlot]
...
proc getBlockSSZ*(f: EraFile, slot: Slot, bytes: var seq[byte]): Result[void, string] =
  ...
  let len = uncompressedLenFramed(tmp).valueOr:
...
proc getStateSZ*(f: EraFile, slot: Slot, bytes: var seq[byte]): Result[void, string] =
...
proc getStateSSZ*(
    f: EraFile, slot: Slot, bytes: var seq[byte], partial = Opt.none(int)
): Result[void, string] =
  var tmp: seq[byte]
...
proc verify*(f: EraFile, cfg: RuntimeConfig): Result[Eth2Digest, string] =
  ...
  let state =
    try:
      newClone(readSszForkedHashedBeaconState(cfg, tmp))
  ...
  if era > 0:
    ...
    for slot in (era - 1).start_slot() ..< era.start_slot():
      ...
      if tmp.len > 0:
        let blck =
          try:
            newClone(readSszForkedSignedBeaconBlock(cfg, tmp))
          except CatchableError as exc:
        ...
        if slot > GENESIS_SLOT:
          ...
          if sig.isNone():
            warn "Signature invalid", sig = blck[].signature, blck = shortLog(blck[])
...
proc getEraFile(
  ...
  let
    eraRoot = eraRoot(
      db.genesis_validators_root, historical_roots, historical_summaries, era
    ).valueOr:
  ...
  let f = EraFile.open(path).valueOr:
...
proc getBlockSZ*(
  ...
  # Block content for the blocks of an era is found in the file for the _next_
  # era
  let f = ?db.getEraFile(historical_roots, historical_summaries, slot.era + 1)
...
proc getBlock*(
    ...
    slot: Slot,
    root: Opt[Eth2Digest],
    T: type ForkyTrustedSignedBeaconBlock,
): Opt[T] =
  var bytes: seq[byte]
  ?db.getBlockSSZ(historical_roots, historical_summaries, slot, bytes).mapConvertErr(
    void
  )
  if bytes.len() == 0:
    return Opt.none(T)
...
proc getStateSZ*(
  ...
  # Block content for the blocks of an era is found in the file for the _next_
  # era
  let f = ?db.getEraFile(historical_roots, historical_summaries, slot.era)
...
type PartialBeaconState = object
...
proc getPartialState(
    ...
    historical_summaries: openArray[HistoricalSummary],
    slot: Slot,
    output: var PartialBeaconState,
): bool =
  static:
    doAssert isFixedSize(PartialBeaconState)
  ...
  var tmp: seq[byte]
  if (
    let e = db.getStateSSZ(
      historical_roots, historical_summaries, slot, tmp, Opt[int].ok(partialBytes)
    )
    e.isErr
  ):
    return false
...
iterator getBlockIds*(
  ...
  while true:
    ...
    let x = slot.uint64 mod state[].block_roots.lenu64
...
proc new*(
    T: type EraDB, cfg: RuntimeConfig, path: string, genesis_validators_root: Eth2Digest
): EraDB =
...
when isMainModule:
  ...
  let f = EraFile.open(dbPath & "/mainnet-00001-40cf2f3c.era").expect("opening works")
