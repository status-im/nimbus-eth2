...
proc initGenesis(node: BeaconNode) {.async.} =
  ...
  of GenesisType.snapshot:
    ...
    try:
      snapshotFile = conf.stateSnapshot.string
...
proc init*(T: type BeaconNode, conf: BeaconNodeConf): Future[BeaconNode] {.async.} =
  ...
  if conf.genesisType == eth1 and conf.depositWeb3Url.len != 0:
  ...
  var (nroot, nblck) = node.blockPool.withState(
    ...
    let (eth1data, deposits) =
      if node.mainchainMonitor.isNil:
        (
          get_eth1data_stub(state.eth1_deposit_index,
                           slot.compute_epoch_of_slot()),
          newSeq[Deposit]()
        )
      else:
