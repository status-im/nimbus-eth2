...
template tableName(sidecar: typedesc[ForkyDataSidecar]): string =
  when sidecar is deneb.BlobSidecar:
    ...
    static:
      raiseAssert "Sidecar's fork is not supported"
...
proc initDataSidecarStore(
  ...
  ok(
    DataSidecarStore(
      getStmt: getStmt, putStmt: putStmt, delStmt: delStmt, countStmt: countStmt
    )
  )

func close(store: var DataSidecarStore) =
  if not (isNil(distinctBase(store.getStmt))):
    store.getStmt.disposeSafe()
  if not (isNil(distinctBase(store.putStmt))):
    store.putStmt.disposeSafe()
  if not (isNil(distinctBase(store.delStmt))):
    store.delStmt.disposeSafe()
  if not (isNil(distinctBase(store.countStmt))):
    store.countStmt.disposeSafe()

iterator sidecars*(
    db: QuarantineDB, T: typedesc[ForkyDataSidecar], blockRoot: Eth2Digest
): T =
  ...
  if not (isNil(distinctBase(statement))):
    ...
    for rowRes in statement.exec(blockRoot.data, row):
      ...
      if not (decodeSZSSZ(row, res)):
        error "Quarantine store corrupted", store = storeName, blockRoot
        break
      yield res

proc putDataSidecars*[T: ForkyDataSidecar](
    db: QuarantineDB, blockRoot: Eth2Digest, dataSidecars: openArray[ref T]
) =
  ...
  if not (isNil(distinctBase(statement))):
    ...
    for sidecar in dataSidecars:
      ...
      statement.exec((blockRoot.data, blob)).expect("SQL query OK")
...
proc sidecarsCount*(db: QuarantineDB, T: typedesc[ForkyDataSidecar]): int64 =
...
proc initQuarantineDB*(backend: SqStoreRef): KvResult[QuarantineDB] =
  ...
  # guarantees.
  let
    electraDataSidecar = ?backend.initDataSidecarStore(tableName(deneb.BlobSidecar))
    fuluDataSidecar = ?backend.initDataSidecarStore(tableName(fulu.DataColumnSidecar))
